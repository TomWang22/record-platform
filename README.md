...
- Scaling factor: 1, prepared query mode, 64 clients, 12 threads, 60 s duration (1 try).
- Transactions processed: **1,682,715** with zero failures; avg latency **1.522 ms**, stddev **2.527 ms**, initial connect 31.8 ms.
- TPS: **28,033** (excluding connect). CSV artifacts live at `bench_sweep_20251113_034029.csv` and `bench_export_20251113_034029.csv`.
- The sweep log includes detailed rows for each TRGM/KNN variant (p95/p99, IO deltas, WAL counts) and is the reference point for future tuning.
