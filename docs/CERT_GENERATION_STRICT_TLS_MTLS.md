...
| **Edge / leaf** | `record.test.crt`, `.key` | `off-campus-housing-local-tls`, `service-tls` | Caddy, api-gateway → auth (gRPC client), backends (gRPC server) |
...
- `record.test.crt`, `record.test.key` — edge leaf for Caddy
...
- `off-campus-housing-local-tls` (leaf) in `ingress-nginx` and `record-platform`
- `dev-root-ca` in `ingress-nginx`, `record-platform`, `envoy-test`
- `service-tls` in `record-platform` (leaf + key + `ca.crt` for gRPC server and client)

Requires: `certs/record.test.crt`, `certs/record.test.key`, `certs/dev-root.pem`.
...
- **Secret:** `kafka-ssl-secret` in `record-platform` with:
...
|--------|--------------|----------|--------|
| `dev-root-ca` | `ingress-nginx`, `record-platform`, `envoy-test` | `dev-root.pem` | Caddy/backends trust; tests |
| `off-campus-housing-local-tls` | `ingress-nginx`, `record-platform` | `tls.crt`, `tls.key` (leaf) | Caddy TLS |
| `service-tls` | `record-platform` | `tls.crt`, `tls.key`, `ca.crt` | api-gateway (gRPC client), auth-service, messaging-service (gRPC server + probes) |
| `envoy-client-tls` | `envoy-test` | `envoy.crt`, `envoy.key` | Envoy (mTLS client to backends) |
| `kafka-ssl-secret` | `record-platform` | `ca-cert.pem`, `client.crt`, `client.key`, JKS/passwords | Kafka broker; messaging-service (and any Node Kafka client) |
