...
- **Pipeline helper**: `./scripts/rehydrate-and-tune.sh` runs the entire “restore + tuning + warmup + pgbench sweep” flow. Use it whenever you need the golden baseline.
- **kubectl shim**: helper scripts now prepend `scripts/shims/kubectl`, which retries transient `kubectl` TLS handshake timeouts (`KUBECTL_MAX_RETRIES`, `KUBECTL_REQUEST_TIMEOUT` overridable via env).
