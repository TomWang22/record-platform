...
- **Root:** `Caddyfile`, `docker-compose.yml` (Redis + Kafka + Zookeeper + Postgres; DB count per-project), **package.json**, **pnpm-workspace.yaml**, **pnpm-lock.yaml**, **tsconfig.base.json**, **.npmrc** (workspace root for housing-platform layout).
