...
Next allowed step:
Phase 32H-R1 COMPLETE with NO_CAUSAL_SEPARATION (do not claim host-suspension remediation). Phase 33A–33E COMPLETE; Phase 33F readiness + committed canary launcher package landed (`scripts/phase33f-launch-capability-canary.mjs`, `scripts/phase33f-runtime-status-readonly.mjs`, `scripts/lib/phase33f-canary-*.mjs`, live auth/QUIC preflight). Launcher smoke PASS under `/tmp/phase33f-canary-launcher-smoke-v1` (72 probes / 24 batches). Canary root `/tmp/phase33f-capability-gauntlet-canary-v1` and target root must remain ABSENT / NOT LAUNCHED until a new exact-SHA CI approval and a new explicit owner launch statement naming the post-launcher HEAD. Edge `record-platform.test` repaired (caddy image + python-ai Node runners); evidence under `/tmp/phase33f-edge-preflight-repair/`. Do not launch the 17280 target or Phase 33G without separate owner approval. Production enablement NOT APPROVED; default keyword; PERCENT=0; ALLOW_PROD_PERCENT=0; hybrid/vector default NOT ENABLED; automatic negotiation sending DISABLED. Canonical matrix: `scripts/ai-platform/intelligence-capability-matrix.json`.
