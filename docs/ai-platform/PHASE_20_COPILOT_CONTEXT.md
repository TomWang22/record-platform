...
**Last updated:** 2026-06-25 (T20.12T tranche 8 actual recorded)  
**Current main SHA:** `df70a96` (verify at commit time)  
...
Vector rollout: NOT APPROVED / NOT READY:
- embedded coverage: 11.0% / 8,065 FAIL (was 10.4% / 7,565 pre–Tranche 8; ≥10k gate still FAIL)
...
- T20.12W next embedding tranche actual write (requires new tranche id, fresh backup, dry-run — see `docs/ai-platform/T20-12V-tranche9-dry-run-plan.md`)
...
| **T20.12** | `t20-tranche-8` (ops) | Tranche 8 actual: +500 embeddings (7,565 → 8,065); post-OBO caps; FORCE not used; validation PASS; rollout still NOT APPROVED |

### Phase 20 tickets NOT started (require explicit approval)

| Ticket | Scope |
|--------|-------|
| **T20.12W** | Next bounded embedding tranche actual write (`t20-tranche-9` dry-run planned — see `docs/ai-platform/T20-12V-tranche9-dry-run-plan.md`) |
...
| **T20.12V** | Tranche 9 dry-run plan only — `docs/ai-platform/T20-12V-tranche9-dry-run-plan.md` |
...
Last tranche: t20-tranche-8 (+500, 2026-06-25; post-OBO caps)
```

### Embedded by source_type (post–Tranche 8; verify before next tranche)

| source_type | embedded |
|-------------|--------:|
| listing | 3,024 |
| obo_offer_summary | 1,544 |
| listing_revision | 1,500 |
| notification | 1,150 |
| record | 594 |
...
|------|--------|---------|--------|
| Embedded coverage | ≥15% or ≥10k embedded | 11.0% / 8,065 | **FAIL** |
...
5. **Do NOT** rerun actual Tranche 2–8 writes — locks exist; blocked exit **2**.
...
1. T20.12W next tranche actual write (`t20-tranche-9` dry-run planned — see T20-12V)
2. No T20.14/T20.15 rollout work until all gates pass
```

Do **not** start T20.12W actual write without explicit approval (`Approved: start T20.12W actual t20-tranche-9 write`).
...
3. **If rerunning Tranche 2–8** → refuse; locks exist (exit **2**).
