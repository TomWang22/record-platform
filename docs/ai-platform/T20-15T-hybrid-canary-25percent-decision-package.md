# T20.15T — Hybrid canary 25% decision package

**Status:** Decision complete (docs only)  
**Generated:** 2026-06-29  
**Baseline SHA:** `1238549` + S eval  
**Image:** `python-ai-service:t20-p215f`  
**Parent:** T20.15S — 25% eval PASS, percent restored to 0
...
```text
T20.15S 25% hybrid canary eval: PASS
Hybrid allowlist canary: KEEP
AI_RAG_HYBRID_CANARY_PERCENT=0 restored
Production default: keyword
Vector production default: NOT APPROVED
T20.15U 50% design: RECOMMENDED (owner approval required)
```

---

## 2. Evidence summary (D-S through S)
...
| T20.15J/K | 5% verify + eval PASS; percent restored |
| T20.15N/O | 10% verify + eval PASS; percent restored |
| T20.15R | 25% verification-only; percent=25 bucket math |
| T20.15S | 25% PASS; buckets 0–24 + bucket25 control; percent restored |

---

## 3. T20.15S gate verdict table

| Gate | Result |
|------|--------|
| HTTP 200 (27 allowlist transcript) | **PASS** |
| Cohort API (20 prompts) | **PASS** |
...
**Not selected.** All gates passed across 1%/5%/10%/25% evals; rollback paths proven; no leakage or canary errors.
...
### C. KEEP allowlist + recommend 50% design only ✅ RECOMMENDED NEXT

Percentage ladder proven through 25% with controlled cohort samples. Further design (T20.15U) before any 50% implementation. **No env change.**

### D. KEEP PERCENT=25 active
...
**Next (if owner approves):** Option **C** — T20.15U 50% design only.

Do **not** implement 50% (T20.15V) without separate approval after U.
...
| Ticket | Scope | Status |
|--------|-------|--------|
| T20.15U | 50% design only | **RECOMMENDED** (owner approval) |
| T20.15V | 50% implementation percent-zero | **NOT APPROVED** |
| T20.15W | 50% eval window | **NOT APPROVED** |
...
```text
Approved: start T20.15U 50 percent hybrid canary design only
```
