# T20.31D — Sustained opt-in hybrid preview rollback drill
...
Participant A (cohort0): UI enroll → `preview_opt_in` → UI revoke → `keyword_default` — **PASS** (API poll after revoke; `e2e/ai-rag-opt-in-hybrid-preview-ui.spec.ts`).
