# OCH → RP rewrite scan: `record-platform-makefile-golden-snapshot-kafka-chaos-20260410`

**Staging (read-only scan):** `/Users/tom/bundle-staging/record-platform-makefile-golden-snapshot-kafka-chaos-20260410`
...
**Hits:** 56 (capped per file in scanner)

- `record-platform-makefile-golden-chaos-kafka-20260410/Makefile`
  - L2: `# Off-Campus-Housing-Tracker — Unified Orchestration Makefile`
  - L138: `@echo "Off-Campus-Housing-Tracker — common make targets"`
  - L191: `@echo " Off-Campus-Housing-Tracker Make Menu"`
  - L1041: `bash $(SCRIPTS)/setup-full-off-campus-housing-stack.sh`
  - L1046: `bash $(SCRIPTS)/setup-full-off-campus-housing-stack.sh`
  - L1051: `bash $(SCRIPTS)/setup-full-off-campus-housing-stack.sh`
  - L1058: `PREFLIGHT_PHASE_D_TAIL_LAB=0 SKIP_COLIMA=1 bash $(SCRIPTS)/setup-full-off-campus-housing-stack.sh`
- `record-platform-makefile-golden-chaos-kafka-20260410/README-BUNDLE.md`
  - L7: `- **Namespace:** `off-campus-housing-tracker` → **`record-platform`** (Makefile default **`HOUSING_NS ?= record-platform`**, scripts, YAML).`
  - L8: `- **Edge / SNI hostname:** `off-campus-housing.test` → **`record.local`** where it appeared in bundled text.`
  - L69: `Follow **Off-Campus-Housing-Tracker** upstream license.`
- `record-platform-makefile-golden-chaos-kafka-20260410/package.json`
  - L52: `"setup:full-stack": "bash scripts/setup-full-off-campus-housing-stack.sh",`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/build-and-load-auth-service.sh`
  - L44: `say "Done. Deploy with: kubectl apply -k infra/k8s/base/auth-service (ensure namespace off-campus-housing exists)."`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/check-all-pods-and-tls.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/generate-kafka-ci-tls.sh`
  - L28: `-subj "/CN=och-kafka-ci-ca/O=off-campus-housing-ci" \`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/deploy-dev.sh`
  - L40: `LEAF_TLS_SECRET="${LEAF_TLS_SECRET:-off-campus-housing-local-tls}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/dev-generate-certs.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ensure-caddy-envoy-strict-tls.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/load-caddy-image-colima.sh`
  - L3: `# Strict TLS/mTLS: Caddy uses off-campus-housing-local-tls + dev-root-ca in ingress-nginx (from strict-tls-bootstrap).`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/reissue-ca-and-leaf-load-all-services.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rollout-caddy.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rotation-suite.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-preflight-scale-and-all-suites.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run_auth_pgbench_sweep.sh`
  - L21: `-n, --namespace NS       Kubernetes namespace (default: off-campus-housing)`
  - L57: `NS="off-campus-housing"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run_pgbench_sweep.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/setup-full-off-campus-housing-stack.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/setup-new-colima-cluster.sh`
  - L138: `say "One-shot (idiot-proof): ./scripts/setup-full-off-campus-housing-stack.sh — see docs/RUN_PIPELINE_ORDER.md"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/strict-tls-bootstrap.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/test-full-chain-with-rotation.sh`
  - L45: `LEAF_TLS_SECRET="${LEAF_TLS_SECRET:-off-campus-housing-local-tls}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/test-tls-mtls-comprehensive.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-http3-edge.sh`
...
**Hits:** 469 (capped per file in scanner)

- `record-platform-makefile-golden-chaos-kafka-20260410/Makefile`
  - L42: `metallb-fix hosts-sanity ensure-edge-hosts wait-for-caddy-ip preflight-gate sslkeylog-seed ollama-note ollama-env verify-network-coherence verify-kafka-dns kafka-diagnose-broker-dns verify-kafka-bo…`
  ...
  - L516: `kafka-tls-guard: ## Mounted CA + JKS uniformity across brokers, och-kafka CA, logs, verify-kafka-cluster (fail-fast)`
  - L531: `service-tls-alias-guard: ## Fail if service-tls vs och-service-tls ca.crt fingerprints differ`
  - L540: `rollout-och-full: ## ensure-housing-cluster-secrets then rollout-deferred-after-kafka-tls; skip secrets: SKIP_ENSURE_CLUSTER_SECRETS=1`
  - L541: `chmod +x $(SCRIPTS)/ensure-housing-cluster-secrets.sh $(SCRIPTS)/rollout-deferred-after-kafka-tls.sh $(SCRIPTS)/rollout-restart-och-full-stack.sh`
  - L542: `NS_ING=$(NS_ING) HOUSING_NS=$(HOUSING_NS) OCH_ROLLOUT_STATUS_TIMEOUT=$(OCH_ROLLOUT_STATUS_TIMEOUT) SKIP_ENSURE_CLUSTER_SECRETS=$(SKIP_ENSURE_CLUSTER_SECRETS) bash $(SCRIPTS)/rollout-restart-och-ful…`
  - L582: `# Local path: Phase 0.25 deps + 0.5 dev-root CA → up-fast → Kafka apply → och-kafka-ssl-secret sync+verify → … (see script header).`
  - L858: `bash $(SCRIPTS)/recycle-och-postgres-compose.sh`
- `record-platform-makefile-golden-chaos-kafka-20260410/README-BUNDLE.md`
  - L38: `Defaults assume OCH-style **in-cluster** ports; change in your **ConfigMaps / Deployments / scripts env** if you remap:`
- `record-platform-makefile-golden-chaos-kafka-20260410/infra/slo/error-budget-policy.md`
  - L12: `## Burn detection (aligned with `infra/k8s/base/observability/prometheus-rules-och-slo.yaml`)`
- `record-platform-makefile-golden-chaos-kafka-20260410/package.json`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/aggressive-cleanup-replicasets.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/apply-och-messaging-and-restart.sh`
  - L3: `# Prereq: OCH secrets exist (och-service-tls, och-kafka-ssl-secret).`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/assert-kafka-integration-cluster.mjs`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/bring-up-external-infra.sh`
  - L10: `#   ./scripts/recycle-och-postgres-compose.sh`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/build-housing-images-k3s.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/delete-kafka-event-topics.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/start-kafka-tls-ci.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/verify-housing-grpc-matrix.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/colima-edge-sysctl-tuning.sh`
  - L6: `# Persist: create /etc/sysctl.d/99-och-edge.conf with the same keys (distribution-specific).`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/colima-patch-app-config-db-host-to-gateway.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/create-kafka-event-topics-k8s.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/create-kafka-event-topics.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/dev-onboard-local.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/dev-onboard-zero-trust-preflight.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/diagnose-k6-edge-connectivity.sh`
  - L81: `echo "  # If DNS missing: add \"$LB_IP $HOST\" to /etc/hosts or: OCH_AUTO_EDGE_HOSTS=1 with sudo"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/diagnose-och-deployment.sh`
  - L3: `# Usage: ./scripts/diagnose-och-deployment.sh media-service`
  - L4: `#        ./scripts/diagnose-och-deployment.sh auth-service`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ensure-all-services-tls.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ensure-edge-hosts.sh`
  - L6: `#   OCH_EDGE_HOSTNAME   — hostname (default record.local)`
  - L19: `HOST="${OCH_EDGE_HOSTNAME:-record.local}"`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ensure-housing-cluster-secrets.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ensure-strict-tls-mtls-preflight.sh`
  ...
  - … *2 more in this file*
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/install-metallb-colima.sh`
  - L124: `if ! och_assert_metallb_pool_coherent "$METALLB_POOL"; then`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/k8s-rollout-och-ordered.sh`
  - L3: `# Usage: ./scripts/k8s-rollout-och-ordered.sh`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-refresh-tls-from-lb.sh`
  - L2: `# Regenerate kafka-ssl-secret (and och-kafka-ssl-secret) with SANs that include current`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-rolling-restart.sh`
  - L36: `if ! och_kafka_kraft_quorum_ok "$NS"; then`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-ssl-from-dev-root.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-tls-guard.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-tls-rotate-atomic.sh`
  - L30: `echo "▶ Regenerate kafka-ssl-secret + och-kafka-ssl-secret (full JKS + PEM)"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/edge-test-url.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/ensure-colima-docker-context.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/ensure-och-grpc-certs.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/grpc-http3-health.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/grpc-utils.sh`
  - L7: `och_housing_ns() {`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/kafka-broker-sans.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/kafka-kraft-quorum-ok.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/metallb-subnet-guard.sh`
  ...
  - … *1 more in this file*
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/och-housing-docker-services-default.sh`
  - L2: `# transport-watchdog: image only; rollouts map to deploy/api-gateway (see rebuild-och-images-and-rollout.sh).`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/och-kafka-event-topics-from-proto.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/och-sequential-rollout.sh`
  - L8: `#     och_kubectl() { kctl "$@"; }`
  ...
  - … *1 more in this file*
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/patch-kafka-external-host.sh`
  - L44: `RAW=$(kubectl run och-resolve-kafka-host --rm -i --restart=Never --image=busybox:1.36 -n record-platform -- \`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/perf/build-canonical-bundle.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/preflight-kafka-k8s-rollout.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/protocol/test-service-protocol.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rebuild-all-housing-images-k3s.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rebuild-och-images-and-rollout.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rebuild-traffic-control-stack.sh`
  - L11: `exec "$ROOT/scripts/rebuild-och-images-and-rollout.sh"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/recycle-och-postgres-compose.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/restore-external-postgres-from-backup.sh`
  - L44: `och_terminate_db_sessions() {`
  - L58: `och_terminate_db_sessions "$port" "$db"`
  - L66: `och_terminate_db_sessions "$port" "$db"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rollout-deferred-after-kafka-tls.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rollout-restart-och-after-pool-tuning.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rollout-restart-och-full-stack.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-all-test-suites.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-housing-k6-edge-smoke.sh`
  - L9: `#   - Hostname must resolve (e.g. /etc/hosts: <MetalLB IP> record.local) — see scripts/lib/edge-test-url.sh, OCH_AUTO_EDGE_HOSTS=1`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-preflight-scale-and-all-suites.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/service-tls-alias-guard.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/slo/check-error-budget.js`
  - L11: `*   OCH_SLO_TARGET      — decimal availability target (default: 0.995)`
  - L21: `const SLO_TARGET = Number(process.env.OCH_SLO_TARGET || "0.995");`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/test-booking-http2-http3.sh`
  - L24: `for _sec in och-service-tls service-tls; do`
  - L115: `for _sec in och-service-tls service-tls; do`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/test-microservices-http2-http3-housing.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/test-packet-capture-standalone.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/tls-k8s-secrets-expiry.sh`
  - L17: `echo "och_tls_cert_days_remaining{namespace=\"$ns\",secret=\"$sec\"} $days"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/transport/http3-concurrent-curl-stress.sh`
  - L5: `#   OCH_H3_URL=https://record.local/api/healthz \`
  - L6: `#   OCH_H3_CA=certs/dev-root.pem \`
  - L14: `URL="${OCH_H3_URL:-https://record.local/api/healthz}"`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/validate-kafka-stack-contract.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-caddy-grpc-routing.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-deletion-flow.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-kafka-cluster.sh`
  ...
  - … *1 more in this file*
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-kafka-event-topic-partitions.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-kafka-kraft-e2e.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-kafka-tls-sans.sh`
  - L104: `done < <(och_kafka_emit_san_verify_dns_specs "$NS" "$REPLICAS")`
  - L120: `done < <(och_kafka_metallb_external_lb_ips_lines "$NS" "$REPLICAS")`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-network-coherence.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-preflight-edge-routing.sh`
  - L8: `#   HOUSING_NS, OCH_EDGE_HOSTNAME (default record.local)`
  - L13: `#   OCH_EDGE_IP — if set to an IPv4, use for 6b2 LB alignment instead of resolving HOST (optional override)`
  - L26: `HOST="${2:-${OCH_EDGE_HOSTNAME:-record.local}}"`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-proto-events-topics.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-proto-topic-alignment.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/wait-for-all-services-ready.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/wait-for-docker-compose-kafka-strict-ready.sh`
  - L6: `#   REPO_ROOT, ENV_PREFIX (default dev), OCH_KAFKA_TOPIC_SUFFIX (optional)`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/README-BUNDLE.md`
  - L42: `| **api-gateway** | **4020** | HTTP edge backend; k6/Playwright often hit **443** on Caddy → gateway. |`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/smoke-api-gateway.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/edge-readiness-gate.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/k6/k6-smoke-gateway.js`
  - L15: `"http://api-gateway.record-platform.svc.cluster.local:4020";`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/lib/edge-test-url.sh`
  - L2: `# Shared helpers: edge-only E2E / k6 (https://record.local), no port-forward / :4020.`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-playwright-e2e-preflight.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/test-lb-coordinated.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/webapp-playwright-strict-edge.sh`
  - L7: `#   E2E_API_BASE   — https only (default https://record.local); :4020 / http localhost rejected`

## HOUSING / legacy env

*Environment variables and assignments*

**Hits:** 180 (capped per file in scanner)

- `record-platform-makefile-golden-chaos-kafka-20260410/Makefile`
  - L347: `HOUSING_NS=$(HOUSING_NS) bash $(REPO_ROOT)/scripts/diagnose-kafka-broker-dns.sh`
  - L370: `HOUSING_NS=$(HOUSING_NS) KAFKA_BROKER_REPLICAS=$(KAFKA_BROKER_REPLICAS) bash $(REPO_ROOT)/scripts/kafka-auto-heal-inter-broker-tls.sh`
  - L380: `HOUSING_NS=$(HOUSING_NS) KAFKA_BROKER_REPLICAS=$(KAFKA_BROKER_REPLICAS) bash $(REPO_ROOT)/scripts/kafka-auto-heal-inter-broker-tls.sh`
  - L398: `HOUSING_NS=$(HOUSING_NS) KAFKA_BROKER_REPLICAS=$(KAFKA_BROKER_REPLICAS) bash $(REPO_ROOT)/scripts/kafka-auto-heal-inter-broker-tls.sh`
  ...
  - L432: `HOUSING_NS=$(HOUSING_NS) KAFKA_BROKER_REPLICAS=$(KAFKA_BROKER_REPLICAS) bash $(REPO_ROOT)/scripts/kafka-auto-heal-inter-broker-tls.sh`
  - L453: `k8s-diagnose-restarts: ## Pods with restarts: namespace events, per-container describe + logs (HOUSING_NS=…)`
  - L505: `HOUSING_NS=$(HOUSING_NS) KAFKA_TLS_ATOMIC_BEFORE_REFRESH=$(KAFKA_TLS_ATOMIC_BEFORE_REFRESH) KAFKA_BROKER_REPLICAS=$(KAFKA_BROKER_REPLICAS) bash $(SCRIPTS)/apply-kafka-kraft-staged.sh`
  - L518: `KAFKA_BROKER_REPLICAS=$(KAFKA_BROKER_REPLICAS) HOUSING_NS=$(HOUSING_NS) bash $(SCRIPTS)/kafka-tls-guard.sh`
  - L522: `KAFKA_BROKER_REPLICAS=$(KAFKA_BROKER_REPLICAS) HOUSING_NS=$(HOUSING_NS) bash $(SCRIPTS)/kafka-tls-rotate-atomic.sh`
  - L529: `HOUSING_NS=$(HOUSING_NS) bash $(SCRIPTS)/kafka-quorum-stable.sh`
  - L533: `HOUSING_NS=$(HOUSING_NS) bash $(SCRIPTS)/service-tls-alias-guard.sh`
  - L537: `NS_ING=$(NS_ING) HOUSING_NS=$(HOUSING_NS) bash $(SCRIPTS)/edge-readiness-gate.sh`
  - L542: `NS_ING=$(NS_ING) HOUSING_NS=$(HOUSING_NS) OCH_ROLLOUT_STATUS_TIMEOUT=$(OCH_ROLLOUT_STATUS_TIMEOUT) SKIP_ENSURE_CLUSTER_SECRETS=$(SKIP_ENSURE_CLUSTER_SECRETS) bash $(SCRIPTS)/rollout-restart-och-ful…`
  - L550: `HOUSING_NS=$(HOUSING_NS) bash $(SCRIPTS)/ensure-housing-cluster-secrets.sh`
  - … *4 more in this file*
- `record-platform-makefile-golden-chaos-kafka-20260410/README-BUNDLE.md`
  - L10: `Override anytime: `make HOUSING_NS=my-ns golden-snapshot`.`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/apply-kafka-kraft-staged.sh`
  - L11: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/auth-outbox-replay.sh`
  - L11: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/bring-up-external-infra.sh`
  - L38: `HOUSING_NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/chaos-kafka-alignment-stochastic.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/chaos-kafka-broker.sh`
  - L16: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/chaos-metallb-kafka-lb.sh`
  - L13: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/check-all-pods-and-tls.sh`
  - L81: `_KNS="${HOUSING_NS:-record-platform}"`
  - L125: `HOUSING_CHECK_NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/check-kafka-config-drift.sh`
  - L12: `NS="${1:-${HOUSING_NS:-record-platform}}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/canary-pod-stability.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/hydrate-certs-for-ci.sh`
  - L14: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/k6-smoke-incluster.sh`
  - L6: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/post-deploy-verify.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/smoke-api-gateway.sh`
  ...
  - L6: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ci/verify-k8s-images.sh`
  - L18: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/cleanup-kafka-ops-cronjob-pods.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/diagnose-kafka-broker-dns.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ensure-ready-for-preflight.sh`
  - L131: `_KNS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/ensure-strict-tls-mtls-preflight.sh`
  - L14: `HOUSING_NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/export-kafka-ca-metric.sh`
  - L9: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/golden-snapshot-verify.sh`
  - L19: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/k8s-diagnose-restarts.sh`
  - L10: `NS="${1:-${HOUSING_NS:-record-platform}}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/k8s-rollout-och-ordered.sh`
  - L4: `#   HOUSING_NS=record-platform  (default)`
  - L12: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-after-rollout-verify-brokers.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-auto-heal-inter-broker-tls.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-clean-slate.sh`
  - L9: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-onboarding-reset.sh`
  - L8: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-quorum-stable.sh`
  - L13: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-refresh-tls-from-lb.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-runtime-sync.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/kafka-sync-metallb.sh`
  - L12: `NS="${HOUSING_NS:-record-platform}"`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/load/k6-edge-load-diagnostics.sh`
  - L16: `HOUSING_NS="${HOUSING_NS:-record-platform}"`
  - L45: `echo "=== $tag @ $(date -u +%Y-%m-%dT%H:%M:%SZ) ns=$HOUSING_NS ==="`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/load/run-k6-listings-concurrency-instrumented.sh`
  - L15: `export HOUSING_NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/probe-kafka-kraft-listeners.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/rebuild-housing-colima.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-all-test-suites.sh`
  - L192: `och_sync_grpc_certs_to_dir "${GRPC_CERTS_DIR:-/tmp/grpc-certs}" "${HOUSING_NS:-record-platform}" || true`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-demo-network-preflight.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-preflight-scale-and-all-suites.sh`
  ...
  - L2806: `HOUSING_NS="$_sk_ns" KAFKA_BROKER_REPLICAS="$_sk_rep" bash "$SCRIPT_DIR/kafka-runtime-sync.sh" --check-only "$_sk_ns" "$_sk_rep" \`
  - L2830: `HOUSING_NS="$_sk_ns" KAFKA_BROKER_REPLICAS="$_sk_rep" \`
  - … *5 more in this file*
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/run-suite-with-packet-capture.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/test-messaging-service-comprehensive.sh`
  - L18: `HOUSING_NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/test-microservices-http2-http3-enhanced.sh`
  - L19: `HOUSING_NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/tests/kafka-alignment-suite.sh`
  ...
  - … *4 more in this file*
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-cluster-kafka-three-brokers.sh`
  - L4: `#   HOUSING_NS=record-platform (default)`
  - L7: `HOUSING_NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-housing-kafka-bootstrap.sh`
  - L7: `NS="${HOUSING_NS:-record-platform}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-kafka-cluster.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-kafka-kraft-advertised-listeners.sh`
  - L15: `NS="${1:-${HOUSING_NS:-record-platform}}"`
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-kafka-no-static-advertised-env.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/verify-required-housing-pods.sh`
...
- `record-platform-makefile-golden-chaos-kafka-20260410/scripts/wait-for-kafka-external-lb-ips.sh`
  - L6: `NS="${HOUSING_NS:-record-platform}"`
