...
Preflight assumes a **healthy cluster**, **TLS material**, **Kafka KRaft (3 brokers)**, **MetalLB** (when external listeners matter), and **edge** reachable at **`https://record.local`** in this bundle.
...
Brokers still expect **JKS** in **`och-kafka-ssl-secret`** today — use init containers or keep **`kafka-ssl-from-dev-root.sh`** until PEM/JKS wiring per pod is implemented (**`infra/k8s/kafka-certs/README.md`**).
