...
1. **T20.12** — Tranche 6 actual completed (`t20-tranche-6`, +500 → 7,065 embedded; T20.12J-S adjusted caps; OBO pool exhausted). Tranche 7 dry-run planned — see `docs/ai-platform/T20-12N-tranche7-dry-run-plan.md`.
2. **T20.12O** — next tranche actual write only with explicit approval (fresh backup, new tranche id, no FORCE).
3. Post-tranche evidence: `scripts/rp-ai-live-inference-transcript.sh` + T20.12M eval doc.
4. **No T20.14/T20.15** rollout work until **all** gates pass and explicit approval is granted.

Do not start T20.12O actual write or T20.14/T20.15 rollout without explicit approval.
