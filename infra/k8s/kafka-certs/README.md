...
# or
HOUSING_NS=record-platform bash scripts/verify-kafka-tls-sans.sh
```
...
After Kafka and `kafka-ssl-secret` exist:

```bash
kubectl apply -f infra/k8s/kafka-certs/kafka-tls-preflight-job.yaml
kubectl -n record-platform wait --for=condition=complete job/kafka-tls-preflight --timeout=120s
kubectl -n record-platform logs job/kafka-tls-preflight
```

Delete the job before re-running: `kubectl -n record-platform delete job kafka-tls-preflight`.
