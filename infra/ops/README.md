...
```bash
kubectl delete job -n record-platform --field-selector status.successful=1
```
...
kubectl create job --from=cronjob/kafka-quorum-check kafka-quorum-check-manual -n record-platform
kubectl logs job/kafka-quorum-check-manual -n record-platform
```
...
2. Rebuild and load, then restart:
   ...
   ```bash
   SERVICES="auth-service trust-service" ./scripts/rebuild-rp-images-and-rollout.sh
   ```

3. If replicas were scaled to zero, scale back: `kubectl scale deploy/auth-service deploy/trust-service --replicas=1 -n record-platform`
