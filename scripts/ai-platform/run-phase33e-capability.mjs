#!/usr/bin/env node
import { runCapability, validateCapabilityResultShape } from '../lib/phase33e-intelligence.mjs';
...
function main() {
  return readStdin().then((raw) => {
    ...
    try {
      ...
      process.stdout.write(JSON.stringify({ status: 'FAIL', error: String(err?.message || err), capability }));
      process.exit(1);
    }
  });
}
