...
#
# Usage: HOUSING_NS=record-platform ./scripts/apply-ollama-gateway-stack.sh
...
#   app-config REDIS_URL (e.g. redis://host.docker.internal:6379/0 for Docker Compose redis). Set to 0 to deploy Redis Stack in-cluster.
set -euo pipefail
...
if [[ "${RP_ENABLE_OLLAMA_EFFECTIVE}" != "1" ]]; then
  echo "apply-ollama-gateway-stack: skipped (core-only — RP_ENABLE_OLLAMA_EFFECTIVE=0)"
  exit 0
fi
...
if [[ "$USE_EXT" != "1" ]]; then
  ...
  echo "▶ skip k8s/redis.yaml (OLLAMA_GATEWAY_USE_EXTERNAL_REDIS=1 — use app-config REDIS_URL, e.g. host :6379)"
fi
