...
#   CLUSTER_OLLAMA_URL default http://ollama.record-platform.svc.cluster.local:11434
#   DEPLOYED_FEEL_URL  default https://record-platform.test/api/analytics/insights/listing-feel
#   LISTING_JSON       POST body (required)
set -euo pipefail

LOCAL_OLLAMA_URL="${LOCAL_OLLAMA_URL:-http://127.0.0.1:11434}"
CLUSTER_OLLAMA_URL="${CLUSTER_OLLAMA_URL:-http://ollama.record-platform.svc.cluster.local:11434}"
DEPLOYED_FEEL_URL="${DEPLOYED_FEEL_URL:-https://record-platform.test/api/analytics/insights/listing-feel}"
