...
#   P1 HOST (node 22, docker, curl HTTP3, openssl, kubectl client — no workload API assumptions)
...
#   BOOTSTRAP_SKIP_RESET=1      — skip P0 hard reset + Colima cycle (cold-bootstrap owns P0.hard_reset + Z.colima_clean).
#   BOOTSTRAP_SKIP_COLIMA=1      — alias: never stop/delete/start Colima in nested bootstrap (cold-bootstrap owns reset).
#   BOOTSTRAP_SKIP_P0=1         — alias for BOOTSTRAP_SKIP_RESET (no [P0] HARD RESET / colima delete).
#   BOOTSTRAP_SKIP_P0B=1        — skip Colima VM tcpdump/tshark install (cold-bootstrap Z.colima_clean did it).
#   BOOTSTRAP_SKIP_P1=1         — skip [P1] host dependency checks (cold-bootstrap P1.host_deps did it).
#   BOOTSTRAP_SKIP_P1B=1        — skip kafka-alignment-report venv (cold-bootstrap A.workspace did it).
#   BOOTSTRAP_SKIP_P1C=1        — skip DEV_CERTS_ENSURE_ONLY dev-generate-certs (cold-bootstrap B.crypto did it).
#   RP_COLD_BOOTSTRAP_RESET_DONE=1 — set by cold-bootstrap after Z.colima_clean; nested bootstrap must not re-reset.
...
#   BOOTSTRAP_GRAPH_COMPLETE_F_WITHOUT_P5C=1 — with skip flags above, mark DAG node F.cluster_deploy complete after G (you assert alignment out-of-band).
...
#   BOOTSTRAP_IMAGE_SERVICES    — override space-separated list (default: HOUSING_DOCKER_SERVICES_DEFAULT + webapp once).
...
#   BOOTSTRAP_SKIP_P6_RUNTIME_IMAGES=1 — skip P6 runtime :dev docker builds (cold-bootstrap E.build_images already ran).
#   RP_COLD_BOOTSTRAP_IMAGES_BUILT=1 — alias for BOOTSTRAP_SKIP_P6_RUNTIME_IMAGES (set by cold-bootstrap F.cluster_deploy).
...
#   OLLAMA_GATEWAY_USE_EXTERNAL_REDIS=1 (default) — P7a skips k8s/redis.yaml; gateway/worker REDIS_URL from app-config (e.g. host :6379). Set 0 for in-cluster Redis Stack.
...
NS="${NAMESPACE:-${HOUSING_NS:-record-platform}}"
...
export RP_ENABLE_OLLAMA OLLAMA_REQUIRED RP_OLLAMA_REQUIRE_MODEL \
  RP_ENABLE_OLLAMA_EFFECTIVE OLLAMA_REQUIRED_EFFECTIVE RP_CORE_ONLY_BOOTSTRAP_EFFECTIVE
if [[ "${RP_ENABLE_OLLAMA_EFFECTIVE}" != "1" ]]; then
  say "ℹ️  core-only cluster bootstrap — ML/Ollama gates disabled (RP_ENABLE_OLLAMA_EFFECTIVE=0)"
  export BOOTSTRAP_SKIP_OLLAMA_GATEWAY_STACK="${BOOTSTRAP_SKIP_OLLAMA_GATEWAY_STACK:-1}"
  export BOOTSTRAP_SKIP_OLLAMA_VERIFY="${BOOTSTRAP_SKIP_OLLAMA_VERIFY:-1}"
else
  export BOOTSTRAP_SKIP_OLLAMA_GATEWAY_STACK="${BOOTSTRAP_SKIP_OLLAMA_GATEWAY_STACK:-0}"
  export BOOTSTRAP_SKIP_OLLAMA_VERIFY="${BOOTSTRAP_SKIP_OLLAMA_VERIFY:-0}"
fi
...
say() {
  [[ "${BOOTSTRAP_QUIET:-0}" == "1" || "${COLD_BOOTSTRAP_QUIET:-0}" == "1" ]] && return 0
  printf '\n\033[1m%s\033[0m\n' "$*"
}
ok() { echo "✅ $*"; }
bad() { echo "❌ $*" >&2; }
...
# shellcheck source=scripts/lib/rp-ensure-node-toolchain.sh
...
if _och_bootstrap_skip_host_path; then
  ...
  say "⏭️  [P0–P1c+] skipped — cold-bootstrap owns destructive reset, Colima start, host deps, workspace, crypto"
  command -v colima >/dev/null 2>&1 || { bad "colima not on PATH"; exit 1; }
  rp_ensure_node_toolchain_require "$REPO_ROOT" || exit 1
  command -v docker >/dev/null 2>&1 || { bad "docker not on PATH"; exit 1; }
  docker info >/dev/null 2>&1 || { bad "docker info failed (point DOCKER_HOST at Colima)"; exit 1; }
  command -v kubectl >/dev/null 2>&1 || { bad "kubectl not on PATH"; exit 1; }
  # shellcheck source=scripts/lib/ensure-colima-docker-context.sh
  source "$SCRIPT_DIR/lib/ensure-colima-docker-context.sh"
  export OCH_KUBE_CONTEXT="${OCH_KUBE_CONTEXT:-$(kubectl config current-context 2>/dev/null || echo colima)}"
...
say "[P0] HARD RESET"
if [[ "${RP_COLD_BOOTSTRAP_RESET_DONE:-0}" == "1" ]]; then
  bad "nested bootstrap attempted [P0] HARD RESET after cold-bootstrap P0.hard_reset/Z.colima_clean — use BOOTSTRAP_SKIP_RESET=1 BOOTSTRAP_SKIP_P0=1 BOOTSTRAP_SKIP_COLIMA=1"
  exit 1
fi
...
_colima_args=()
rp_colima_build_start_argv _colima_args "$CPU" "$MEMORY" "$DISK" "$COLIMA_K3S_VERSION"
rp_colima_print_start_argv _colima_args
if ! colima "${_colima_args[@]}"; then
  RP_COLIMA_USE_VZ=0
  _colima_args=()
  rp_colima_build_start_argv _colima_args "$CPU" "$MEMORY" "$DISK" "$COLIMA_K3S_VERSION"
  rp_colima_print_start_argv _colima_args
...
ok "Colima + k3s up (k3s servicelb disabled — MetalLB-only edge)"
if ! rp_colima_verify_no_servicelb kube-system; then
  bad "k3s ServiceLB components present after colima start — edge is not MetalLB-only"
  exit 1
fi
ok "no k3s ServiceLB / svclb / klipper in kube-system"
...
if [[ "${BOOTSTRAP_SKIP_COLIMA_VM_CAPTURE_TOOLS:-0}" == "1" ]] || [[ "${BOOTSTRAP_SKIP_P0B:-0}" == "1" ]]; then
  echo "  ℹ️  skipping VM capture tooling (BOOTSTRAP_SKIP_COLIMA_VM_CAPTURE_TOOLS / BOOTSTRAP_SKIP_P0B)"
else
...
# --- P1 HOST ---
if [[ "${BOOTSTRAP_SKIP_P1:-0}" == "1" ]]; then
  say "⏭️  [P1] HOST dependencies skipped (BOOTSTRAP_SKIP_P1=1)"
else
...
if [[ "${BOOTSTRAP_SKIP_P1B:-0}" == "1" ]] || [[ "${BOOTSTRAP_SKIP_KAFKA_ALIGNMENT_REPORT_VENV:-0}" == "1" ]]; then
...
if [[ "${BOOTSTRAP_COMPOSE_DOWN:-0}" == "1" ]] && [[ -f "$REPO_ROOT/docker-compose.yml" ]]; then
  ...
  if [[ "$_bk" == "1" ]]; then
    ...
    chmod +x "$SCRIPT_DIR/backup-rp-postgres-dbs.sh" 2>/dev/null || true
    bash "$SCRIPT_DIR/backup-rp-postgres-dbs.sh" || echo "⚠️  backup-rp-postgres-dbs.sh failed — continuing with compose down"
  fi
...
if [[ "$_bootstrap_force_ns_delete" == "1" ]]; then
  say "⚠️  destructive Record Platform namespace delete (BOOTSTRAP_FORCE_NS_DELETE=1)"
  kubectl delete namespace "$NS" --ignore-not-found --wait=true --timeout="${BOOTSTRAP_NS_DELETE_TIMEOUT:-60s}" || true
elif [[ "${BOOTSTRAP_SKIP_NS_DELETE:-0}" == "0" ]]; then
  echo "ℹ️  namespace delete skipped by policy — P0 hard reset already produced a clean cluster (use BOOTSTRAP_FORCE_NS_DELETE=1 for recovery)" >&2
fi
...
ok "Record Platform ns + app-config"
...
HOUSING_NS="$NS" bash "$SCRIPT_DIR/ensure-rp-cluster-secrets.sh"
ok "strict-tls + app-secrets + cluster secret aliases"
...
if [[ "${BOOTSTRAP_SKIP_KAFKA_APPLY:-0}" == "1" ]]; then
  ...
  export RP_KAFKA_STAGED_APPLY_GUARD="${RP_KAFKA_STAGED_APPLY_GUARD:-${REPO_ROOT}/bench_logs/.rp-kafka-staged-apply-guard}"
  ...
  if [[ -x "$SCRIPT_DIR/rp-ensure-kafka-ssl-clients.sh" ]]; then
    HOUSING_NS="$NS" RP_KAFKA_SSL_RESTART_APPS=0 bash "$SCRIPT_DIR/rp-ensure-kafka-ssl-clients.sh" || {
      bad "kafka-ssl-secret missing client mTLS after P5b — run: bash scripts/kafka-ssl-from-dev-root.sh && HOUSING_NS=$NS bash scripts/apply-rp-kafka-ssl-secret.sh"
      exit 1
    }
    ok "kafka-ssl-secret client mTLS verified (P5b post-apply)"
  fi
  if [[ -x "$SCRIPT_DIR/validate-kafka-dns.sh" ]]; then
    KAFKA_NAMESPACE="$NS" bash "$SCRIPT_DIR/validate-kafka-dns.sh" || {
      bad "validate-kafka-dns failed — fix headless/external slices before deploy-dev"
      exit 1
    }
    ok "Kafka DNS (headless + kafka-N-external) validated"
  fi
fi
...
if [[ "${BOOTSTRAP_SKIP_KAFKA_APPLY:-0}" == "1" ]]; then
  ...
  if [[ "${BOOTSTRAP_SKIP_KAFKA_ALIGNMENT_SUITE:-0}" != "1" ]]; then
    ...
    KAFKA_ALIGNMENT_BASELINE_REMEDIATE_RETRY=0 KAFKA_ALIGNMENT_TEST_MODE=1 make kafka-alignment-suite
...
if { [[ "${BOOTSTRAP_SKIP_KAFKA_APPLY:-0}" == "1" ]] || [[ "${BOOTSTRAP_SKIP_KAFKA_TOPIC_PROVISION:-0}" == "1" ]]; } \
  ...
  echo "  ℹ️  BOOTSTRAP_GRAPH_COMPLETE_F_WITHOUT_P5C=1 — F.cluster_deploy will be marked after G.app_runtime (post P8 verify-app-runtime)."
fi
...
if [[ "${BOOTSTRAP_SKIP_P6_RUNTIME_IMAGES:-0}" == "1" ]] || [[ "${RP_COLD_BOOTSTRAP_IMAGES_BUILT:-0}" == "1" ]]; then
  _p6_skip_runtime_build=1
fi

if [[ "$_p6_skip_runtime_build" == "1" ]]; then
  say "[P6] IMAGE lifecycle — skipped (E.build_images already built runtime :dev images)"
  if [[ "${RP_SKIP_IMAGE_FRESHNESS:-0}" != "1" ]]; then
    chmod +x "$SCRIPT_DIR/audit-rp-image-freshness.sh" 2>/dev/null || true
    bash "$SCRIPT_DIR/audit-rp-image-freshness.sh" || {
      bad "image freshness audit failed before deploy (run make rp-build-missing-images)"
      exit 1
    }
  fi
  ok "runtime images fresh — deploy only (no P6 docker build)"
fi
...
if [[ "$_p6_skip_runtime_build" != "1" ]]; then
  ...
  # Dedupe if caller passed webapp in HOUSING_DOCKER_SERVICES_DEFAULT (legacy).
  read -r -a _img_arr <<< "$IMG_LIST"
  declare -A _img_seen=()
  _img_deduped=()
  for _s in "${_img_arr[@]}"; do
    [[ -n "$_s" ]] || continue
    [[ -n "${_img_seen[$_s]:-}" ]] && continue
    _img_seen[$_s]=1
    _img_deduped+=("$_s")
  done
  IMG_LIST="${_img_deduped[*]}"
  IMG_LIST="${IMG_LIST//,/ }"
  _force_rebuild=0
...
say "[P6b] Sync proto-files ConfigMap (records/social/shopping — required by @common/grpc-clients)"
...
if [[ -x "$SCRIPT_DIR/rp-ensure-kafka-ssl-clients.sh" ]]; then
  HOUSING_NS="$NS" RP_KAFKA_SSL_RESTART_APPS=1 bash "$SCRIPT_DIR/rp-ensure-kafka-ssl-clients.sh" || {
    bad "kafka-ssl-secret missing client mTLS after deploy-dev — run: bash scripts/kafka-ssl-from-dev-root.sh && HOUSING_NS=$NS bash scripts/apply-rp-kafka-ssl-secret.sh"
    exit 1
  }
  ok "kafka-ssl client mTLS verified for app pods"
fi

if [[ "${RP_ENABLE_OLLAMA_EFFECTIVE}" == "1" && "${BOOTSTRAP_SKIP_OLLAMA_GATEWAY_STACK:-0}" != "1" ]]; then
  say "[P7a] Ollama gateway stack (k8s/ — gateway + worker; REDIS_URL from app-config; OLLAMA_GATEWAY_USE_EXTERNAL_REDIS=${OLLAMA_GATEWAY_USE_EXTERNAL_REDIS:-1})"
  ...
  chmod +x "$SCRIPT_DIR/audit-rp-ollama-stack.sh" 2>/dev/null || true
  HOUSING_NS="$NS" bash "$SCRIPT_DIR/audit-rp-ollama-stack.sh"
  ok "P7a ollama-gateway stack + audit"
elif [[ "${RP_ENABLE_OLLAMA_EFFECTIVE}" != "1" ]]; then
  echo "  ℹ️  core-only bootstrap — skipping apply-ollama-gateway-stack.sh (ML/Ollama trust disabled)"
else
  warn "BOOTSTRAP_SKIP_OLLAMA_GATEWAY_STACK=1 — skipping Ollama gateway stack (not full ML trust bootstrap)"
fi

if [[ "${BOOTSTRAP_SKIP_DEPLOYMENT_INTEGRITY:-0}" != "1" ]]; then
  say "[P7b] deployment integrity + Record Platform rollout gates"
...
for ((_rz_i = 1; _rz_i <= _rz_max; _rz_i++)); do
  ...
  if ((_rz_i < _rz_max)); then
    if ((_rz_i % 5 == 1)); then
      _hosts_ip="$(grep -E "[[:space:]]+${EDGE_HOST//./\\.}([[:space:]]|$)" /etc/hosts 2>/dev/null | awk '{print $1}' | head -1 || true)"
      echo "    … HTTP ${_http} (attempt ${_rz_i}/${_rz_max}) caddy_lb=${CADDY_LB_IP:-?} /etc/hosts_ip=${_hosts_ip:-?} curl_err=${_curl_err:-none}"
    else
