...
if [[ "$_resolved" != "1" ]] || [[ -z "${JAEGER_QUERY_BASE:-}" ]]; then
  ...
  echo "  Fix: make align-hosts; export JAEGER_PUBLIC_URL=http://<metallb-jaeger-ip>:16686 OR use edge JAEGER_QUERY_BASE=https://record-platform.test/jaeger"
...
_base_url="${E2E_API_BASE:-https://record-platform.test}"
