...
_KNS="${HOUSING_NS:-record-platform}"
...
_count_app_ready() {
  ...
  for svc in "${SERVICES[@]}"; do
    R=$(_kubectl get deploy -n "${HOUSING_NS:-record-platform}" "$svc" -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
    S=$(_kubectl get deploy -n "${HOUSING_NS:-record-platform}" "$svc" -o jsonpath='{.spec.replicas}' 2>/dev/null || echo "1")
    [[ -z "$R" ]] && R="0"
    [[ -z "$S" ]] && S="1"
    if [[ "$R" == "1" && "$S" == "1" ]]; then
      ((READY++)) || true
...
if [[ "${READY:-0}" -ne "$APP_EXPECTED" ]]; then
  ...
  for svc in "${NOT_READY[@]}"; do
    ...
    POD=$(_kubectl get pods -n "${HOUSING_NS:-record-platform}" -l app="$svc" --sort-by=.metadata.creationTimestamp -o jsonpath='{.items[-1].metadata.name}' 2>/dev/null || echo "")
    ...
    PHASE=$(_kubectl get pod "$POD" -n "${HOUSING_NS:-record-platform}" -o jsonpath='{.status.phase}' 2>/dev/null || echo "Unknown")
    READY_STATUS=$(_kubectl get pod "$POD" -n "${HOUSING_NS:-record-platform}" -o jsonpath='{.status.containerStatuses[0].ready}' 2>/dev/null || echo "false")
    RESTARTS=$(_kubectl get pod "$POD" -n "${HOUSING_NS:-record-platform}" -o jsonpath='{.status.containerStatuses[0].restartCount}' 2>/dev/null || echo "0")
    ...
    _kubectl describe pod "$POD" -n "${HOUSING_NS:-record-platform}" 2>&1 | tee -a "$DIAG_LOG" | grep -A 20 "Events:" | head -25 || true
    
    # Get container logs (last 50 lines)
    echo "" | tee -a "$DIAG_LOG"
    echo "  === kubectl logs $POD (last 50 lines) ===" | tee -a "$DIAG_LOG"
    _kubectl logs "$POD" -n "${HOUSING_NS:-record-platform}" --tail=50 2>&1 | tee -a "$DIAG_LOG" | tail -30 || true
    ...
    if [[ "$PHASE" == "Pending" ]] || [[ "$PHASE" == "ContainerCreating" ]]; then
      AGE=$(_kubectl get pod "$POD" -n "${HOUSING_NS:-record-platform}" -o jsonpath='{.metadata.creationTimestamp}' 2>/dev/null || echo "")
      if [[ -n "$AGE" ]]; then
    ...
    if [[ "$PHASE" == "CrashLoopBackOff" ]] || [[ "$RESTARTS" -gt 5 ]]; then
      ...
      _kubectl rollout restart deployment "$svc" -n "${HOUSING_NS:-record-platform}" 2>&1 | tee -a "$DIAG_LOG" || true
      FIXED_THIS_SERVICE=1
    fi
    
    # If FailedMount, check if secret exists
    MOUNT_ERROR=$(_kubectl describe pod "$POD" -n "${HOUSING_NS:-record-platform}" 2>&1 | grep -i "FailedMount\|MountVolume" | head -1 || echo "")
    if [[ -n "$MOUNT_ERROR" ]]; then
      ...
      if ! _kubectl get secret service-tls -n "${HOUSING_NS:-record-platform}" >/dev/null 2>&1; then
    ...
    if [[ "$svc" == "analytics-service" ]]; then
      KAFKA_ERROR=$(_kubectl logs "$POD" -n "${HOUSING_NS:-record-platform}" --tail=100 2>&1 | grep -i "ECONNREFUSED.*9093\|kafka.*connection\|kafka.*error" | head -1 || echo "")
      if [[ -n "$KAFKA_ERROR" ]]; then
        ...
        KAFKA_ENDPOINT_IP=$(_kubectl get endpoints kafka-external -n "${HOUSING_NS:-record-platform}" -o jsonpath='{.subsets[0].addresses[0].ip}' 2>/dev/null || echo "")
        if [[ -z "$KAFKA_ENDPOINT_IP" ]] || [[ "$KAFKA_ENDPOINT_IP" == "10.43"* ]]; then
          ...
          if [[ -f "$SCRIPT_DIR/patch-kafka-external-host.sh" ]]; then
            ...
            _kubectl patch endpoints kafka-external -n "${HOUSING_NS:-record-platform}" --type=merge -p="{\"subsets\":[{\"addresses\":[{\"ip\":\"$HOST_IP\"}],\"ports\":[{\"port\":${KAFKA_SSL_PORT:-29094},\"name\":\"kafka-ssl\"}]}]}" 2>&1 | tee -a "$DIAG_LOG" || true
            FIXED_THIS_SERVICE=1
          fi
        fi
      fi
    ...
    PROBE_ERROR=$(_kubectl describe pod "$POD" -n "${HOUSING_NS:-record-platform}" 2>&1 | grep -i "startup probe failed\|liveness probe failed\|readiness probe failed" | head -1 || echo "")
...
# 6. Exporters (app namespace, 1/1 each)
say "6. Checking exporters ($HOUSING_CHECK_NS, 1/1 each)..."
for ex in nginx-exporter haproxy-exporter; do
  R=$(_kubectl get deploy -n "${HOUSING_NS:-record-platform}" "$ex" -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
  S=$(_kubectl get deploy -n "${HOUSING_NS:-record-platform}" "$ex" -o jsonpath='{.spec.replicas}' 2>/dev/null || echo "1")
  [[ -z "$R" ]] && R="0"; [[ -z "$S" ]] && S="1"
  if [[ "$R" == "1" && "$S" == "1" ]]; then
    ok "$ex: 1/1"
  else
