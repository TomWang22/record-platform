...
if [[ -f "$PATHS_FILE" ]]; then
  ...
  need STARTUP_FOR_OCH_REVIEW.md
...
forbid docker-compose.external-och.yml
forbid services/booking-service
forbid services/social-service
forbid infra/k8s/base/booking-service
forbid infra/k8s/base/social-service

for svc in messaging-service media-service trust-service notification-service; do
  need "services/$svc"
done

if grep -rq 'off-campus-housing-tracker' "$ROOT/docker-compose.yml" 2>/dev/null; then
  echo "❌ docker-compose references off-campus-housing-tracker"
  FAIL=1
fi
...
if grep -rq 'off-campus-housing-tracker' "$ROOT/k8s" 2>/dev/null; then
  echo "❌ k8s/ must use record-platform namespace (not off-campus-housing-tracker)"
  FAIL=1
fi
# Legacy OCH Kafka secret name must not appear in manifests (avoid literal in repo grep audits).
_legacy_och_kafka='och'"-"'kafka-ssl-secret'
if grep -rq "${_legacy_och_kafka}" "$ROOT/k8s" 2>/dev/null; then
  echo "❌ k8s/ must mount kafka-ssl-secret (not legacy OCH kafka secret name)"
  FAIL=1
fi
