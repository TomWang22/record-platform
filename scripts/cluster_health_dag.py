...
def _doctor_min_score(*, strict: bool) -> int:
    if strict or os.environ.get("CLUSTER_DOCTOR_STRICT", "") == "1":
        return int(os.environ.get("RP_CLUSTER_DOCTOR_STRICT_MIN", "95"))
    return int(os.environ.get("RP_CLUSTER_DOCTOR_MIN_SCORE", "90"))


def _bootstrap_min_score() -> int:
    return int(os.environ.get("RP_CLUSTER_DOCTOR_MIN_SCORE", "90"))


# Keep in sync with scripts/lib/rp-runtime-deploy-services.sh (+ edge nginx/webapp).
...
def _redis_host_port() -> tuple[str, int]:
    ...
    port = int(os.environ.get("RP_REDIS_PORT", "6379"))
    return host, port


def _edge_leaf_cert(repo: Path) -> Path | None:
    for name in ("record-platform.test.crt", "record.test.crt"):
        p = repo / "certs" / name
        if p.is_file():
            return p
    return None
...
def _housing_deployments() -> list[str]:
    names = [*RP_RUNTIME_APP_DEPLOYS, "nginx", "webapp"]
    if os.environ.get("RP_CLUSTER_DOCTOR_INCLUDE_TRANSPORT_WATCHDOG", "").strip() == "1":
        names.append("transport-watchdog")
    return names


def _default_image_services() -> list[str]:
    # scripts/lib/rp-active-image-targets.sh — RP_DOCKER_BUILD_SERVICES + webapp
    names = [
        "api-gateway",
        "auth-service",
        "records-service",
    ...
    if os.environ.get("RP_CLUSTER_DOCTOR_INCLUDE_TRANSPORT_WATCHDOG", "").strip() != "1":
        names = [n for n in names if n != "transport-watchdog"]
    return list(dict.fromkeys(names))


def _endpoint_services() -> list[str]:
    # Webapp is reached via nginx; no standalone Service required for doctor score.
    return list(dict.fromkeys([*RP_RUNTIME_APP_DEPLOYS, "nginx"]))


def _forbidden_cluster_violations(ns: str) -> list[str]:
    violations: list[str] = []
    for bad_ns in RP_FORBIDDEN_NAMESPACES:
        out, rc = _kubectl(["get", "ns", bad_ns, "--no-headers"], timeout=15)
        if rc == 0 and out.strip():
            violations.append(f"forbidden namespace present: {bad_ns}")
    for dep in RP_FORBIDDEN_DEPLOYMENTS:
        out, rc = _kubectl(
            ["get", "deploy", dep, "-n", ns, "--no-headers"],
            timeout=15,
        )
        if rc == 0 and out.strip():
            violations.append(f"forbidden deployment in {ns}: {dep}")
    return violations
...
def score_infra(ns: str, repo: Path, warnings: list[str]) -> float:
    ...
    try:
        ...
        warnings.append(f"infra: Redis {redis_host}:{redis_port} not reachable ({e})")
...
def score_security(ns: str, repo: Path, warnings: list[str]) -> float:
    ...
    if leaf_crt is not None:
        ...
        warnings.append("security: missing edge leaf cert (record-platform.test.crt)")

    need = [(name, ns) for name in _required_cluster_secrets()]
    ok_s = 0
    ...
    for rel in (
        ...
        if path.is_file():
            with open(path, encoding="utf-8") as f:
                return json.load(f)
    raise FileNotFoundError("missing rp-cluster-dependency-dag.json")


def _ns_default(dag: dict[str, Any], node_cfg: dict[str, Any]) -> str:
    return str(node_cfg.get("namespace") or dag.get("housing_ns_default", "record-platform"))
...
def _artifact_drift_markers(ns: str, repo: Path) -> dict[str, Any]:
    ...
    for sec_name in _required_cluster_secrets():
        rv, rc = _kubectl(
...
def cmd_bootstrap(ns: str, repo: Path) -> int:
    ...
    min_score = _bootstrap_min_score()
    if health["score"] < min_score:
        print(
            f"❌ bootstrap-health: score < {min_score} — failing bootstrap",
            file=sys.stderr,
        )
        return 2
    if not dag["valid"]:
...
def cmd_doctor(repo: Path, strict: bool) -> int:
    ...
    dag = validate_dependency_dag(ns, repo)
    transport = _edge_transport(repo)
    forbidden = _forbidden_cluster_violations(ns)
    if forbidden:
        for v in forbidden:
            dag.setdefault("violations", []).append(v)
        dag["valid"] = False
    ...
    min_score = _doctor_min_score(strict=strict)
    if forbidden:
        print("Forbidden (RP):", flush=True)
        for v in forbidden:
            print(f"  - {v}", flush=True)
    if live["score"] < min_score:
        print(f"❌ cluster-doctor: live score < {min_score}", flush=True)
        return 1
    if forbidden:
        return 1
    return 0
...
def main() -> int:
    ...
    b.add_argument("--ns", default=os.environ.get("HOUSING_NS", "record-platform"))
    ...
    dr.add_argument("--ns", default=os.environ.get("HOUSING_NS", "record-platform"))
