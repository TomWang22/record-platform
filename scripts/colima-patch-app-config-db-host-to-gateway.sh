...
#   DB_HOST_GATEWAY_IP=192.168.5.2 HOUSING_NS=record-platform ./scripts/colima-patch-app-config-db-host-to-gateway.sh
#
# After apply, restart app pods if needed: ./scripts/rebuild-rp-images-and-rollout.sh with ROLLOUT only, or kubectl rollout restart.
set -euo pipefail
