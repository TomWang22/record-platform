...
if colima status 2>/dev/null | grep -q Running; then
  ...
  _colima_args=()
  rp_colima_build_start_argv _colima_args "$COLIMA_CPU" "$COLIMA_MEMORY" "$COLIMA_DISK" "$COLIMA_K3S_VERSION"
  rp_colima_print_start_argv _colima_args
  if ! colima "${_colima_args[@]}" 2>&1; then
    RP_COLIMA_USE_VZ=0
    _colima_args=()
    rp_colima_build_start_argv _colima_args "$COLIMA_CPU" "$COLIMA_MEMORY" "$COLIMA_DISK" "$COLIMA_K3S_VERSION"
    rp_colima_print_start_argv _colima_args
    colima "${_colima_args[@]}" 2>&1
  fi
  ok "Colima started (k3s servicelb + traefik disabled)"
fi
