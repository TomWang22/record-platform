...
if [[ "${RP_USE_BRIDGE_KUBECONFIG:-1}" == "1" ]]; then
  info "RP_USE_BRIDGE_KUBECONFIG=1 — skipping 127.0.0.1:6443 tunnel; aligning kubeconfig to bridge IP after API is up"
else
...
if [[ "${RP_USE_BRIDGE_KUBECONFIG:-1}" == "1" ]] && [[ -f "$REPO_ROOT/scripts/lib/rp-bootstrap-kubeconfig.sh" ]]; then
  # shellcheck source=lib/rp-bootstrap-kubeconfig.sh
  source "$REPO_ROOT/scripts/lib/rp-bootstrap-kubeconfig.sh"
  rp_bootstrap_align_host_kubeconfig || warn "kubeconfig bridge align failed (run scripts/rp-align-colima-kubeconfig.sh)"
fi
