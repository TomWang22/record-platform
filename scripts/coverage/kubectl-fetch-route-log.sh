...
#
# Remote path inside the api-gateway container: GATEWAY_ROUTE_COVERAGE_POD_FILE (default /tmp/rp-routes-hit.jsonl).
...
mkdir -p "$(dirname "$OUT")"
REMOTE="${GATEWAY_ROUTE_COVERAGE_POD_FILE:-/tmp/rp-routes-hit.jsonl}"
