...
#
# Prereqs: Colima bridge kubeconfig API (./scripts/rp-kube-api-health.sh), pods kafka-0..2 Ready.
#
# Env:
...
NS="${KAFKA_K8S_NS:-record-platform}"
...
if [[ "${KAFKA_K8S_SKIP_API_HEALTH:-0}" != "1" ]] && [[ -f "$SCRIPT_DIR/rp-ensure-kube-api.sh" ]]; then
  bash "$SCRIPT_DIR/rp-ensure-kube-api.sh" || die "API unhealthy — re-run make cold-bootstrap or make bootstrap"
fi
...
rp_kafka_event_topics_fill || die "Could not build topic list from proto/events mirror"
...
k8s_kafka_topics() {
  kubectl exec -n "$NS" "$KPOD" -- kafka-topics --bootstrap-server "$BS" --command-config /tmp/rp-k8s-topics.props "$@"
}
