#!/usr/bin/env bash
# Create domain event topics for Record Platform (must stay aligned with proto/events/* mirror and producers).
...
# Topic names from active proto/events mirror. See scripts/lib/rp-kafka-event-topics-from-proto.sh
...
} > /tmp/rp-kafka-event-topics.props'
...
if [[ -n "$KAFKA_DOCKER_CONTAINER" || -n "$KAFKA_DOCKER_COMPOSE_SERVICE" ]]; then
  ...
  CONFIG_ARGS=(--command-config /tmp/rp-kafka-event-topics.props)
