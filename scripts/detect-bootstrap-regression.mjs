...
const THRESHOLD = Number.parseFloat(process.env.REGRESSION_THRESHOLD || "1.2", 10);
...
function main() {
  ...
  for (const [phase, raw] of Object.entries(current)) {
    ...
    if (time > base.p95 * THRESHOLD) {
      ...
      if (phase === "B.crypto") {
        const destructiveTime = current["B.crypto:destructive"];
        const nonDestructiveTime = current["B.crypto:non-destructive"];
        if (destructiveTime != null) entry.mode = "destructive";
        else if (nonDestructiveTime != null) entry.mode = "non-destructive";
        entry.hint = "B.crypto records separate baselines for destructive vs non-destructive modes (B.crypto:destructive, B.crypto:non-destructive)";
      }
