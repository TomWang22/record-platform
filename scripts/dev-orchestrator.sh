...
_validate_server_leaf_if_present() {
  ...
  local _och=()
  if [[ -f "$REPO_ROOT/certs/record-platform.test.pem" ]]; then
    _och+=("$REPO_ROOT/certs/record-platform.test.pem")
  elif [[ -f "$REPO_ROOT/certs/record-platform.test.crt" ]]; then
    _och+=("$REPO_ROOT/certs/record-platform.test.crt")
  fi
  for f in "${_och[@]}" "$REPO_ROOT/certs/record-platform.test.pem" "$REPO_ROOT/certs/record-platform.test.crt"; do
    [[ -f "$f" ]] || continue
