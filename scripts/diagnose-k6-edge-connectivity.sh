...
# Caddy (ingress-nginx ns) routes /api/* and /auth/* to api-gateway:4000 (see repo Caddyfile + dev ingress).
...
#   HOUSING_NS=record-platform CA=certs/dev-root.pem ./scripts/diagnose-k6-edge-connectivity.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
HOUSING_NS="${HOUSING_NS:-record-platform}"
HOST="${EDGE_HOST:-record-platform.test}"
...
echo "  kubectl -n $HOUSING_NS port-forward svc/api-gateway 4000:4000"
echo "  curl -sS http://127.0.0.1:4000/healthz   # gateway liveness (no TLS)"
echo "  curl -sS http://127.0.0.1:4000/api/healthz"

say "k6 env (from host)"
echo "  export SSL_CERT_FILE=$CA"
echo "  export BASE_URL=https://${HOST}"
echo "  # If DNS missing: add \"$LB_IP $HOST\" to /etc/hosts or: RP_AUTO_EDGE_HOSTS=1 with sudo"
echo "  ./scripts/run-platform-k6-edge-smoke.sh"
