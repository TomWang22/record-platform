...
if kubectl get deploy "$GW_DEPLOY" -n "$NS_APP" --request-timeout=20s >/dev/null 2>&1; then
  _gc="$(
    kubectl exec -n "$NS_APP" "deploy/$GW_DEPLOY" -c app --request-timeout=30s -- \
      sh -c 'if command -v curl >/dev/null 2>&1; then curl -gfsS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 15 http://127.0.0.1:4000/healthz; elif command -v wget >/dev/null 2>&1; then wget -q -O /dev/null --timeout=15 http://127.0.0.1:4000/healthz && echo 200; else echo 000; fi' 2>/dev/null || echo "000"
  )"
  if [[ "$_gc" != "200" ]]; then
  ...
  _gz="$(
    kubectl exec -n "$NS_APP" "deploy/$GW_DEPLOY" -c app --request-timeout=45s -- \
      sh -c 'if command -v curl >/dev/null 2>&1; then curl -gfsS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 25 http://127.0.0.1:4000/readyz; elif command -v wget >/dev/null 2>&1; then wget -q -O /dev/null --timeout=25 http://127.0.0.1:4000/readyz && echo 200; else echo 000; fi' 2>/dev/null || echo "000"
  )"
  if [[ "$_gz" != "200" ]]; then
