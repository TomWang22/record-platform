...
# Get CA certificate for strict TLS (k6 and curl need this for record-platform.test:30443; use absolute path for SSL_CERT_FILE)
...
[[ -z "$CA_CERT" ]] && [[ -n "$K8S_CA" ]] && CA_CERT="/tmp/test-ca-adversarial-$$.pem" && echo "$K8S_CA" > "$CA_CERT" && ok "Using Kubernetes CA ($NS) for strict TLS"
