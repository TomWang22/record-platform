...
_tls_deploy_has_leaf() {
  local f="$1"
  grep -qE 'secretName:\s*(service-tls|edge-service-tls)\b' "$f"
}
...
for svc in "${SERVICES[@]}"; do
  ...
  if [[ "$svc" == "media-service" ]] && ! _tls_deploy_has_leaf "$DEPLOY_FILE"; then
    if grep -qE 'secretName:\s*kafka-ssl-secret\b' "$DEPLOY_FILE"; then
      ok "$svc: Kafka client mTLS (kafka-ssl-secret); gRPC health via TCP (no edge-service-tls mount in this Deployment)"
      continue
    fi
  fi
