...
# Record platform defaults (override with HOUSING_NS / K3D_REGISTRY* for other clusters).
_KNS="${HOUSING_NS:-record-platform}"
_K3D_REG="${K3D_REGISTRY:-k3d-record-platform-registry:5000}"
_K3D_REG_HOST="${K3D_REGISTRY_NAME:-k3d-record-platform-registry}"
...
if [[ "$ctx" == *"k3d"* ]] && command -v k3d >/dev/null 2>&1; then
  ...
  if kubectl get nodes --request-timeout=15s >/dev/null 2>&1; then
    ...
    _pods_not_ready=$(kubectl get pods -n "$_KNS" --no-headers 2>/dev/null | grep -v "Running\|Completed" | wc -l | tr -d ' ' || echo "?")
    _pods_total=$(kubectl get pods -n "$_KNS" --no-headers 2>/dev/null | wc -l | tr -d ' ' || echo "0")
    [[ "$_pods_total" -gt 0 ]] && echo "  $_KNS pods: $_pods_total total ($_pods_not_ready not Running/Completed)"
  else
...
REQUIRED_APP_IMAGES=( api-gateway auth-service records-service listings-service shopping-service messaging-service trust-service analytics-service media-service notification-service python-ai-service auction-monitor )
...
if [[ "$ENSURE_IMAGES" == "1" ]] && [[ "$ctx" == *"k3d"* ]] && command -v docker >/dev/null 2>&1; then
  ...
  if [[ ${#_missing[@]} -gt 0 ]]; then
    ...
    echo "  Then: kubectl rollout restart deploy -n $_KNS"
    exit 1
  fi
  ...
  if curl -s -o /dev/null -w "%{http_code}" --connect-timeout 2 "http://127.0.0.1:5000/v2/" 2>/dev/null | grep -qE '200|401'; then
    ...
    if [[ ${#_reg_missing[@]} -gt 0 ]]; then
      ...
      echo "  (Requires: 127.0.0.1 $_K3D_REG_HOST in /etc/hosts and registry in Docker insecure-registries)"
      exit 1
    fi
