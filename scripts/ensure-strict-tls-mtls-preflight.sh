...
_provision_from_repo() {
  if [[ -f "$CERTS_DIR/dev-root.pem" ]] && [[ -f "$CERTS_DIR/record-platform.test.crt" ]] && [[ -f "$CERTS_DIR/record-platform.test.key" ]]; then
    mkdir -p "$CERTS_DIR"
    kubectl -n "$NS" create secret generic service-tls \
      --from-file=ca.crt="$CERTS_DIR/dev-root.pem" \
      --from-file=tls.crt="$CERTS_DIR/record-platform.test.crt" \
      --from-file=tls.key="$CERTS_DIR/record-platform.test.key" \
...
_provision_from_mkcert() {
  ...
  mkcert -cert-file "$tmpd/tls.crt" -key-file "$tmpd/tls.key" \
    record-platform.test "*.record-platform.test" localhost 127.0.0.1 \
  ...
  cp -f "$tmpd/tls.crt" "$CERTS_DIR/record-platform.test.crt"
  cp -f "$tmpd/tls.key" "$CERTS_DIR/record-platform.test.key"
...
if [[ "$need" == "1" ]]; then
  if _provision_from_repo; then
    ...
    fail "Cannot provision: ensure certs/dev-root.pem, certs/record-platform.test.crt, certs/record-platform.test.key exist, or install mkcert (brew install mkcert && mkcert -install). Then re-run or run pnpm run reissue."
  fi
else
...
if [[ "${FORCE_TLS_RESTART:-0}" == "1" ]] || [[ $SECRET_UPDATED -eq 1 ]]; then
  say "Rollout restart (FORCE_TLS_RESTART=$FORCE_TLS_RESTART, SECRET_UPDATED=$SECRET_UPDATED; RP_ROLLOUT_SEQUENTIAL=${RP_ROLLOUT_SEQUENTIAL:-1})"
  # shellcheck source=scripts/lib/rp-sequential-rollout.sh
  source "$SCRIPT_DIR/lib/rp-sequential-rollout.sh"
  if [[ "${RP_ROLLOUT_SEQUENTIAL:-1}" == "1" ]]; then
    RP_ROLLOUT_NS="$NS" NS_ING="$NS_ING" rp_rollout_ordered_housing_apps
...
# Deployments mount edge-service-tls; reissue updates service-tls — always refresh alias so mounts never go stale.
_ensure_edge_service_tls_alias() {
  if ! kubectl -n "$NS" get secret service-tls -o name &>/dev/null; then
    warn "edge-service-tls skipped: service-tls missing"
    return 0
  fi
  local d
  d=$(mktemp -d 2>/dev/null || echo "/tmp/rp-tls-$$")
...
if [[ -f "$SCRIPT_DIR/lib/ensure-rp-grpc-certs.sh" ]]; then
  ...
  if rp_sync_grpc_certs_to_dir "${GRPC_CERTS_DIR:-/tmp/grpc-certs}" "$NS"; then
    ok "Synced edge-service-tls → ${GRPC_CERTS_DIR:-/tmp/grpc-certs}/{tls.crt,tls.key,ca.crt} (grpcurl mTLS)"
  else
