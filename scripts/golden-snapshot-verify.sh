#!/usr/bin/env bash
# "Ship it" golden snapshot: rebuild all Record Platform :dev images (Colima load), restart workloads, verify Kafka alignment.
# Prefer this over ad-hoc loops — uses the same service list as build-record-platform-images-k3s.sh.
#
...
#
# Env: HOUSING_NS (namespace), same as rebuild-all-record-platform-images-k3s.sh (IMAGE_TAG, SKIP_LOAD, DOCKER_DEFAULT_PLATFORM, …)
set -euo pipefail
...
if [[ "${SKIP_BUILD:-0}" != "1" ]]; then
  say "Step 1 — Rebuild all Record Platform images + Colima load + per-service rollouts"
  bash "$SCRIPT_DIR/rebuild-all-record-platform-images-k3s.sh"
else
  warn "SKIP_BUILD=1 — skipping rebuild-all-record-platform-images-k3s.sh"
fi
