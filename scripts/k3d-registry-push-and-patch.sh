...
# Patch deployments to use registry (inside k3d, nodes resolve ${K3D_REGISTRY_NAME:-k3d-record-platform-registry}:5000)
...
for app in "${APPS[@]}"; do
  if kubectl get deployment "$app" -n "$_APP_NS" --request-timeout=5s >/dev/null 2>&1; then
    kubectl set image "deployment/$app" -n "$_APP_NS" "app=${REGISTRY_ADDR}/${app}:dev" --request-timeout=15s 2>/dev/null && info "Patched $app" || warn "Patch $app failed"
  fi
done
