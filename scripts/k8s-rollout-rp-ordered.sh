...
# Usage: ./scripts/k8s-rollout-rp-ordered.sh
#   HOUSING_NS=record-platform  (default)
...
DEPS=()
for _d in "${RP_RUNTIME_APP_DEPLOYS[@]}"; do
  [[ "$_d" == "api-gateway" ]] && continue
  [[ "$_d" == "auth-service" ]] && continue
  DEPS+=("$_d")
done
