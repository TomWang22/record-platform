...
# Pins kafka-*-external MetalLB IPs first (KAFKA_SYNC_SKIP_METALLB_PIN=1 to skip).
#
...
chmod +x "$SCRIPT_DIR/ensure-dev-root-ca.sh" "$SCRIPT_DIR/kafka-runtime-sync.sh" "$SCRIPT_DIR/kafka-refresh-tls-from-lb.sh" "$SCRIPT_DIR/verify-kafka-cluster.sh" "$SCRIPT_DIR/kafka-auto-heal-inter-broker-tls.sh" "$SCRIPT_DIR/patch-kafka-external-metallb-pinned-ips.sh" 2>/dev/null || true
if [[ "${KAFKA_SYNC_SKIP_METALLB_PIN:-0}" != "1" ]]; then
  METALLB_POOL="${METALLB_POOL:-192.168.64.240-192.168.64.250}" HOUSING_NS="$NS" KAFKA_BROKER_REPLICAS="$REP" \
    bash "$SCRIPT_DIR/patch-kafka-external-metallb-pinned-ips.sh" || echo "⚠️  kafka-sync-metallb: MetalLB pin step failed (continuing)" >&2
fi
