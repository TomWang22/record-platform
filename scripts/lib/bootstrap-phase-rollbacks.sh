...
rp_bootstrap_rollback_dispatch() {
  local node="${1:?node id}"
  case "$node" in
    A.workspace) rp_rollback_A_workspace ;;
    B.crypto) rp_rollback_B_crypto ;;
    C.infra) rp_rollback_C_infra ;;
    C.images) rp_rollback_C_images ;;
    C.image_contract) echo "  ↩️  rollback C.image_contract — noop (re-run scripts/rp-verify-image-build-contract.sh)" >&2 ;;
    G.app_runtime) rp_rollback_G_app_runtime ;;
    D.observability) rp_rollback_D_observability ;;
    F.kafka_alignment) rp_rollback_F_kafka_alignment ;;
    E.transport) rp_rollback_E_transport ;;
    *)
