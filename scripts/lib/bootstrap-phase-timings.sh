...
# Wall clock ms (stdout). Use as $(_rp_bootstrap_ms_now) — inside $((…)) a bare _rp_bootstrap_ms_now is a variable name, not a call (breaks with set -u).
_rp_bootstrap_ms_now() {
  python3 -c "import time; print(int(time.time()*1000))"
}

_rp_bootstrap_timing_json() {
  echo "${REPO_ROOT}/bench_logs/bootstrap_phase_timings.json"
}

_rp_bootstrap_record_phase_timing_ms() {
  local phase="${1:?phase}"
  local ms="${2:?ms}"
  mkdir -p "${REPO_ROOT}/bench_logs"
  local f
  f="$(_rp_bootstrap_timing_json)"
...
_rp_bootstrap_errors_dir() {
  echo "${REPO_ROOT}/bench_logs/bootstrap_errors"
}

# Write a host-side error snippet for a DAG node (path safe for logs).
_rp_bootstrap_write_phase_error_log() {
  local node="${1:?node}"
  local title="${2:-error}"
  local out
  mkdir -p "$(_rp_bootstrap_errors_dir)"
  out="$(_rp_bootstrap_errors_dir)/${node}.log"
  {
