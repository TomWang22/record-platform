...
#   2) Bridge IP: RP_COLIMA_HOST_IP, else parse `colima list`, else RP_COLIMA_HOST_IP_FALLBACK (192.168.64.7).
#
# Env:
#   RP_K8S_API_SERVER_OVERRIDE            — full URL (e.g. https://192.168.64.7:50620); wins over VM port + bridge IP.
#   RP_COLIMA_KUBECONFIG_ALIGN_HOST_API — default 1: rewrite host kubeconfig only. Set 0 to skip (strict 127.0.0.1:6443 tunnel; see colima-forward-6443.sh).
#   RP_COLIMA_HOST_IP                     — bridge IP for the VM (overrides colima list + fallback).
#   RP_COLIMA_HOST_IP_FALLBACK            — default 192.168.64.7 when colima list has no ADDRESS yet.

rp_colima_bridge_host_ip() {
  if [[ -n "${RP_COLIMA_HOST_IP:-}" ]]; then
    printf '%s\n' "$RP_COLIMA_HOST_IP"
    return 0
  fi
  if [[ -n "${RP_COLIMA_HOST_IP:-}" ]]; then
  ...
  printf '%s\n' "${RP_COLIMA_HOST_IP_FALLBACK:-${RP_COLIMA_HOST_IP_FALLBACK:-192.168.64.7}}"
}

rp_colima_compute_host_api_server() {
  if [[ -n "${RP_K8S_API_SERVER_OVERRIDE:-}" ]]; then
    printf '%s\n' "$RP_K8S_API_SERVER_OVERRIDE"
    return 0
  fi
  ...
  host="$(rp_colima_bridge_host_ip)"
  [[ -n "$host" ]] || return 1
  printf 'https://%s:%s\n' "$host" "$port"
  return 0
}

_rp_kubectl_set_cluster_server() {
...
rp_align_colima_host_api_in_kubeconfig() {
  local kcfg="$1"
  [[ "${RP_COLIMA_KUBECONFIG_ALIGN_HOST_API:-1}" == "0" ]] && return 0
  [[ -s "$kcfg" ]] || return 1
  ...
  if [[ -z "${_RP_COLIMA_HOST_API_SERVER_CACHED:-}" ]]; then
    _RP_COLIMA_HOST_API_SERVER_CACHED="$(rp_colima_compute_host_api_server)" || true
  fi
  [[ -n "${_RP_COLIMA_HOST_API_SERVER_CACHED:-}" ]] || return 1
  _rp_kubectl_set_cluster_server "$kcfg" "${_RP_COLIMA_HOST_API_SERVER_CACHED}"
}

# Align both Colima kubeconfig paths (new + legacy) when present; export KUBECONFIG to the first existing file.
rp_align_colima_kubeconfig_host_api() {
  unset _RP_COLIMA_HOST_API_SERVER_CACHED
  local f s
  s="$(rp_colima_compute_host_api_server)" || return 1
  [[ -n "$s" ]] || return 1
  for f in "${HOME}/.colima/default/kubernetes/kubeconfig" "${HOME}/.colima/default/kubeconfig"; do
    [[ -s "$f" ]] || continue
    _rp_kubectl_set_cluster_server "$f" "$s" || true
  done
  for f in "${HOME}/.colima/default/kubernetes/kubeconfig" "${HOME}/.colima/default/kubeconfig"; do
    if [[ -s "$f" ]]; then
      export KUBECONFIG="$f"
...
rp_export_colima_kubeconfig_prefer_reachable() {
  unset _RP_COLIMA_HOST_API_SERVER_CACHED
  local align="${RP_COLIMA_KUBECONFIG_ALIGN_HOST_API:-1}"

  _rp_try_one_colima_kubeconfig() {
    local f="$1"
    [[ -s "$f" ]] || return 1
    export KUBECONFIG="$f"
    if kubectl config get-contexts -o name 2>/dev/null | grep -qi colima; then
    ...
    if [[ "$align" != "0" ]]; then
      rp_align_colima_host_api_in_kubeconfig "$f" || true
    fi
  ...
  local f other
  for f in "${HOME}/.colima/default/kubernetes/kubeconfig" "${HOME}/.colima/default/kubeconfig"; do
    if _rp_try_one_colima_kubeconfig "$f"; then
      if [[ "$align" != "0" ]]; then
        unset _RP_COLIMA_HOST_API_SERVER_CACHED
        for other in "${HOME}/.colima/default/kubernetes/kubeconfig" "${HOME}/.colima/default/kubeconfig"; do
          [[ "$other" == "$f" ]] && continue
          [[ -s "$other" ]] || continue
          rp_align_colima_host_api_in_kubeconfig "$other" || true
        done
      fi
      return 0
    fi
