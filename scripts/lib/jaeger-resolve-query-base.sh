...
#   3) Edge URL https://record-platform.test/jaeger (Caddy → Jaeger; needs QUERY_BASE_PATH=/jaeger on deploy)
#   4) Any svc jaeger / jaeger-query with .status.loadBalancer.ingress[0].ip in common namespaces
#
# Env:
#   JAEGER_DISCOVER_EDGE_JAEGER — default 1; set 0 to skip https://record-platform.test/jaeger probe
#   JAEGER_EDGE_JAEGER_URL — default https://record-platform.test/jaeger
#   NODE_EXTRA_CA_CERTS — for HTTPS edge probe (fallback: $REPO_ROOT/certs/dev-root.pem)
#   TARGET_IP / OCH_EDGE_IP — optional --resolve record-platform.test:443:<ip> when DNS has no LB IP
#
...
och_jaeger_services_curl_ok() {
  ...
  if [[ "$base" == https:* ]]; then
    ...
    [[ -f "$ca" ]] && c+=(--cacert "$ca")
    if [[ -n "${TARGET_IP:-}" ]]; then
      c+=(--resolve "record-platform.test:443:${TARGET_IP}")
    elif [[ -n "${OCH_EDGE_IP:-}" ]]; then
      c+=(--resolve "record-platform.test:443:${OCH_EDGE_IP}")
    elif [[ -n "${REACHABLE_LB_IP:-}" ]]; then
      c+=(--resolve "record-platform.test:443:${REACHABLE_LB_IP}")
    fi
...
och_jaeger_resolve_query_base() {
  ...
  if [[ "${JAEGER_DISCOVER_EDGE_JAEGER:-1}" != "0" ]]; then
    local edge="${JAEGER_EDGE_JAEGER_URL:-https://record-platform.test/jaeger}"
    edge="${edge%/}"
