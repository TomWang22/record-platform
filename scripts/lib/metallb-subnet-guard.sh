...
#   rp_metallb_pool_first_ip "$METALLB_POOL"  → first IP in range string
#   rp_ipv4_prefix "a.b.c.d" → a.b.c
#   rp_colima_eth0_ipv4
#   rp_k8s_node_internal_ipv4
#   rp_assert_metallb_pool_coherent "$METALLB_POOL"  → exit 0 or 1 (stderr on failure)
...
rp_assert_metallb_pool_coherent() {
  local pool="${1:-}"
  ...
  pool_ip="$(rp_metallb_pool_first_ip "$pool")" || {
    echo "❌ Invalid METALLB_POOL (expected start IP): $pool" >&2
    return 1
  }
  pool_p="$(rp_ipv4_prefix "$pool_ip")" || return 1

  local vm_ip="" vm_p="" node_ip="" node_p=""
  vm_ip="$(rp_colima_eth0_ipv4 || true)"
  if [[ -n "$vm_ip" ]]; then
    vm_p="$(rp_ipv4_prefix "$vm_ip" || true)"
  fi
  ...
  if command -v kubectl >/dev/null 2>&1 && kubectl get nodes --request-timeout=8s &>/dev/null; then
    node_ip="$(rp_k8s_node_internal_ipv4 || true)"
    if [[ -n "$node_ip" ]]; then
      node_p="$(rp_ipv4_prefix "$node_ip")" || true
