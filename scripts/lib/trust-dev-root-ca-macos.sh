#!/usr/bin/env bash
# Add the dev-root CA to macOS keychain so k6, curl, and browsers trust https://record-platform.test (avoids x509 and manual Keychain Access).
# Call after rotation syncs certs/dev-root.pem, or before any step that hits record-platform.test with TLS.
...
if security add-trusted-cert -d -r trustRoot -k "$LOGIN_KEYCHAIN" "$CA_FILE" 2>/dev/null; then
  ok "Dev CA added to macOS login keychain (Always Trust for SSL). k6/curl/browser will trust record-platform.test."
  exit 0
fi
if security add-trusted-cert -d -r trustAsRoot -k "$LOGIN_KEYCHAIN" "$CA_FILE" 2>/dev/null; then
  ok "Dev CA added to macOS login keychain (trustAsRoot). k6/curl/browser will trust record-platform.test."
  exit 0
fi
