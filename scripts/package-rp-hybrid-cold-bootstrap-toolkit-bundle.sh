#!/usr/bin/env bash
# Record Platform hybrid cold-bootstrap toolkit (NOT och-cold-bootstrap-toolkit-*).
#
...
RSYNC_EXCLUDES=(
  ...
  --exclude '.DS_Store'
  --exclude '.env'
  --exclude 'docker-compose.external-och.yml'
  --exclude 'services/booking-service'
  --exclude 'services/social-service'
  --exclude 'proto/events/booking.proto'
  --exclude 'proto/events/social.proto'
  --exclude 'infra/k8s/base/booking-service'
  --exclude 'infra/k8s/base/social-service'
  --exclude 'toolkit-reference'
  --exclude 'reports/och-cold-bootstrap-*'
  --exclude 'reports/record-platform-och-*'
)
...
NOT och-cold-bootstrap-toolkit-*. RP runtime: Postgres 5433–5443, namespace record-platform,
host record-platform.test, k3s KRaft Kafka only (no booking/social/OCH compose ports).
...
For OCH comparison: read STARTUP_FOR_OCH_REVIEW.md in this tree.
