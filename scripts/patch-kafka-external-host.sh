...
resolve_host_ip() {
  ...
  local RAW
  RAW=$(kubectl run rp-resolve-kafka-host --rm -i --restart=Never --image=busybox:1.36 -n "${HOUSING_NS:-record-platform}" -- \
...
kubectl patch endpoints kafka-external -n "${HOUSING_NS:-record-platform}" --type=merge \
