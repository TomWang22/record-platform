...
#   SKIP_ZIP=1        — only create rp-perf-canonical-10-v2 + summary.json
...
#   rp-perf-canonical-10-v2/ — exactly 10 named files
#   summary.json              — manifest + sha256 per file
#   perf-bundle-<STAMP>.zip   — rp-perf-canonical-10-v2/ + summary.json
set -euo pipefail
...
OUT="$RUN_DIR/rp-perf-canonical-10-v2"
mkdir -p "$OUT"
...
out_dir = run_dir / "rp-perf-canonical-10-v2"
names = [
  "latency-report.md",
  "service-latency.csv",
...
doc = {
    "schema": "rp-perf-canonical-10-v2",
    "run_dir": str(run_dir),
    "stamp": "$STAMP",
...
if [[ "${SKIP_ZIP:-0}" != "1" ]]; then
  (cd "$RUN_DIR" && zip -q -r "perf-bundle-${STAMP}.zip" "rp-perf-canonical-10-v2" summary.json)
  echo "Wrote: $ZIP"
else
