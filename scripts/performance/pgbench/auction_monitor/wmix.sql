-- WMIX_OWNER_RANDOMIZED (auction_monitor): weighted mix of W1 / W2 / W3 style statements.
...
\if :pick <= 40
BEGIN;
INSERT INTO auction_monitor.pgbench_domain_touch (id, touched_at, note)
VALUES (
...
ON CONFLICT (id) DO UPDATE
  SET touched_at = EXCLUDED.touched_at,
      note = EXCLUDED.note;
INSERT INTO auction_monitor.outbox_events (
...
WITH claimed AS (
  SELECT id
  FROM auction_monitor.outbox_events
  WHERE published = false
  ORDER BY created_at
  LIMIT :batch
  FOR UPDATE SKIP LOCKED
)
UPDATE auction_monitor.outbox_events AS o
SET published = true
FROM claimed
