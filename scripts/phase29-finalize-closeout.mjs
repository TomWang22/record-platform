...
import { writeMatrixArtifacts, MATRIX_TARGET } from './lib/phase29-controlled-matrix-summary.mjs';
import { mergeRows } from './phase29-summarize-controlled-matrix.mjs';

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const outBase = process.env.PHASE28_MATRIX_OUT || '/tmp/phase29-controlled-observability-matrix';

function mergeShardsWithRetryOverrides() {
  const rows = mergeRows(outBase);
  const merged = path.join(outBase, 'phase29-matrix.jsonl');
...
run('node', [path.join(repoRoot, 'scripts/phase29-generate-kpi-report-readonly.mjs'), '/tmp/phase29-kpi-report']);
const reportDir = path.join(outBase, 'phase29-kpi-report');
fs.mkdirSync(reportDir, { recursive: true });
spawnSync('cp', ['-R', '/tmp/phase29-kpi-report/.', reportDir], { stdio: 'inherit' });
run(path.join(repoRoot, 'services/python-ai-service/.venv/bin/python'), [
  path.join(repoRoot, 'scripts/phase29-disable-switch-rollback-drill.py'),
]);
