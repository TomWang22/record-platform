#!/usr/bin/env python3
"""
Phase 29C — local/dev KPI pipeline durability drill.
Run with: services/python-ai-service/.venv/bin/python scripts/phase29-local-dev-kpi-pipeline-durability-drill.py
"""
...
OUT_JSON = Path("/tmp/phase29-pipeline-durability-drill.json")
REPORT_DIR = Path("/tmp/phase29-pipeline-durability-report")
...
async def ensure_ingestion_run(conn, label: str = "phase29") -> str:
...
async def populate_pipeline_rows(conn, run_id: str) -> Dict[str, Any]:
    ...
    source_hash = hash_source_id("phase29-controlled-fixture")
    arrived = _now()

    ingestion_id = await write_kpi_ingestion_event(
        {
            "ingestion_run_id": run_id,
            "source_type": "phase29_controlled",
    ...
    searchability_id = await write_kpi_searchability_check(
        {
            ...
            "arrival_to_searchable_ms": 55,
            "probe_query_hash": hash_source_id("phase29-probe"),
            "probe_status": "PASS",
            "protocol": "HTTP/1.1",
        }
    )
        ...
        qid = await write_kpi_query_observation(
            {
                ...
                "case_id": f"phase29-{protocol.replace('/', '')}",
    ...
    usefulness_specs = [
        ...
        ("HTTP/1.1", "Phase 29 controlled observability production-enablement matrix: 25920/25920 target"),
    ]
...
async def main() -> int:
    ...
    try:
        ...
        run_id = await ensure_ingestion_run(conn, "phase29c")
