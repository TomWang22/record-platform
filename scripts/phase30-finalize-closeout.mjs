...
import { writeMatrixArtifacts, MATRIX_TARGET } from './lib/phase30-controlled-matrix-summary.mjs';
import { mergeRows } from './phase30-summarize-controlled-matrix.mjs';

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const outBase = process.env.PHASE30_MATRIX_OUT || '/tmp/phase30-controlled-staging-matrix';

function mergeShardsWithRetryOverrides() {
  const rows = mergeRows(outBase);
  const merged = path.join(outBase, 'phase30-matrix.jsonl');
...
run('node', [path.join(repoRoot, 'scripts/phase30-generate-kpi-report-readonly.mjs'), '/tmp/phase30-kpi-report']);
const reportDir = path.join(outBase, 'phase30-kpi-report');
fs.mkdirSync(reportDir, { recursive: true });
spawnSync('cp', ['-R', '/tmp/phase30-kpi-report/.', reportDir], { stdio: 'inherit' });
run(path.join(repoRoot, 'services/python-ai-service/.venv/bin/python'), [
  path.join(repoRoot, 'scripts/phase30-disable-switch-rollback-drill.py'),
]);
