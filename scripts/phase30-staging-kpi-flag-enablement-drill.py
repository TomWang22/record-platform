#!/usr/bin/env python3
"""
Phase 30DB–27G controlled staging/non-prod KPI enablement drill.
...
async def ensure_ingestion_run(conn) -> str:
    run_id = await conn.fetchval(
        ...
        VALUES ('completed', now(), now(), '{"phase30":1}'::jsonb)
        RETURNING id::text
        """
    )
    return str(run_id)
...
async def populate_rows(conn, run_id: str) -> Dict[str, Any]:
        ...
        qid = await write_kpi_query_observation(
            {
                ...
                "workflow": "phase30_controlled_smoke",
                "rag_total_ms": 25,
    ...
    for protocol, label in usefulness_specs:
        payload = build_usefulness_observation_payload(
            ...
            environment="staging",
            workflow="phase30_controlled_smoke",
            case_id=f"phase30-usefulness-{label[:8]}",
            quality_score=4.0,
        )
...
async def main() -> int:
    ...
    out_path = Path("/tmp/phase30-staging-kpi-flag-enablement-drill.json")
