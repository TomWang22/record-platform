#!/usr/bin/env python3
"""
Phase 31C — local/dev KPI pipeline durability drill.
Run with: services/python-ai-service/.venv/bin/python scripts/phase31-local-dev-kpi-pipeline-durability-drill.py
"""
...
OUT_JSON = Path("/tmp/phase31-pipeline-durability-drill.json")
REPORT_DIR = Path("/tmp/phase31-pipeline-durability-report")
...
async def ensure_ingestion_run(conn, label: str = "phase31") -> str:
...
async def populate_pipeline_rows(conn, run_id: str) -> Dict[str, Any]:
    ...
    source_hash = hash_source_id("phase31-controlled-fixture")
    arrived = _now()

    ingestion_id = await write_kpi_ingestion_event(
        {
            "ingestion_run_id": run_id,
            "source_type": "phase31_controlled",
    ...
    searchability_id = await write_kpi_searchability_check(
        {
            ...
            "arrival_to_searchable_ms": 55,
            "probe_query_hash": hash_source_id("phase31-probe"),
            "probe_status": "PASS",
            "protocol": "HTTP/1.1",
        }
    )
        ...
        qid = await write_kpi_query_observation(
            {
                ...
                "case_id": f"phase31-{protocol.replace('/', '')}",
                "workflow": "phase31_staging_matrix",
                "rag_total_ms": 120,
    ...
    usefulness_specs = [
        ...
        ("HTTP/1.1", "Phase 31D-R2 repaired staging long-soak matrix: 51840/51840 target"),
    ]
    ...
    for protocol, label in usefulness_specs:
        payload = build_usefulness_observation_payload(
            ...
            environment="staging",
            workflow="phase31_controlled_matrix",
            case_id=f"phase31-{label[:12]}",
            quality_score=4.0,
        )
...
async def main() -> int:
    ...
    try:
        ...
        run_id = await ensure_ingestion_run(conn, "phase31c")
