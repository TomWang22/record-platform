#!/usr/bin/env node
/**
 * Phase 32H-R1 — collector exclusivity smoke (30 triplet batches / 90 probes).
 */
...
import { R1_CANARY_PER_PROTOCOL, R1_EVIDENCE_LABEL_BASELINE } from './lib/phase32h-r1-config.mjs';
import {
  acquireLauncherLock,
  generateRunId,
  initRunState,
...
async function main() {
  ...
  if (!opts.out.startsWith('/tmp/')) throw new Error('collector exclusivity smoke out must be under /tmp');
  ...
  return withSmokeCollectorCleanup(
    ...
    async () => {
      ...
      acquireLauncherLock(opts.out, { pid: process.pid, run_id: runId, role: 'collector-exclusivity-smoke' });
      ...
      const report = {
        ...
        collector_exclusivity: 'PASS',
        collector_coverage: 'PASS',
        target_batches: TARGET_BATCHES,
        target_probes: TARGET_PROBES,
      };
      fs.writeFileSync(
        path.join(opts.out, 'phase32h-r1-collector-exclusivity-smoke.json'),
