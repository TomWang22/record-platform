#!/usr/bin/env node
/**
 * Phase 32H-R1 — collector registry + ring segment growth smoke (30 batches / 90 probes).
 */
...
const DEFAULT_OUT = '/tmp/phase32h-r1-collector-registry-ring-smoke-v1';
...
function evaluateRegistryRingSemantic(outRoot, runId, launchHead) {
  ...
  const pcapStatus = fs.existsSync(path.join(outRoot, 'pcap/capture-status.json'))
  ...
  const configuredBaseMatch = entry?.output_path === ringSpec.configured_output_base ? 'PASS' : 'FAIL';
  const ringDiscovery =
    ringSpec.output_mode === 'ring_buffer' && discovery.segment_count >= 1 && discovery.active_segment ? 'PASS' : 'FAIL';
  const activeSegmentGrowth = identity.failure_class === 'PCAP_OUTPUT_NOT_GROWING' ? 'FAIL' : 'PASS';
  return {
    ...
    configured_output_base_match: configuredBaseMatch,
    output_path_match: entry?.output_path === proc?.output_path ? 'PASS' : 'FAIL',
    root_match: entry?.evidence_root === outRoot ? 'PASS' : 'FAIL',
    ring_segment_discovery: ringDiscovery,
    active_segment_growth: activeSegmentGrowth,
    ...
    identity,
    verification,
    ring_discovery: discovery,
  };
}

async function main() {
  const opts = parseArgs(process.argv.slice(2));
  if (!opts.out.startsWith('/tmp/')) throw new Error('collector registry ring smoke out must be under /tmp');
  ...
  return withSmokeCollectorCleanup(
    ...
    async () => {
      ...
      acquireLauncherLock(opts.out, { pid: process.pid, run_id: runId, role: 'collector-registry-ring-smoke' });
      ...
      const pcapStatus = fs.existsSync(path.join(opts.out, 'pcap/capture-status.json'))
        ...
        : null;
      const registrySemantic = evaluateRegistryRingSemantic(opts.out, runId, launchHead);
      ...
      const pass =
        ...
        registrySemantic.configured_output_base_match === 'PASS' &&
      ...
      fs.writeFileSync(
        path.join(opts.out, 'phase32h-r1-collector-registry-ring-smoke.json'),
      ...
      const shutdown = finalizeSmokeWithFreeze(opts.out, {
        repoRoot: REPO_ROOT,
        pass,
        hashManifestName: 'phase32h-r1-collector-registry-ring-smoke-sha256.txt',
        hashExcludeSuffixes: ['phase32h-r1-collector-registry-ring-smoke-sha256.txt', 'FROZEN_PASS_EVIDENCE'],
        markerName: 'FROZEN_PASS_EVIDENCE',
        markerContent: `${new Date().toISOString()}\nCOLLECTOR_REGISTRY_RING_SMOKE_PASS\n`,
