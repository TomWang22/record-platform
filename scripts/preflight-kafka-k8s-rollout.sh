...
kafka-topics --bootstrap-server $BS --command-config /tmp/rp-pf.props --list | head -25
echo --- total: \$(kafka-topics --bootstrap-server $BS --command-config /tmp/rp-pf.props --list | wc -l | tr -d \" \") topics ---
"
...
kafka-topics --bootstrap-server $BS --command-config /tmp/rp-pf.props --describe --topic $SAMPLE_TOPIC" 2>/dev/null; then
  :
else
  echo "Could not describe $SAMPLE_TOPIC (pick first topic)..."
  ft="$(kubectl exec -n "$NS" "$KPOD" -- bash -ec "$write_props
kafka-topics --bootstrap-server $BS --command-config /tmp/rp-pf.props --list | head -1")"
  if [[ -n "$ft" ]]; then
    kubectl exec -n "$NS" "$KPOD" -- bash -ec "$write_props
kafka-topics --bootstrap-server $BS --command-config /tmp/rp-pf.props --describe --topic \"$ft\""
  fi
fi
