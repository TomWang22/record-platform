...
if [[ -n "${TARGET_IP:-}" ]] && command -v grpcurl >/dev/null 2>&1 && [[ -d "$proto_dir" ]] && [[ -f "$proto_dir/health.proto" ]]; then
  ...
  [[ -f "$SCRIPT_DIR/../lib/ensure-rp-grpc-certs.sh" ]] && source "$SCRIPT_DIR/../lib/ensure-rp-grpc-certs.sh" && rp_sync_grpc_certs_to_dir "$grpc_certs_dir" "$NS" 2>/dev/null || true
  ca_file="${CA_CERT:-}"
