#!/usr/bin/env bash
# Rebuild selected Record Platform service images and rollout matching Deployments.
# Use this when you changed one or more backend services and want build -> load -> restart to be one command.
#
# Usage (repo root):
#   ./scripts/rebuild-record-platform-images-and-rollout.sh
...
#
# Env:
#   SERVICES   — space-separated (default: api-gateway analytics-service)
#   IMAGE_TAG  — default dev
#   HOUSING_NS — Kubernetes namespace (default record-platform; Makefile sets HOUSING_NS)
#   SKIP_LOAD  — passed to build-record-platform-images-k3s.sh (1 = do not colima docker load)
...
SERVICES="${SERVICES:-api-gateway analytics-service}"
...
export SERVICES
export IMAGE_TAG
"$SCRIPT_DIR/build-record-platform-images-k3s.sh"
