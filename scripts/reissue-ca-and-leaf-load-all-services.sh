...
#   HOST=record-platform.test      — leaf CN and SANs (default record-platform.test)
...
#   RP_ROLLOUT_STATUS_TIMEOUT — seconds for each kubectl rollout status during step 7 (default 180; dev-onboard strict sets 300).
...
data:
  ...
  ca.crt: \"$(_b64 "$CA_CRT")\"" > "$f_svc"
    ...
    sleep "$STEP2_SLEEP"
    ok "service-tls updated (record-platform) — tls.crt=leaf only, ca.crt=CA (apply)"
  else
    ...
    sleep "$STEP2_SLEEP"
    ok "service-tls updated (record-platform) — tls.crt=leaf only, ca.crt=CA"
  fi
  # api-gateway mounts service-tls; auth and most services mount edge-service-tls — must match or gRPC mTLS /readyz fails (wrong CA).
  log_progress "step 2c: syncing edge-service-tls from service-tls…"
  _ochd=$(mktemp -d)
  if _kubectl_step2 -n "$NS_APP" get secret service-tls -o jsonpath='{.data.ca\.crt}' 2>/dev/null | base64 -d >"$_ochd/ca.crt" \
    ...
    _kubectl_step2 -n "$NS_APP" create secret generic edge-service-tls \
    ...
    ok "edge-service-tls synced from service-tls (alias)"
  else
    warn "Could not read service-tls to sync edge-service-tls"
  fi
  rm -rf "$_ochd"
  ...
  cp "$LEAF_CRT" "$REPO_ROOT/certs/record-platform.test.crt"
  cp "$LEAF_KEY" "$REPO_ROOT/certs/record-platform.test.key"
  if [[ "${KAFKA_SSL:-0}" == "1" ]]; then
    cp "$CA_KEY" "$REPO_ROOT/certs/dev-root.key"
    chmod 600 "$REPO_ROOT/certs/dev-root.key" 2>/dev/null || true
    ok "certs/dev-root.pem|.key, record-platform.test.crt|.key (KAFKA_SSL=1: CA key persisted for kafka-ssl-from-dev-root)"
  else
    ok "certs/dev-root.pem, certs/record-platform.test.crt|.key updated"
  fi
  ...
  if [[ "$RESTART_SERVICES" == "1" ]]; then
    ...
    RP_ROLLOUT_NS="$NS_APP" NS_ING="$NS_ING" rp_rollout_ordered_housing_apps
    unset -f rp_kubectl 2>/dev/null || true
