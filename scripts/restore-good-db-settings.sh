...
say "=== Restoring Good Database Settings (from working backup) ==="
say "Target: 1.13M records, 100k hot slice, 28k TPS"
...
say "Applying GOOD settings from working backup..."

kubectl -n "$NS" exec "$POD" -c db -- psql -U postgres -d records -X -P pager=off -v ON_ERROR_STOP=1 <<'SQL'
-- ============================================
-- RESTORE GOOD SETTINGS (from working backup)
-- Target: 1.13M records, 100k hot slice, 28k TPS
-- ============================================

-- 1. System-level settings (persistent) - GOOD RUN SETTINGS
...
-- GIN indexes with fastupdate=off for better query performance
...
-- 6. VACUUM ANALYZE for fresh statistics (critical for 1.13M records)
VACUUM ANALYZE records.records;

-- 7. Prewarm critical indexes (for 100k hot slice)
...
-- 8. Verify settings match good run
SELECT 
  name, 
  setting, 
  unit,
...
ok "Good settings restored"
SQL

say "=== Settings Restored ==="
echo ""
echo "Applied settings from good run:"
echo "✅ random_page_cost = 1.1"
echo "✅ work_mem = 32MB"
echo "✅ effective_cache_size = 4GB"
echo "✅ max_parallel_workers = 12"
echo "✅ max_parallel_workers_per_gather = 4"
echo "✅ track_io_timing = on"
echo "✅ jit = off"
echo "✅ Indexes optimized (fastupdate=off)"
echo "✅ VACUUM ANALYZE completed"
echo "✅ Index prewarming completed"
echo ""
echo "Target: 1.13M records, 100k hot slice, 28k TPS"
echo "Next: Run benchmark to verify performance"
