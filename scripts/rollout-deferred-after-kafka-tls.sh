...
#
# Env: HOUSING_NS, NS_ING (default ingress-nginx), RP_ROLLOUT_STATUS_TIMEOUT
set -euo pipefail
...
export RP_ROLLOUT_NS="${HOUSING_NS:-record-platform}"
export NS_ING="${NS_ING:-ingress-nginx}"

echo "=== rollout-deferred-after-kafka-tls (ns=$RP_ROLLOUT_NS) ==="
