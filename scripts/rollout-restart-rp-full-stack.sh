...
#   api-gateway → caddy-h3 (see scripts/lib/rp-sequential-rollout.sh).
#
# Env:
...
#   RP_ROLLOUT_STATUS_TIMEOUT — seconds per rollout status (default 180)
set -euo pipefail
...
echo "=== rollout-restart-rp-full-stack (ns=$HOUSING_NS, ingress=$NS_ING) ==="

if [[ "${SKIP_ENSURE_CLUSTER_SECRETS:-0}" != "1" ]]; then
  echo "▶ ensure-housing-cluster-secrets (kafka-ssl-secret + edge-service-tls aliases)"
...
echo "✅ rollout-restart-rp-full-stack complete"
