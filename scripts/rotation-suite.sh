...
if [[ $wait_failed -eq 0 ]]; then
  ...
  echo "  ℹ️  LEAF_ING=leaf in ingress-nginx, LEAF_APP=leaf in record-platform (override HOUSING_NS), SVC_TLS=service-tls, CA_*=dev-root-ca"
...
say "Warmup: 8s + one HTTP/3 health to record-platform.test (clear stale QUIC sessions)…"
sleep 8
...
log_info "Certificate verified above: issuer = dev CA (CN=dev-root-ca, O=record-platform); subject = CN=record-platform.test."
