...
if [[ -f "$SCRIPT_DIR/lib/ensure-rp-grpc-certs.sh" ]]; then
  ...
  rp_sync_grpc_certs_to_dir "${GRPC_CERTS_DIR:-/tmp/grpc-certs}" "${HOUSING_NS:-record-platform}" || true
...
export HOST="${HOST:-record-platform.test}"
...
if [[ "${USE_LB_FOR_TESTS:-0}" == "1" ]] && [[ -n "${REACHABLE_LB_IP:-}" ]]; then
  ...
  _lb_code=$(curl -k -sS -o /dev/null -w "%{http_code}" --connect-timeout 3 --max-time 5 --http2 \
    --resolve "record-platform.test:443:${REACHABLE_LB_IP}" -H "Host: record-platform.test" "https://record-platform.test/_caddy/healthz" 2>/dev/null || echo "000")
  _setup_ran=0
  ...
  if [[ "$_lb_code" != "200" ]]; then
    if [[ "${FORCE_METALLB_ONLY:-0}" == "1" ]]; then
      ...
      _np_code=$(curl -k -sS -o /dev/null -w "%{http_code}" --connect-timeout 3 --max-time 5 --http2 \
        -H "Host: record-platform.test" "https://127.0.0.1:$_caddy_np/_caddy/healthz" 2>/dev/null || echo "000")
    ...
    if [[ -f "$_ca_h3" ]]; then
      ...
      if [[ "$(uname -s)" == "Darwin" ]] && "$CURL_BIN" --help all 2>/dev/null | grep -q -- "--http3-only"; then
        _h3_out=$(NGTCP2_ENABLE_GSO=0 "$CURL_BIN" --http3-only -sS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 10 --cacert "$_ca_h3" \
          --resolve "record-platform.test:443:${REACHABLE_LB_IP}" "https://record-platform.test/_caddy/healthz" 2>/dev/null) || true
...
if [[ "$ctx" == *"k3d"* ]] && [[ "${USE_LB_FOR_TESTS:-0}" != "1" ]]; then
  ...
  _code=$(curl -k -s -o /dev/null -w "%{http_code}" --connect-timeout 3 --max-time 5 --http2 \
    -H "Host: record-platform.test" "https://127.0.0.1:30443/_caddy/healthz" 2>/dev/null || echo "000")
  if [[ "$_code" == "200" ]]; then
    export PORT=30443
    ...
    if [[ -f "$_ca" ]] && [[ -f "$SCRIPT_DIR/lib/http3.sh" ]]; then
      ...
      _h3_out=$(http3_curl --cacert "$_ca" -sS -o /dev/null -w "%{http_code}" --max-time 5 --http3-only \
        -H "Host: record-platform.test" "https://127.0.0.1:30443/_caddy/healthz" 2>/dev/null) || true
...
if [[ "${RUN_K6:-0}" == "1" ]] && command -v k6 >/dev/null 2>&1; then
  ...
  if [[ ! -f "$K6_CA_ROOT" ]] || [[ ! -s "$K6_CA_ROOT" ]]; then
    kubectl -n "${HOUSING_NS:-record-platform}" get secret dev-root-ca -o jsonpath='{.data.dev-root\.pem}' 2>/dev/null | base64 -d > "$K6_CA_ROOT" 2>/dev/null && [[ -s "$K6_CA_ROOT" ]] && info "Fetched CA from app namespace (${HOUSING_NS:-record-platform}) secret to certs/dev-root.pem" || true
  ...
  _k6_extract_to_repo_root() {
    local out="$1"
    kubectl -n "${HOUSING_NS:-record-platform}" get secret dev-root-ca -o jsonpath='{.data.dev-root\.pem}' 2>/dev/null | base64 -d > "$out" 2>/dev/null && [[ -s "$out" ]] && return 0
    ...
    if [[ "$ctx" == *"colima"* ]] && command -v colima >/dev/null 2>&1; then
      local b64
      b64=$(colima ssh -- kubectl -n "${HOUSING_NS:-record-platform}" get secret dev-root-ca -o jsonpath='{.data.dev-root\.pem}' --request-timeout=10s 2>/dev/null || true)
  ...
  PORT="${PORT:-30443}"
  # From host: use hostname + K6_RESOLVE (never raw IP — cert SAN is record-platform.test, not MetalLB IP)
  ...
  if [[ -n "$K6_CA_ABSOLUTE" ]] && [[ -s "$K6_CA_ABSOLUTE" ]]; then
    ...
    # macOS: add dev CA to keychain (replaces manual Keychain Access → Import → Always Trust). Do this so k6/curl/browser trust record-platform.test and we avoid x509.
...
if [[ $FAILED -eq 0 ]]; then
  ...
  echo "   - Cache: Redis (externalized) = port ${REDIS_PORT:-6379}, Lua"
