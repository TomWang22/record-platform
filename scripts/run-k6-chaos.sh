  ...
  template:
    spec:
      ...
      - name: k6
        ...
        env:
        - name: HOST
          value: "${HOST:-record-platform.test}"
        - name: DURATION
          value: "${DURATION:-180s}"
        - name: SSL_CERT_FILE
  ...
  local)
    ...
    if [[ "$(uname -s)" == "Darwin" ]] && [[ -f "$SCRIPT_DIR/lib/trust-dev-root-ca-macos.sh" ]]; then
      if ! "$SCRIPT_DIR/lib/trust-dev-root-ca-macos.sh" "$CA"; then
        echo "ERROR: On macOS, k6 cannot verify record-platform.test without the CA in the keychain (Go ignores SSL_CERT_FILE)." >&2
    ...
    K6_TARGET="${K6_TARGET_URL:-https://record-platform.test:30443/_caddy/healthz}"
