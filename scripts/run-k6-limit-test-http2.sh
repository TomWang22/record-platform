...
# k6 HTTP/2 Limit Test Runner with Strict TLS and Pod Count Reporting
...
echo "=== Running HTTP/2 Limit Test (increment by ${INCREMENT} VUs) ==="
echo "Starting at ${START_VUS} VUs, max ${MAX_VUS} VUs, ${DURATION} per test"
echo ""

RESULTS_FILE="/tmp/k6-http2-limit-test-$(date +%Y%m%d-%H%M%S).log"
...
for vus in $(seq "$START_VUS" "$INCREMENT" "$MAX_VUS"); do
  echo "=== Testing with $vus VUs (HTTP/2) ===" | tee -a "$RESULTS_FILE"
