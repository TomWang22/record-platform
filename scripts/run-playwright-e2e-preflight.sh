...
# No kubectl port-forward; no http://127.0.0.1:4000 — legacy E2E_API_BASE values are ignored.
...
#   E2E_API_BASE           — must be https (default https://record-platform.test)
...
_ensure_cluster_stability() {
  if [[ "${PREFLIGHT_RUN_CLUSTER_STABILITY_GUARD:-1}" == "1" ]] && [[ -x "$SCRIPT_DIR/cluster-stability-guard.sh" ]]; then
    HOUSING_NS="$HOUSING_NS" bash "$SCRIPT_DIR/cluster-stability-guard.sh"
  fi
  if [[ "${PREFLIGHT_ENSURE_METRICS_SERVER:-1}" == "1" ]] && [[ -x "$SCRIPT_DIR/ensure-metrics-server-ready.sh" ]]; then
    ...
    if ! bash "$SCRIPT_DIR/ensure-metrics-server-ready.sh"; then
  ...
  if command -v kubectl >/dev/null 2>&1; then
    ...
    kubectl wait --for=condition=ready pod -l app=api-gateway -n "$HOUSING_NS" --timeout="${_stability_timeout}" 2>/dev/null || true
    kubectl wait --for=condition=ready pod -l app=auth-service -n "$HOUSING_NS" --timeout="${_stability_timeout}" 2>/dev/null || true
    kubectl wait --for=condition=ready pod -l app=jaeger -n observability --timeout="${_stability_timeout}" 2>/dev/null || true
