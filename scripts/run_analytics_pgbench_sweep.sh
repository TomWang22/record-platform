...
#   - Postgres host (from this script): localhost:5439
#   - Database: records (or analytics, depending on setup)
#   - Docker Compose service: postgres-analytics (ports: 5439:5432)
#   - Schemas used: analytics, bench (results), public, auth
#
# Important:
#   - This script benchmarks the Docker Postgres instance for analytics service, NOT the K8s postgres pod.
#   - K8s microservices connect to the same Docker DB via:
#       host.docker.internal:5439 (db=records, search_path=analytics|...)
set -Euo pipefail
...
Examples:
  ...
  MODE=deep ./scripts/run_analytics_pgbench_sweep.sh
  ...
  TRACK_IO_TIMING=off MODE=quick ./scripts/run_analytics_pgbench_sweep.sh
USAGE
}

# Canonical analytics DB connection (override via env if needed)
ANALYTICS_DB_HOST="${ANALYTICS_DB_HOST:-localhost}"
ANALYTICS_DB_PORT="${ANALYTICS_DB_PORT:-5439}"  # Docker Compose analytics DB
ANALYTICS_DB_USER="${ANALYTICS_DB_USER:-postgres}"
ANALYTICS_DB_NAME="${ANALYTICS_DB_NAME:-records}"
ANALYTICS_DB_PASS="${ANALYTICS_DB_PASS:-postgres}"
...
LOG_DIR="$REPO_ROOT/bench_logs/analytics_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
wait_for_db_ready() {
  ...
  echo "🔍 Checking database readiness (${ANALYTICS_DB_HOST}:${ANALYTICS_DB_PORT})..."
  if PGPASSWORD="$ANALYTICS_DB_PASS" psql -h "$ANALYTICS_DB_HOST" -p "$ANALYTICS_DB_PORT" -U "$ANALYTICS_DB_USER" -d postgres -c "SELECT 1;" >/dev/null 2>&1; then
    echo "✅ Database is ready"
    return 0
  fi
  while [[ $attempt -lt $max_attempts ]]; do
    local recovery_status
    recovery_status=$(PGPASSWORD="$ANALYTICS_DB_PASS" psql -h "$ANALYTICS_DB_HOST" -p "$ANALYTICS_DB_PORT" -U "$ANALYTICS_DB_USER" -d postgres -tAc "SELECT pg_is_in_recovery();" 2>/dev/null || echo "t")
...
if [[ -z "$POD" ]]; then
  if command -v pgbench >/dev/null 2>&1; then
    ...
    echo "   Postgres is external (Docker), connecting to ${ANALYTICS_DB_HOST}:${ANALYTICS_DB_PORT}" >&2
...
psql_in_pod() {
  PGPASSWORD="$ANALYTICS_DB_PASS" psql \
    -h "$ANALYTICS_DB_HOST" -p "$ANALYTICS_DB_PORT" \
...
if command -v pgbench >/dev/null 2>&1; then
  echo "✅ Using local pgbench (connecting to ${ANALYTICS_DB_HOST}:${ANALYTICS_DB_PORT})"
  USE_LOCAL_PGBENCH=true
  POD=""
fi

tmpdir=$(mktemp -d)
trap 'cd "$REPO_ROOT" 2>/dev/null || true; rm -rf "$tmpdir"; if [[ -n "${LOG_DIR:-}" ]] && [[ -d "$LOG_DIR" ]]; then find "$REPO_ROOT" -maxdepth 1 -name "analytics_bench_sweep_*.csv" -type f -exec mv {} "$LOG_DIR/" \; 2>/dev/null || true; echo ""; echo "📁 All results and logs saved to: $LOG_DIR"; fi' EXIT
...
# Price snapshot insert benchmark (schema: record_id, source, price, currency, snapshot_date)
cat > "$bench_sql_dir/bench_price_snapshot.sql" <<'EOF'
SET search_path = analytics, public, pg_catalog;
INSERT INTO analytics.price_snapshots (record_id, source, price, currency, snapshot_date)
VALUES (gen_random_uuid(), (ARRAY['discogs','popsike','ebay','manual'])[1 + floor(random()*4)::int], (random() * 100 + 10)::numeric(10,2), 'USD', CURRENT_DATE)
RETURNING id;
EOF

# Price trend query benchmark (randomized limit) — pgbench 14+ uses \set var random(min,max)
cat > "$bench_sql_dir/bench_price_trend.sql" <<'EOF'
\set lim random(10, 50)
SET search_path = analytics, public, pg_catalog;
SELECT record_id, price, currency, snapshot_date
FROM analytics.price_snapshots
WHERE record_id IN (SELECT record_id FROM analytics.price_snapshots ORDER BY snapshot_date DESC LIMIT 1)
ORDER BY snapshot_date DESC
LIMIT :lim;
EOF

# Search analytics query benchmark (table: search_analytics with query, result_count, search_timestamp)
cat > "$bench_sql_dir/bench_search_analytics.sql" <<'EOF'
\set lim random(5, 30)
SET search_path = analytics, public, pg_catalog;
SELECT query, result_count, search_timestamp
...
# Trending searches benchmark
cat > "$bench_sql_dir/bench_trending_searches.sql" <<'EOF'
...
# Average price calculation benchmark
cat > "$bench_sql_dir/bench_avg_price.sql" <<'EOF'
\set lim random(10, 30)
SET search_path = analytics, public, pg_catalog;
SELECT record_id, AVG(price) as avg_price, MIN(price) as min_price, MAX(price) as max_price
FROM analytics.price_snapshots
...
if ! psql_in_pod -c "SELECT 1;" >/dev/null 2>&1; then
  echo "❌ Cannot connect to database at ${ANALYTICS_DB_HOST}:${ANALYTICS_DB_PORT}" >&2
  exit 1
fi
...
if [[ -n "$pgopts" ]]; then
  if [[ "$pgopts" != *"search_path"* ]]; then
    export PGOPTIONS="$pgopts -c search_path=public,analytics,pg_catalog"
  else
    export PGOPTIONS="$pgopts"
  fi
else
  export PGOPTIONS="-c search_path=public,analytics,pg_catalog"
fi
...
RUN_ID="analytics_run_${TIMESTAMP}"
echo "RUN_ID=${RUN_ID}"
results_csv="$tmpdir/analytics_bench_sweep_${TIMESTAMP}.csv"
...
run_variant() {
  ...
  if [[ "$DISABLE_AUTOVACUUM" == "true" ]]; then
    ...
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'analytics' AND table_name = 'price_snapshots') THEN
        ALTER TABLE analytics.price_snapshots SET (autovacuum_enabled = false);
      END IF;
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'analytics' AND table_name = 'search_history') THEN
        ALTER TABLE analytics.search_history SET (autovacuum_enabled = false);
      END IF;
    END $$;
SQL
  fi
  ...
  if [[ "$USE_LOCAL_PGBENCH" == "true" ]]; then
    ...
    PGHOST="$ANALYTICS_DB_HOST" PGPORT="$ANALYTICS_DB_PORT" PGUSER="$ANALYTICS_DB_USER" PGDATABASE="$ANALYTICS_DB_NAME" PGPASSWORD="$ANALYTICS_DB_PASS" \
    PGOPTIONS="$PGOPTIONS_EXTRA -c search_path=public,analytics,pg_catalog" \
    pgbench \
      -n -M prepared \
  ...
  if [[ "$DISABLE_AUTOVACUUM" == "true" ]]; then
    ...
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'analytics' AND table_name = 'price_snapshots') THEN
        ALTER TABLE analytics.price_snapshots SET (autovacuum_enabled = true);
      END IF;
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'analytics' AND table_name = 'search_history') THEN
        ALTER TABLE analytics.search_history SET (autovacuum_enabled = true);
      END IF;
    END $$;
SQL
  fi
...
cold_cache_reset() {
  ...
  local pg_container
  pg_container=$(docker ps --filter "name=postgres" --filter "publish=5439" --format "{{.Names}}" | head -1)
...
SQL
  ...
  if [[ "${REAL_COLD_CACHE:-0}" == "1" ]]; then
    ...
    psql_in_pod -tA -c "SELECT count(*) FROM analytics.events;" >/dev/null 2>&1 || true
    sleep 1
  fi
  sleep 2
}
...
# Variants to test
declare -a variants=("price_snapshot" "price_trend" "search_analytics" "trending_searches" "avg_price" "noop")
...
run_phase_block() {
  ...
  for variant in "${variants[@]}"; do
    ...
    case "$variant" in
      price_snapshot) sql_file="bench_price_snapshot.sql" ;;
      price_trend) sql_file="bench_price_trend.sql" ;;
      search_analytics) sql_file="bench_search_analytics.sql" ;;
      trending_searches) sql_file="bench_trending_searches.sql" ;;
      avg_price) sql_file="bench_avg_price.sql" ;;
...
cp -f "$results_csv" "$output_dir/analytics_bench_sweep_${TIMESTAMP}.csv" 2>/dev/null || {
  echo "⚠️  Failed to copy CSV to $output_dir/analytics_bench_sweep_${TIMESTAMP}.csv" >&2
}
cp -f "$results_csv" "$output_dir/analytics_bench_sweep.csv" 2>/dev/null || {
  echo "⚠️  Failed to copy CSV to $output_dir/analytics_bench_sweep.csv" >&2
}

echo "✅ Wrote $output_dir/analytics_bench_sweep_${TIMESTAMP}.csv"
echo "✅ Wrote $output_dir/analytics_bench_sweep.csv"
echo ""
...
if [[ "$RUN_DIFF_MODE" == "true" && -n "$BASELINE_CSV" && -f "$BASELINE_CSV" ]] && command -v python3 >/dev/null 2>&1; then
  ...
  current_path="$output_dir/analytics_bench_sweep_${TIMESTAMP}.csv"
  python3 <<PY
import pandas as pd
...
for variant in price_snapshot price_trend search_analytics trending_searches avg_price noop; do
