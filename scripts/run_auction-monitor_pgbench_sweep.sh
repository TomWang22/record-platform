...
#   - Postgres host (from this script): localhost:5438
#   - Database: records (or auction_monitor, depending on setup)
#   - Docker Compose service: postgres-auction_monitor (ports: 5438:5432)
#   - Schemas used: auction_monitor, bench (results), public, auth
#
# Important:
#   - This script benchmarks the Docker Postgres instance for auction_monitor service, NOT the K8s postgres pod.
#   - K8s microservices connect to the same Docker DB via:
#       host.docker.internal:5438 (db=records, search_path=auction_monitor|...)
set -Euo pipefail
...
Examples:
  ...
  MODE=deep ./scripts/run_auction_monitor_pgbench_sweep.sh
  ...
  TRACK_IO_TIMING=off MODE=quick ./scripts/run_auction_monitor_pgbench_sweep.sh
USAGE
}

# Canonical auction_monitor DB connection (override via env if needed)
AUCTION_MONITOR_DB_HOST="${AUCTION_MONITOR_DB_HOST:-localhost}"
AUCTION_MONITOR_DB_PORT="${AUCTION_MONITOR_DB_PORT:-5438}"  # Docker Compose auction_monitor DB
AUCTION_MONITOR_DB_USER="${AUCTION_MONITOR_DB_USER:-postgres}"
AUCTION_MONITOR_DB_NAME="${AUCTION_MONITOR_DB_NAME:-records}"
AUCTION_MONITOR_DB_PASS="${AUCTION_MONITOR_DB_PASS:-postgres}"
...
LOG_DIR="$REPO_ROOT/bench_logs/auction_monitor_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
wait_for_db_ready() {
  ...
  echo "🔍 Checking database readiness (${AUCTION_MONITOR_DB_HOST}:${AUCTION_MONITOR_DB_PORT})..."
  if PGPASSWORD="$AUCTION_MONITOR_DB_PASS" psql -h "$AUCTION_MONITOR_DB_HOST" -p "$AUCTION_MONITOR_DB_PORT" -U "$AUCTION_MONITOR_DB_USER" -d postgres -c "SELECT 1;" >/dev/null 2>&1; then
    echo "✅ Database is ready"
    return 0
  fi
  while [[ $attempt -lt $max_attempts ]]; do
    local recovery_status
    recovery_status=$(PGPASSWORD="$AUCTION_MONITOR_DB_PASS" psql -h "$AUCTION_MONITOR_DB_HOST" -p "$AUCTION_MONITOR_DB_PORT" -U "$AUCTION_MONITOR_DB_USER" -d postgres -tAc "SELECT pg_is_in_recovery();" 2>/dev/null || echo "t")
...
if [[ -z "$POD" ]]; then
  if command -v pgbench >/dev/null 2>&1; then
    ...
    echo "   Postgres is external (Docker), connecting to ${AUCTION_MONITOR_DB_HOST}:${AUCTION_MONITOR_DB_PORT}" >&2
...
psql_in_pod() {
  PGPASSWORD="$AUCTION_MONITOR_DB_PASS" psql \
    -h "$AUCTION_MONITOR_DB_HOST" -p "$AUCTION_MONITOR_DB_PORT" \
    -U "$AUCTION_MONITOR_DB_USER" -d "$AUCTION_MONITOR_DB_NAME" \
    -X -P pager=off "$@"
}

# Force local pgbench when pgbench is available locally
if command -v pgbench >/dev/null 2>&1; then
  echo "✅ Using local pgbench (connecting to ${AUCTION_MONITOR_DB_HOST}:${AUCTION_MONITOR_DB_PORT})"
  USE_LOCAL_PGBENCH=true
  POD=""
fi

tmpdir=$(mktemp -d)
trap 'cd "$REPO_ROOT" 2>/dev/null || true; rm -rf "$tmpdir"; if [[ -n "${LOG_DIR:-}" ]] && [[ -d "$LOG_DIR" ]]; then find "$REPO_ROOT" -maxdepth 1 -name "auction_monitor_bench_sweep_*.csv" -type f -exec mv {} "$LOG_DIR/" \; 2>/dev/null || true; echo ""; echo "📁 All results and logs saved to: $LOG_DIR"; fi' EXIT

# Create bench schema
cat <<'SQL' > "$tmpdir/create_bench_schema.sql"
SET search_path = public, auction_monitor;
...
# Auction results insert benchmark (schema: total_cost NOT NULL — use price as total_cost when no shipping)
cat > "$bench_sql_dir/bench_monitor_insert.sql" <<'EOF'
SET search_path = auction_monitor, public, pg_catalog;
INSERT INTO auction_monitor.auction_results (source, external_id, title, artist, price, currency, total_cost, sold_at)
SELECT (ARRAY['discogs','popsike','gripseeat','ebay'])[1 + floor(random()*4)::int], 'ext-' || gen_random_uuid()::text, 'Bench Title', 'Bench Artist', p, 'USD', p, now()
FROM (SELECT (random() * 100 + 10)::numeric(10,2) AS p) x
RETURNING id;
EOF

# Auction status query benchmark (randomized limit) — pgbench 14+ uses \set var random(min,max)
cat > "$bench_sql_dir/bench_monitor_status.sql" <<'EOF'
\set lim random(10, 30)
SET search_path = auction_monitor, public, pg_catalog;
SELECT id, source, title, price, sold_at, created_at
FROM auction_monitor.auction_results
WHERE id IN (SELECT id FROM auction_monitor.auction_results ORDER BY created_at DESC LIMIT 1)
ORDER BY created_at DESC
LIMIT :lim;
EOF

# Active auctions query benchmark (randomized limit) — pgbench 14+ uses \set var random(min,max)
cat > "$bench_sql_dir/bench_active_auctions.sql" <<'EOF'
\set lim random(10, 25)
SET search_path = auction_monitor, public, pg_catalog;
SELECT record_id, COUNT(*) as result_count, MAX(created_at) as last_created
FROM auction_monitor.auction_results
...
# Event aggregation benchmark (by source)
cat > "$bench_sql_dir/bench_event_aggregate.sql" <<'EOF'
SET search_path = auction_monitor, public, pg_catalog;
SELECT source, COUNT(*) as count, MIN(created_at) as first_created, MAX(created_at) as last_created
FROM auction_monitor.auction_results
...
if ! psql_in_pod -c "SELECT 1;" >/dev/null 2>&1; then
  echo "❌ Cannot connect to database at ${AUCTION_MONITOR_DB_HOST}:${AUCTION_MONITOR_DB_PORT}" >&2
  exit 1
fi
...
if [[ -n "$pgopts" ]]; then
  if [[ "$pgopts" != *"search_path"* ]]; then
    export PGOPTIONS="$pgopts -c search_path=public,auction_monitor,pg_catalog"
  else
    export PGOPTIONS="$pgopts"
  fi
else
  export PGOPTIONS="-c search_path=public,auction_monitor,pg_catalog"
fi
...
RUN_ID="auction_monitor_run_${TIMESTAMP}"
echo "RUN_ID=${RUN_ID}"
results_csv="$tmpdir/auction_monitor_bench_sweep_${TIMESTAMP}.csv"
...
run_variant() {
  ...
  if [[ "$DISABLE_AUTOVACUUM" == "true" ]]; then
    ...
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auction_monitor' AND table_name = 'auction_results') THEN
        ALTER TABLE auction_monitor.auction_results SET (autovacuum_enabled = false);
      END IF;
    END $$;
SQL
  fi
  ...
  if [[ "$USE_LOCAL_PGBENCH" == "true" ]]; then
    ...
    PGHOST="$AUCTION_MONITOR_DB_HOST" PGPORT="$AUCTION_MONITOR_DB_PORT" PGUSER="$AUCTION_MONITOR_DB_USER" PGDATABASE="$AUCTION_MONITOR_DB_NAME" PGPASSWORD="$AUCTION_MONITOR_DB_PASS" \
    PGOPTIONS="$PGOPTIONS_EXTRA -c search_path=public,auction_monitor,pg_catalog" \
    pgbench \
      -n -M prepared \
  ...
  if [[ "$DISABLE_AUTOVACUUM" == "true" ]]; then
    ...
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auction_monitor' AND table_name = 'auction_results') THEN
        ALTER TABLE auction_monitor.auction_results SET (autovacuum_enabled = true);
      END IF;
    END $$;
SQL
  fi
...
cold_cache_reset() {
  ...
  local pg_container
  pg_container=$(docker ps --filter "name=postgres" --filter "publish=5438" --format "{{.Names}}" | head -1)
...
SQL
  ...
  if [[ "${REAL_COLD_CACHE:-0}" == "1" ]]; then
    ...
    psql_in_pod -tA -c "SELECT count(*) FROM auction_monitor.auction_results;" >/dev/null 2>&1 || true
    sleep 1
  fi
  sleep 2
}
...
# Variants to test
declare -a variants=("monitor_insert" "monitor_status" "active_auctions" "event_aggregate" "noop")
...
run_phase_block() {
  ...
  for variant in "${variants[@]}"; do
    ...
    case "$variant" in
      monitor_insert) sql_file="bench_monitor_insert.sql" ;;
      monitor_status) sql_file="bench_monitor_status.sql" ;;
      active_auctions) sql_file="bench_active_auctions.sql" ;;
      event_aggregate) sql_file="bench_event_aggregate.sql" ;;
...
cp -f "$results_csv" "$output_dir/auction_monitor_bench_sweep_${TIMESTAMP}.csv" 2>/dev/null || {
  echo "⚠️  Failed to copy CSV to $output_dir/auction_monitor_bench_sweep_${TIMESTAMP}.csv" >&2
}
cp -f "$results_csv" "$output_dir/auction_monitor_bench_sweep.csv" 2>/dev/null || {
  echo "⚠️  Failed to copy CSV to $output_dir/auction_monitor_bench_sweep.csv" >&2
}

echo "✅ Wrote $output_dir/auction_monitor_bench_sweep_${TIMESTAMP}.csv"
echo "✅ Wrote $output_dir/auction_monitor_bench_sweep.csv"
echo ""
...
if [[ "$RUN_DIFF_MODE" == "true" && -n "$BASELINE_CSV" && -f "$BASELINE_CSV" ]] && command -v python3 >/dev/null 2>&1; then
  ...
  current_path="$output_dir/auction_monitor_bench_sweep_${TIMESTAMP}.csv"
  python3 <<PY
import pandas as pd
...
for variant in monitor_insert monitor_status active_auctions event_aggregate noop; do
