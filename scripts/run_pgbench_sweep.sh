...
THREADS=12
LIMIT=50
PGOPTIONS_EXTRA="-c jit=off -c enable_seqscan=off -c random_page_cost=1.0 -c cpu_index_tuple_cost=0.0005 -c cpu_tuple_cost=0.01 -c effective_cache_size=4GB -c work_mem=256MB -c track_io_timing=on -c max_parallel_workers=12 -c max_parallel_workers_per_gather=4"
...
SELECT count(*) FROM (
  SELECT id
  FROM public.search_records_fuzzy_ids_core(
    :uid::uuid,
    :q::text,
    LEAST(2000, GREATEST(1, (:lim::integer * 20)::bigint)),
    0::bigint
  )
  ORDER BY rank DESC
  LIMIT :lim::integer
...
run_variant() {
  ...
  kubectl -n "$NS" exec "$POD" -c db -- psql -U postgres -d records -v ON_ERROR_STOP=1 \
