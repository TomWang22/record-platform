...
#   - Postgres host (from this script): localhost:5440
#   - Database: records (or python_ai, depending on setup)
#   - Docker Compose service: postgres-python_ai (ports: 5440:5432)
#   - Schemas used: python_ai, bench (results), public, auth
#
# Important:
#   - This script benchmarks the Docker Postgres instance for python_ai service, NOT the K8s postgres pod.
#   - K8s microservices connect to the same Docker DB via:
#       host.docker.internal:5440 (db=records, search_path=ai|...)
set -Euo pipefail
...
Examples:
  ...
  MODE=deep ./scripts/run_python_ai_pgbench_sweep.sh
  ...
  TRACK_IO_TIMING=off MODE=quick ./scripts/run_python_ai_pgbench_sweep.sh
USAGE
}

# Canonical python_ai DB connection (override via env if needed)
PYTHON_AI_DB_HOST="${PYTHON_AI_DB_HOST:-localhost}"
PYTHON_AI_DB_PORT="${PYTHON_AI_DB_PORT:-5440}"  # Docker Compose python_ai DB
PYTHON_AI_DB_USER="${PYTHON_AI_DB_USER:-postgres}"
PYTHON_AI_DB_NAME="${PYTHON_AI_DB_NAME:-records}"
PYTHON_AI_DB_PASS="${PYTHON_AI_DB_PASS:-postgres}"
...
LOG_DIR="$REPO_ROOT/bench_logs/python_ai_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
wait_for_db_ready() {
  ...
  echo "🔍 Checking database readiness (${PYTHON_AI_DB_HOST}:${PYTHON_AI_DB_PORT})..."
  if PGPASSWORD="$PYTHON_AI_DB_PASS" psql -h "$PYTHON_AI_DB_HOST" -p "$PYTHON_AI_DB_PORT" -U "$PYTHON_AI_DB_USER" -d postgres -c "SELECT 1;" >/dev/null 2>&1; then
    echo "✅ Database is ready"
    return 0
  fi
  while [[ $attempt -lt $max_attempts ]]; do
    local recovery_status
    recovery_status=$(PGPASSWORD="$PYTHON_AI_DB_PASS" psql -h "$PYTHON_AI_DB_HOST" -p "$PYTHON_AI_DB_PORT" -U "$PYTHON_AI_DB_USER" -d postgres -tAc "SELECT pg_is_in_recovery();" 2>/dev/null || echo "t")
...
if [[ -z "$POD" ]]; then
  if command -v pgbench >/dev/null 2>&1; then
    ...
    echo "   Postgres is external (Docker), connecting to ${PYTHON_AI_DB_HOST}:${PYTHON_AI_DB_PORT}" >&2
...
psql_in_pod() {
  PGPASSWORD="$PYTHON_AI_DB_PASS" psql \
    -h "$PYTHON_AI_DB_HOST" -p "$PYTHON_AI_DB_PORT" \
    -U "$PYTHON_AI_DB_USER" -d "$PYTHON_AI_DB_NAME" \
    -X -P pager=off "$@"
}

# Force local pgbench when pgbench is available locally
if command -v pgbench >/dev/null 2>&1; then
  echo "✅ Using local pgbench (connecting to ${PYTHON_AI_DB_HOST}:${PYTHON_AI_DB_PORT})"
  USE_LOCAL_PGBENCH=true
  POD=""
fi

tmpdir=$(mktemp -d)
trap 'cd "$REPO_ROOT" 2>/dev/null || true; rm -rf "$tmpdir"; if [[ -n "${LOG_DIR:-}" ]] && [[ -d "$LOG_DIR" ]]; then find "$REPO_ROOT" -maxdepth 1 -name "python_ai_bench_sweep_*.csv" -type f -exec mv {} "$LOG_DIR/" \; 2>/dev/null || true; echo ""; echo "📁 All results and logs saved to: $LOG_DIR"; fi' EXIT

# Ensure ai.inference_log exists (benchmarks require it). Full schema first; if that fails (e.g. extensions), bootstrap minimal table.
_ensure_ai_inference_log() {
  if psql_in_pod -tAc "SELECT 1 FROM information_schema.tables WHERE table_schema = 'ai' AND table_name = 'inference_log';" 2>/dev/null | grep -q 1; then
    return 0
  fi
  echo "Applying python-ai schema (ai.inference_log missing on ${PYTHON_AI_DB_NAME})..."
  if [[ -f "$REPO_ROOT/infra/db/python-ai-schema.sql" ]]; then
    if PGPASSWORD="$PYTHON_AI_DB_PASS" psql -h "$PYTHON_AI_DB_HOST" -p "$PYTHON_AI_DB_PORT" -U "$PYTHON_AI_DB_USER" -d "$PYTHON_AI_DB_NAME" -v ON_ERROR_STOP=0 -f "$REPO_ROOT/infra/db/python-ai-schema.sql" >/dev/null 2>&1; then
      :
    fi
  fi
  if ! psql_in_pod -tAc "SELECT 1 FROM information_schema.tables WHERE table_schema = 'ai' AND table_name = 'inference_log';" 2>/dev/null | grep -q 1; then
    echo "  Bootstrapping minimal ai.inference_log (no extensions)..."
    psql_in_pod -v ON_ERROR_STOP=1 <<'BOOTSTRAP' >/dev/null 2>&1 || true
...
CREATE TABLE IF NOT EXISTS ai.inference_log (
  ...
  analytics_data_used BOOLEAN DEFAULT FALSE,
...
BOOTSTRAP
  ...
  if psql_in_pod -tAc "SELECT 1 FROM information_schema.tables WHERE table_schema = 'ai' AND table_name = 'inference_log';" 2>/dev/null | grep -q 1; then
    echo "✅ ai.inference_log ready"
  else
    echo "⚠️  ai.inference_log still missing; ai benchmarks will fail"
  fi
}
_ensure_ai_inference_log
...
# AI inference log insert benchmark (schema ai.inference_log: query, inference_type, input_data, output_data, created_at)
cat > "$bench_sql_dir/bench_ai_insert.sql" <<'EOF'
SET search_path = ai, public, pg_catalog;
INSERT INTO ai.inference_log (query, inference_type, input_data, output_data, created_at)
VALUES ('bench-' || gen_random_uuid()::text, (ARRAY['selling','buying','negotiation','bidding'])[1 + floor(random()*4)::int], jsonb_build_object('data', random()::text), jsonb_build_object('result', random()), now())
RETURNING id;
EOF

# AI inference query benchmark (randomized limit) — pgbench 14+ uses \set var random(min,max) not \setrandom
cat > "$bench_sql_dir/bench_ai_query.sql" <<'EOF'
\set lim random(5, 40)
SET search_path = ai, public, pg_catalog;
SELECT id, query, inference_type, created_at
...
cat > "$bench_sql_dir/bench_ai_performance.sql" <<'EOF'
SET search_path = ai, public, pg_catalog;
SELECT inference_type, COUNT(*) as inference_count, MIN(created_at) as first_inference, MAX(created_at) as last_inference
FROM ai.inference_log
...
# AI data aggregation benchmark (randomized limit) — pgbench 14+ uses \set var random(min,max) not \setrandom
cat > "$bench_sql_dir/bench_ai_aggregate.sql" <<'EOF'
\set lim random(12, 48)
SET search_path = ai, public, pg_catalog;
...
if ! psql_in_pod -c "SELECT 1;" >/dev/null 2>&1; then
  echo "❌ Cannot connect to database at ${PYTHON_AI_DB_HOST}:${PYTHON_AI_DB_PORT}" >&2
  exit 1
fi
...
if [[ -n "$pgopts" ]]; then
  if [[ "$pgopts" != *"search_path"* ]]; then
    export PGOPTIONS="$pgopts -c search_path=public,ai,pg_catalog"
  else
    export PGOPTIONS="$pgopts"
  fi
else
  export PGOPTIONS="-c search_path=public,ai,pg_catalog"
fi
...
RUN_ID="python_ai_run_${TIMESTAMP}"
echo "RUN_ID=${RUN_ID}"
results_csv="$tmpdir/python_ai_bench_sweep_${TIMESTAMP}.csv"
...
run_variant() {
  ...
  if [[ "$DISABLE_AUTOVACUUM" == "true" ]]; then
    ...
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'ai' AND table_name = 'inference_log') THEN
        ALTER TABLE ai.inference_log SET (autovacuum_enabled = false);
      END IF;
    END $$;
SQL
  fi
  ...
  if [[ "$USE_LOCAL_PGBENCH" == "true" ]]; then
    ...
    PGHOST="$PYTHON_AI_DB_HOST" PGPORT="$PYTHON_AI_DB_PORT" PGUSER="$PYTHON_AI_DB_USER" PGDATABASE="$PYTHON_AI_DB_NAME" PGPASSWORD="$PYTHON_AI_DB_PASS" \
    PGOPTIONS="$PGOPTIONS_EXTRA -c search_path=public,ai,pg_catalog" \
    pgbench \
      -n -M prepared \
  ...
  if [[ "$DISABLE_AUTOVACUUM" == "true" ]]; then
    ...
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'ai' AND table_name = 'inference_log') THEN
        ALTER TABLE ai.inference_log SET (autovacuum_enabled = true);
      END IF;
    END $$;
SQL
  fi
...
cold_cache_reset() {
  ...
  # Optional: real cold via Postgres restart (COLD_POSTGRES_RESTART=1). Evicts shared_buffers.
  if [[ "${COLD_POSTGRES_RESTART:-0}" == "1" ]]; then
    local pg_container
    pg_container=$(docker ps --filter "name=postgres" --filter "publish=5440" --format "{{.Names}}" | head -1)
    if [[ -n "$pg_container" ]]; then
      echo "   Restarting Postgres container ($pg_container) for real cold..."
      docker restart "$pg_container" >/dev/null 2>&1 || true
      local wait_attempt=0
      while ! PGPASSWORD="$PYTHON_AI_DB_PASS" psql -h "$PYTHON_AI_DB_HOST" -p "$PYTHON_AI_DB_PORT" -U "$PYTHON_AI_DB_USER" -d postgres -c "SELECT 1;" >/dev/null 2>&1; do
        sleep 2
        wait_attempt=$((wait_attempt + 1))
        [[ $wait_attempt -gt 30 ]] && { echo "⚠️  Postgres did not become ready after restart" >&2; break; }
      done
...
SQL
  ...
  # Optional: evict working set for real cold (REAL_COLD_CACHE=1; true cold needs Postgres restart or drop_caches)
  if [[ "${REAL_COLD_CACHE:-0}" == "1" ]]; then
    echo "   Evicting working set (REAL_COLD_CACHE=1)..."
    psql_in_pod -tA -c "SELECT count(*) FROM ai.inference_log;" >/dev/null 2>&1 || true
    sleep 1
  fi
  sleep 2
}
...
# Variants to test
declare -a variants=("ai_insert" "ai_query" "ai_performance" "ai_aggregate" "noop")
...
run_phase_block() {
  ...
  for variant in "${variants[@]}"; do
    ...
    case "$variant" in
      ai_insert) sql_file="bench_ai_insert.sql" ;;
      ai_query) sql_file="bench_ai_query.sql" ;;
      ai_performance) sql_file="bench_ai_performance.sql" ;;
      ai_aggregate) sql_file="bench_ai_aggregate.sql" ;;
...
cp -f "$results_csv" "$output_dir/python_ai_bench_sweep_${TIMESTAMP}.csv" 2>/dev/null || {
  echo "⚠️  Failed to copy CSV to $output_dir/python_ai_bench_sweep_${TIMESTAMP}.csv" >&2
}
cp -f "$results_csv" "$output_dir/python_ai_bench_sweep.csv" 2>/dev/null || {
  echo "⚠️  Failed to copy CSV to $output_dir/python_ai_bench_sweep.csv" >&2
}

echo "✅ Wrote $output_dir/python_ai_bench_sweep_${TIMESTAMP}.csv"
echo "✅ Wrote $output_dir/python_ai_bench_sweep.csv"
echo ""
...
if [[ "$RUN_DIFF_MODE" == "true" && -n "$BASELINE_CSV" && -f "$BASELINE_CSV" ]] && command -v python3 >/dev/null 2>&1; then
  ...
  current_path="$output_dir/python_ai_bench_sweep_${TIMESTAMP}.csv"
  python3 <<PY
import pandas as pd
...
for variant in ai_insert ai_query ai_performance ai_aggregate noop; do
