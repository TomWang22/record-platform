...
#   - Postgres host (from this script): localhost:5434
#   - Database: records (or social, depending on setup)
#   - Docker Compose service: postgres-social (ports: 5434:5432)
...
Examples:
  ...
  TRACK_IO_TIMING=off MODE=quick ./scripts/run_social_pgbench_sweep.sh
USAGE
}

# Canonical social DB connection (override via env if needed)
SOCIAL_DB_HOST="${SOCIAL_DB_HOST:-localhost}"
SOCIAL_DB_PORT="${SOCIAL_DB_PORT:-5434}"  # Docker Compose social DB
SOCIAL_DB_USER="${SOCIAL_DB_USER:-postgres}"
SOCIAL_DB_NAME="${SOCIAL_DB_NAME:-records}"
SOCIAL_DB_PASS="${SOCIAL_DB_PASS:-postgres}"
...
# Regression and diff (align with run_pgbench_sweep.sh; see scripts/PGBENCH_HARDENING.md)
...
# Cold-first: run cold phase then warm (cold testing is key across the whole layer). Default: warm then cold.
COLD_FIRST="${COLD_FIRST:-0}"
# Planner stability and timeouts (prevent planner lag/lying; see PGBENCH_HARDENING.md)
...
LOG_DIR="$REPO_ROOT/bench_logs/social_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
wait_for_db_ready() {
  ...
  echo "🔍 Checking database readiness (${SOCIAL_DB_HOST}:${SOCIAL_DB_PORT})..."
  if PGPASSWORD="$SOCIAL_DB_PASS" psql -h "$SOCIAL_DB_HOST" -p "$SOCIAL_DB_PORT" -U "$SOCIAL_DB_USER" -d postgres -c "SELECT 1;" >/dev/null 2>&1; then
    echo "✅ Database is ready"
    return 0
  fi
  while [[ $attempt -lt $max_attempts ]]; do
    local recovery_status
    recovery_status=$(PGPASSWORD="$SOCIAL_DB_PASS" psql -h "$SOCIAL_DB_HOST" -p "$SOCIAL_DB_PORT" -U "$SOCIAL_DB_USER" -d postgres -tAc "SELECT pg_is_in_recovery();" 2>/dev/null || echo "t")
...
if [[ -z "$POD" ]]; then
  if command -v pgbench >/dev/null 2>&1; then
    ...
    echo "   Postgres is external (Docker), connecting to ${SOCIAL_DB_HOST}:${SOCIAL_DB_PORT}" >&2
...
psql_in_pod() {
  PGPASSWORD="$SOCIAL_DB_PASS" psql \
...
if command -v pgbench >/dev/null 2>&1; then
  echo "✅ Using local pgbench (connecting to ${SOCIAL_DB_HOST}:${SOCIAL_DB_PORT})"
  USE_LOCAL_PGBENCH=true
  POD=""
fi

tmpdir=$(mktemp -d)
trap 'cd "$REPO_ROOT" 2>/dev/null || true; rm -rf "$tmpdir"; if [[ -n "${LOG_DIR:-}" ]] && [[ -d "$LOG_DIR" ]]; then find "$REPO_ROOT" -maxdepth 1 -name "social_bench_sweep_*.csv" -type f -exec mv {} "$LOG_DIR/" \; 2>/dev/null || true; echo ""; echo "📁 All results and logs saved to: $LOG_DIR"; fi' EXIT

# Create bench schema (include bench in search_path so INSERT INTO bench.results and ALTER see bench)
cat <<'SQL' > "$tmpdir/create_bench_schema.sql"
SET search_path = public, forum, messages, bench;
...
# Forum post creation benchmark
cat > "$bench_sql_dir/bench_forum_post.sql" <<'EOF'
SET search_path = forum, public, pg_catalog;
INSERT INTO forum.posts (user_id, title, content, flair)
VALUES (:uid::uuid, 'Benchmark Post ' || random()::text, 'This is a benchmark post content ' || random()::text, 'Discussion')
RETURNING id;
EOF

# Forum post listing benchmark
cat > "$bench_sql_dir/bench_forum_list.sql" <<'EOF'
SET search_path = forum, public, pg_catalog;
...
# Forum comment creation benchmark
cat > "$bench_sql_dir/bench_forum_comment.sql" <<'EOF'
SET search_path = forum, public, pg_catalog;
INSERT INTO forum.comments (post_id, user_id, content)
SELECT id, :uid::uuid, 'Benchmark comment ' || random()::text
FROM forum.posts
ORDER BY created_at DESC
LIMIT 1;
EOF

# Message creation benchmark (direct)
cat > "$bench_sql_dir/bench_message_direct.sql" <<'EOF'
SET search_path = messages, public, pg_catalog;
INSERT INTO messages.messages (sender_id, recipient_id, message_type, subject, content)
VALUES (:uid::uuid, gen_random_uuid(), 'General', 'Benchmark Message', 'This is a benchmark message ' || random()::text)
RETURNING id;
EOF

# Message creation benchmark (group)
cat > "$bench_sql_dir/bench_message_group.sql" <<'EOF'
SET search_path = messages, public, pg_catalog;
INSERT INTO messages.messages (sender_id, group_id, message_type, subject, content)
SELECT :uid::uuid, g.id, 'General', 'Benchmark Group Message', 'This is a benchmark group message ' || random()::text
FROM messages.groups g
INNER JOIN messages.group_members gm ON g.id = gm.group_id
WHERE gm.user_id = :uid::uuid
LIMIT 1
RETURNING id;
EOF

# Message listing benchmark
cat > "$bench_sql_dir/bench_message_list.sql" <<'EOF'
SET search_path = messages, public, pg_catalog;
SELECT m.id, m.sender_id, m.recipient_id, m.group_id, m.subject, m.content, m.is_read, m.created_at
FROM messages.messages m
WHERE m.recipient_id = :uid::uuid OR m.group_id IN (
  SELECT group_id FROM messages.group_members WHERE user_id = :uid::uuid
)
ORDER BY m.created_at DESC
LIMIT 20;
EOF

# Group chat listing benchmark
cat > "$bench_sql_dir/bench_group_list.sql" <<'EOF'
SET search_path = messages, public, pg_catalog;
...
if ! psql_in_pod -c "SELECT 1;" >/dev/null 2>&1; then
  echo "❌ Cannot connect to database at ${SOCIAL_DB_HOST}:${SOCIAL_DB_PORT}" >&2
  exit 1
fi
...
# Per-schema coverage: ensure both forum and messages schemas are reachable (pgbench runs both via variants)
echo "=== Per-schema coverage (forum, messages) ==="
forum_count=$(psql_in_pod -tAc "SELECT count(*) FROM forum.posts;" 2>/dev/null || echo "?")
messages_count=$(psql_in_pod -tAc "SELECT count(*) FROM messages.messages;" 2>/dev/null || echo "?")
echo "  forum.posts: $forum_count rows"
echo "  messages.messages: $messages_count rows"
echo "✅ Both schemas reachable (benchmark variants: forum_post/forum_list/forum_comment, message_direct/message_group/message_list, group_list)"
...
if [[ -n "$pgopts" ]]; then
  if [[ "$pgopts" != *"search_path"* ]]; then
    export PGOPTIONS="$pgopts -c search_path=public,forum,messages,pg_catalog"
  else
    export PGOPTIONS="$pgopts"
  fi
else
  export PGOPTIONS="-c search_path=public,forum,messages,pg_catalog"
fi
...
RUN_ID="social_run_${TIMESTAMP}"
echo "RUN_ID=${RUN_ID}"
results_csv="$tmpdir/social_bench_sweep_${TIMESTAMP}.csv"
...
ALTER TABLE forum.posts SET (autovacuum_enabled = false);
ALTER TABLE forum.comments SET (autovacuum_enabled = false);
ALTER TABLE messages.messages SET (autovacuum_enabled = false);
SQL
  fi
  ...
  if [[ "$USE_LOCAL_PGBENCH" == "true" ]]; then
    ...
    PGHOST="$SOCIAL_DB_HOST" PGPORT="$SOCIAL_DB_PORT" PGUSER="$SOCIAL_DB_USER" PGDATABASE="$SOCIAL_DB_NAME" PGPASSWORD="$SOCIAL_DB_PASS" \
    PGOPTIONS="$PGOPTIONS_EXTRA -c search_path=public,forum,messages,pg_catalog" \
    pgbench \
      -n -M prepared \
  ...
  if [[ "$DISABLE_AUTOVACUUM" == "true" ]]; then
    psql_in_pod <<'SQL' >/dev/null 2>&1 || true
ALTER TABLE forum.posts SET (autovacuum_enabled = true);
ALTER TABLE forum.comments SET (autovacuum_enabled = true);
ALTER TABLE messages.messages SET (autovacuum_enabled = true);
SQL
  fi
  ...
  psql_in_pod -v ON_ERROR_STOP=1 \
    ...
    -f - <<'EOSQL'
      SET search_path = bench, public, forum, messages;
...
cold_cache_reset() {
  ...
  local pg_container
  pg_container=$(docker ps --filter "name=postgres" --filter "publish=5434" --format "{{.Names}}" | head -1)
...
SQL
  ...
  if [[ "${REAL_COLD_CACHE:-0}" == "1" ]]; then
    ...
    psql_in_pod -tA -c "SELECT count(*) FROM forum.topics;" >/dev/null 2>&1 || true
    sleep 1
  fi
  sleep 2
}
...
# Variants to test
declare -a variants=("forum_post" "forum_list" "forum_comment" "message_direct" "message_group" "message_list" "group_list" "noop")
...
run_phase_block() {
  ...
  for variant in "${variants[@]}"; do
    ...
    case "$variant" in
      forum_post) sql_file="bench_forum_post.sql" ;;
      forum_list) sql_file="bench_forum_list.sql" ;;
      forum_comment) sql_file="bench_forum_comment.sql" ;;
      message_direct) sql_file="bench_message_direct.sql" ;;
      message_group) sql_file="bench_message_group.sql" ;;
      message_list) sql_file="bench_message_list.sql" ;;
      group_list) sql_file="bench_group_list.sql" ;;
...
cp -f "$results_csv" "$output_dir/social_bench_sweep_${TIMESTAMP}.csv" 2>/dev/null || {
  echo "⚠️  Failed to copy CSV to $output_dir/social_bench_sweep_${TIMESTAMP}.csv" >&2
}
cp -f "$results_csv" "$output_dir/social_bench_sweep.csv" 2>/dev/null || {
  echo "⚠️  Failed to copy CSV to $output_dir/social_bench_sweep.csv" >&2
}

echo "✅ Wrote $output_dir/social_bench_sweep_${TIMESTAMP}.csv"
echo "✅ Wrote $output_dir/social_bench_sweep.csv"
echo ""
...
if [[ "$RUN_DIFF_MODE" == "true" && -n "$BASELINE_CSV" && -f "$BASELINE_CSV" ]] && command -v python3 >/dev/null 2>&1; then
  ...
  current_path="$output_dir/social_bench_sweep_${TIMESTAMP}.csv"
  python3 <<PY
import pandas as pd
...
for variant in forum_post forum_list forum_comment message_direct message_group message_list group_list noop; do
...
echo "  Regression: RUN_DIFF_MODE=true BASELINE_CSV=<path> (REG_THRESH_TPS_DROP, REG_THRESH_P95_INCREASE)"
echo "  EXPLAIN ANALYZE: RUN_PLAN_DUMP=true (log to LOG_DIR); re-run after ANALYZE to avoid planner lag"
echo "  Reference: run_pgbench_sweep.sh (records 5433); scale plan for 7–8 figure row counts in PGBENCH_HARDENING.md"
echo ""
