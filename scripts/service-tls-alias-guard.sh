#!/usr/bin/env bash
# Fail if service-tls and edge-service-tls ca.crt fingerprints differ (mTLS drift: api-gateway vs auth, etc.).
#
...
for s in service-tls edge-service-tls; do
...
_b="$(kubectl get secret edge-service-tls -n "$NS" -o jsonpath='{.data.ca\.crt}' --request-timeout=20s | base64 -d | _fp)"

if [[ -z "$_a" || -z "$_b" ]]; then
  bad "Could not read ca.crt from service-tls and/or edge-service-tls"
  exit 1
fi

echo "   service-tls     ca.crt SHA-256=$_a"
echo "   edge-service-tls ca.crt SHA-256=$_b"
...
ok "service-tls and edge-service-tls ca.crt match"
