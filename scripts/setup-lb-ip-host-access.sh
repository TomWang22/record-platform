...
if [[ "$USE_DIRECT_LB" == "1" ]] || [[ "${COLIMA_DIRECT_LB:-0}" == "1" ]]; then
  ...
  _h2=$(curl -k -sS -o /dev/null -w '%{http_code}' --connect-timeout 5 --max-time 8 --resolve "record-platform.test:443:$LB_IP" "https://record-platform.test/_caddy/healthz" 2>/dev/null || echo "000")
  if [[ "$_h2" == "200" ]]; then
    _h3="000"
    if [[ -n "$CURL_HTTP3" ]] && [[ -x "$CURL_HTTP3" ]]; then
      _h3=$(NGTCP2_ENABLE_GSO=0 "$CURL_HTTP3" --http3-only -k -sS -o /dev/null -w '%{http_code}' --connect-timeout 5 --max-time 10 --resolve "record-platform.test:443:$LB_IP" "https://record-platform.test/_caddy/healthz" 2>/dev/null || echo "000")
    fi
...
sleep 2
VERIFY_H2=$(curl -k --http2 -sS -o /dev/null -w '%{http_code}' --connect-timeout 5 --resolve "record-platform.test:443:$LB_IP" "https://record-platform.test/_caddy/healthz" 2>/dev/null || echo "000")
...
if [[ -n "$CURL_HTTP3" ]]; then
  VERIFY_H3=$(NGTCP2_ENABLE_GSO=0 "$CURL_HTTP3" --http3-only -k -sS -o /dev/null -w '%{http_code}' --connect-timeout 5 --resolve "record-platform.test:443:$LB_IP" "https://record-platform.test/_caddy/healthz" 2>/dev/null) || true
...
echo "  curl -k --http2 -sS -o /dev/null -w '%{http_code}' --resolve record-platform.test:443:$LB_IP https://record-platform.test/_caddy/healthz"
if [[ -n "$CURL_HTTP3" ]]; then
  echo "  NGTCP2_ENABLE_GSO=0 $CURL_HTTP3 --http3-only -k -sS -o /dev/null -w '%{http_code}' --connect-timeout 5 --resolve record-platform.test:443:$LB_IP https://record-platform.test/_caddy/healthz"
  ...
  echo "  NGTCP2_ENABLE_GSO=0 /opt/homebrew/opt/curl/bin/curl --http3-only -k -sS -o /dev/null -w '%{http_code}' --connect-timeout 5 --resolve record-platform.test:443:$LB_IP https://record-platform.test/_caddy/healthz   # requires: brew install curl"
fi
