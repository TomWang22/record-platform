#!/usr/bin/env bash
# Idiot-proof one-shot: certs → TLS secrets → Caddy (ingress-nginx) + Envoy (envoy-test) + app namespace (HOUSING_NS, default record-platform).
...
#
# What this does:
#   1. Create namespaces: ingress-nginx (Caddy), envoy-test (Envoy), record-platform / HOUSING_NS (app pods)
...
_APP_NS="${HOUSING_NS:-record-platform}"

say "=== TLS + Edge setup (Caddy + Envoy + namespaces) — run from repo root ==="

# --- 1. Namespaces (Caddy in ingress-nginx, Envoy in envoy-test, app in $_APP_NS) ---
say "Step 1/7: Create namespaces (ingress-nginx, envoy-test, $_APP_NS)"
for ns in ingress-nginx envoy-test "$_APP_NS"; do
  kubectl create namespace "$ns" --dry-run=client -o yaml | kubectl apply -f - 2>/dev/null || true
done
ok "Namespaces ensured"

# --- 2. Generate all certs (CA, Caddy leaf, Envoy client, services) ---
say "Step 2/7: Generate certs (CA, record-platform.test, envoy-client, services)"
"$SCRIPT_DIR/dev-generate-certs.sh" 2>&1 || true
if [[ ! -f "$REPO_ROOT/certs/record-platform.test.crt" ]] || [[ ! -f "$REPO_ROOT/certs/dev-root.pem" ]]; then
...
ok "TLS secrets in ingress-nginx, $_APP_NS, envoy-test"
...
echo "  Namespaces: ingress-nginx (Caddy), envoy-test (Envoy), $_APP_NS (app services)"
...
echo ""
ok "TLS + edge setup complete. Next: deploy app services to $_APP_NS or run ./scripts/ensure-caddy-envoy-strict-tls.sh to re-check."
