...
#
# Usage: HOUSING_NS=record-platform ./scripts/smart-rollout-rp-if-image-changed.sh
# Env:
...
DEPLOYS=("${RP_RUNTIME_APP_DEPLOYS[@]}")
...
say "smart-rollout-rp-if-image-changed (Record Platform ns=$NS)"
_any=0
for d in "${DEPLOYS[@]}"; do
