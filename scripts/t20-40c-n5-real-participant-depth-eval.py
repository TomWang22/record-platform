#!/usr/bin/env python3
"""T20.40C-LIVE — N=5, 24-window real-participant depth eval."""
...
OUT_DIR = REPO / "bench_logs/ai-platform/t20-40c-n5-real-participant-depth-eval"
WINDOWS = 24
RUNS_PER_USER = 5
PROMPTS = [
...
def validate_summary(summary: Dict[str, Any], cases: List[Dict[str, Any]]) -> None:
    expected_total = 6480
    expected_gate_counts = {"preview_opt_in": 5400, "allowlist": 1080}
    expected_mode_counts = {"hybrid_canary": 6480}
...
def main() -> int:
    ...
    for window in range(1, WINDOWS + 1):
        print(f"=== T20.40C window {window}/{WINDOWS}: lifecycle gate ===", flush=True)
    ...
    print("=== T20.40C post-eval revoke all preview enrollments ===", flush=True)
    revoke_and_verify_keyword(sessions)
    verify_contract_allowlist(sessions)
