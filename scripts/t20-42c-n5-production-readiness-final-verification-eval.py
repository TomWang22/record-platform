#!/usr/bin/env python3
"""T20.42C-LIVE — N=5, 16-window production-readiness final verification eval."""
...
OUT_DIR = REPO / "bench_logs/ai-platform/t20-42c-n5-production-readiness-final-verification-eval"
WINDOWS = 16
RUNS_PER_USER = 5
EXPECTED_TOTAL = 4320
EXPECTED_GATE_COUNTS = {"preview_opt_in": 3600, "allowlist": 720}
EXPECTED_MODE_COUNTS = {"hybrid_canary": 4320}
PROMPTS = [
...
def main() -> int:
    ...
    for window in range(1, WINDOWS + 1):
        print(f"=== T20.42C window {window}/{WINDOWS}: lifecycle gate ===", flush=True)
    ...
    print("=== T20.42C post-eval revoke all preview enrollments ===", flush=True)
    revoke_and_verify_keyword(sessions)
    verify_contract_allowlist(sessions)
