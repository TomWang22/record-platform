...
if [[ -z "$CLUSTER_IP" ]]; then
  ...
  # Host: record-platform.test required so Caddy matches https://record-platform.test vhost (otherwise :443 catch-all; -k = no CA in pod)
  _kb run "$CADDY_POD" --image="$CURL_IMAGE" --restart=Never -n "$NS_ING" -- \
    curl -sS -o /dev/null -w "%{http_code}" -k --connect-timeout 5 --max-time 10 --resolve "record-platform.test:443:${CLUSTER_IP}" "https://record-platform.test/_caddy/healthz" 2>/dev/null || true
...
for attempt in $(seq 1 "$HAPROXY_ATTEMPTS"); do
  ...
  if [[ "$CODE" == "503" ]]; then
    ...
    if [[ $consecutive_503 -ge "$HAPROXY_CONSECUTIVE_503_FAIL" ]]; then
      info "  Diagnose (HAProxy pod has no curl): kubectl run curl-diagnose --rm -i --restart=Never -n $NS_APP --image=curlimages/curl:latest -- curl -s -o /dev/null -w '%{http_code}' http://api-gateway.${NS_APP}.svc.cluster.local:4000/readyz"
