...
if [[ "$REGISTER_CODE" == "201" ]]; then
  TOKEN=$(echo "$REGISTER_RESPONSE" | head -n -1 | grep -o '"token":"[^"]*"' | cut -d'"' -f4 || echo "")
...
if [[ "$LOGIN_CODE" == "200" ]]; then
  TOKEN=$(echo "$LOGIN_RESPONSE" | head -n -1 | grep -o '"token":"[^"]*"' | cut -d'"' -f4 || echo "")
...
if "$CURL_BIN" -k -sS -I --http3-only --max-time 10 -H "Host: $HOST" "https://$HOST:8443/_caddy/healthz" 2>&1 | head -n1 | grep -q "200"; then
