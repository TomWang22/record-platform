...
# mTLS client certs: sync from cluster edge-service-tls (or repo leaf) — never rely on missing certs/tls.crt in repo root.
...
if [[ -n "$PROTO_DIR" ]] && command -v grpcurl >/dev/null 2>&1; then
  ...
  grpc_authority="${HOST:-record-platform.test}"
