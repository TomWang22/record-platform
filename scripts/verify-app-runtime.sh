...
NS_CFG="$(jq -r '.namespace // "record-platform"' "$CONFIG")"
...
# Cold-bootstrap first probes are slow (rollouts, image pull, JVM/Prisma warm-up). Warm/CI keep tight SLOs.
if [[ "${VERIFY_APP_RUNTIME_PHASE:-}" == "cold" ]]; then
  : "${RP_APP_RUNTIME_P95_MAX_MS:=120000}"
  : "${RP_APP_RUNTIME_P99_MAX_MS:=180000}"
  : "${RP_APP_RUNTIME_P100_MAX_MS:=240000}"
  : "${RP_APP_RUNTIME_CRITICAL_PATH_MAX_MS:=240000}"
  export VERIFY_APP_RUNTIME_SKIP_SLO_GATE="${VERIFY_APP_RUNTIME_SKIP_SLO_GATE:-1}"
else
  : "${RP_APP_RUNTIME_P95_MAX_MS:=2500}"
  : "${RP_APP_RUNTIME_P99_MAX_MS:=5000}"
  : "${RP_APP_RUNTIME_P100_MAX_MS:=8000}"
  : "${RP_APP_RUNTIME_CRITICAL_PATH_MAX_MS:=10000}"
fi

_emit_app_runtime_json() {
  local ok_flag="$1"
  NS="$NS" REPO_ROOT="$REPO_ROOT" PHASE="$APP_RUNTIME_PHASE" PERC="${TMP_DIR:+$TMP_DIR/.percentiles.json}" DAG="${TMP_DIR:+$TMP_DIR/.dag_analysis.json}" \
    SERVICES_DIR="$TMP_DIR" OK_FLAG="$ok_flag" \
    P95_MAX="${RP_APP_RUNTIME_P95_MAX_MS}" P99_MAX="${RP_APP_RUNTIME_P99_MAX_MS}" \
    P100_MAX="${RP_APP_RUNTIME_P100_MAX_MS}" CP_MAX="${RP_APP_RUNTIME_CRITICAL_PATH_MAX_MS}" \
    python3 <<'PY'
...
lat = {"p50": 0, "p95": 0, "p99": 0, "p100": 0}
pc = os.environ.get("PERC", "")
if pc and os.path.isfile(pc):
    lat = json.load(open(pc, encoding="utf-8"))
cp = 0
dag = os.environ.get("DAG", "")
if dag and os.path.isfile(dag):
    cp = int(json.load(open(dag, encoding="utf-8")).get("critical_path_ms") or 0)
services = []
svc_dir = os.environ.get("SERVICES_DIR", "")
if svc_dir and os.path.isdir(svc_dir):
    for path in sorted(glob.glob(os.path.join(svc_dir, "*.json"))):
        base = os.path.basename(path)
        if base.startswith("."):
            continue
        d = json.load(open(path, encoding="utf-8"))
        services.append(d.get("name") or base.replace(".json", ""))
p95_max = int(os.environ.get("P95_MAX", "2500"))
p99_max = int(os.environ.get("P99_MAX", "5000"))
p100_max = int(os.environ.get("P100_MAX", "8000"))
cp_max = int(os.environ.get("CP_MAX", "10000"))
slo = {
    "p95_ok": lat.get("p95", 0) <= p95_max,
    "p99_ok": lat.get("p99", 0) <= p99_max,
    "p100_ok": lat.get("p100", 0) <= p100_max,
    "critical_path_ok": cp <= cp_max,
}
out = {
    "ok": ok and all(slo.values()),
    "namespace": ns,
    "phase": phase,
...
if not ok:
    errs = [l.strip() for l in sys.stdin if l.strip()]
    out["errors"] = errs
print(json.dumps(out))
repo = os.environ.get("REPO_ROOT", ".")
bench = os.path.join(repo, "bench_logs", "app_runtime_latest.json")
try:
...
emit_fail() {
  printf '%s\n' "$@" | REPO_ROOT="$REPO_ROOT" _emit_app_runtime_json 0
}

emit_ok() {
  local _json
  _json="$(REPO_ROOT="$REPO_ROOT" _emit_app_runtime_json 1 </dev/null)"
  printf '%s\n' "$_json"
  if [[ "${VERIFY_APP_RUNTIME_SKIP_SLO_GATE:-0}" == "1" ]]; then
    return 0
  fi
  if ! printf '%s' "$_json" | jq -e '.ok == true' >/dev/null 2>&1; then
    echo "verify-app-runtime: SLO regression (latency/critical-path) — see latency_percentiles_ms and slo in JSON above" >&2
    jq -r '.slo | to_entries[] | select(.value == false) | "  SLO fail: \(.key)"' <<<"$_json" >&2 || true
    exit 1
  fi
}
...
_check_service_worker() {
  ...
  if [[ "$health_type" != "http" && "$health_type" != "grpc" && "$health_type" != "both" ]]; then
    err="deployment/${name}: invalid health_type '${health_type}' (use http, grpc, both, or auto)"
  ...
  elif [[ "$health_type" == "both" ]]; then
    if [[ -z "$port" ]]; then
      err="deployment/${name}: health_type=both requires port and health_path"
    fi
  else
    if [[ -z "$port" ]]; then
  ...
  local ready_path="${path}"
  local http_url="http://${name}.${NS}.svc.cluster.local:${port}${ready_path}"
  if [[ "$name" == "ollama" ]]; then
    http_url="http://ollama.${NS}.svc.cluster.local:${port}${ready_path}"
  fi

  t0="$(python3 -c "import time; print(int(time.time()*1000))")"
  chmod +x "$SCRIPT_DIR/probe-rp-runtime-service.sh" 2>/dev/null || true
  ...
  if [[ -n "$err" ]]; then
    ...
    while [[ "$i" -le "$retries" ]]; do
      probe_report="$TMP_DIR/probe-${name}"
      mkdir -p "$probe_report"
      if HOUSING_NS="$NS" RP_RUNTIME_HEALTH_REPORT_DIR="$probe_report" \
        bash "$SCRIPT_DIR/probe-rp-runtime-service.sh" "$name" --mode "$health_type" >/dev/null 2>&1; then
        success_protocol="$(jq -r 'if .http_ok then "http" elif .grpc_ok then "grpc" else "unknown" end' "$probe_report/summary.json" 2>/dev/null || echo http)"
        rt_line="$(jq -r '"\(.service // "'"$name"'") http=\(.http_label // .http_status) grpc=\(.grpc_label // "unknown") runtime=\(.runtime_verdict // "pass")"' "$probe_report/summary.json" 2>/dev/null || echo "")"
        echo "  ✅ ${rt_line:-$name ready}" >&2
        ok=1
        break
      fi
      ...
      echo "  ⏳ ${fail_line:-$name} probe failed (${fail_detail}) attempt ${i}/${retries} → sleep ${sleep_sec}s" >&2
      sleep "$sleep_sec"
      i=$((i + 1))
    done
    if [[ "$ok" -ne 1 ]]; then
      err="deployment/${name}: runtime probe mode=${health_type} target=${http_url} status=${fail_status:-?} error=${fail_detail:-probe failed} report=${probe_report:-}"
    fi
  fi
...
if [[ "${VERIFY_APP_RUNTIME_SKIP_ANALYTICS_OLLAMA:-0}" != "1" ]] && [[ -z "${VERIFY_APP_RUNTIME_SERVICES:-}" ]]; then
  if jq -e '.services[]? | select(.name == "analytics-service")' "$CONFIG" >/dev/null 2>&1; then
    if kubectl get deploy/analytics-service -n "$NS" --request-timeout=15s &>/dev/null; then
      _ollama_url="${VERIFY_APP_RUNTIME_OLLAMA_URL:-http://ollama.${NS}.svc.cluster.local:11434/api/tags}"
      _ollama_timeout_ms="${VERIFY_APP_RUNTIME_OLLAMA_TIMEOUT_MS:-30000}"
      _ollama_attempts="${VERIFY_APP_RUNTIME_OLLAMA_ATTEMPTS:-5}"
      echo "  verify-app-runtime: analytics-service → ollama reachability (${_ollama_url}, timeout=${_ollama_timeout_ms}ms, attempts=${_ollama_attempts})" >&2
      _ollama_ok=0
      for ((_oa = 1; _oa <= _ollama_attempts; _oa++)); do
        if kubectl exec -n "$NS" deploy/analytics-service -- node -e "
          const u = process.argv[1];
          const ms = Number(process.argv[2]);
          fetch(u, { signal: AbortSignal.timeout(ms) })
        ...
        " "$_ollama_url" "$_ollama_timeout_ms" 2>/dev/null; then
          _ollama_ok=1
          break
        fi
        echo "    … ollama probe attempt ${_oa}/${_ollama_attempts} failed; retry in 5s" >&2
        sleep 5
      done
      if [[ "$_ollama_ok" != "1" ]]; then
        emit_fail "deployment/analytics-service: cannot reach ollama at ${_ollama_url} after ${_ollama_attempts} attempts (timeout=${_ollama_timeout_ms}ms — check ollama Service, HPA, and networkPolicy)"
        exit 1
      fi
    fi
  fi
