...
# Uses grpcurl to Caddy :443 with authority record-platform.test; expects grpc.health.v1.Health/Check SERVING.
...
if [[ -f "$SCRIPT_DIR/lib/ensure-rp-grpc-certs.sh" ]]; then
  ...
  rp_sync_grpc_certs_to_dir "$GRPC_SYNC_DIR" "${HOUSING_NS:-record-platform}" || true
fi
