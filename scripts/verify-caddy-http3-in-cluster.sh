...
#   HOST=record-platform.test  (required when TARGET_IP set for SNI)
#
...
if [[ -n "$_curl_host" ]] && [[ -n "$_lb_ip" ]] && [[ "${USE_HOST_CURL:-0}" == "1" ]]; then
  ...
  [[ -s "$_ca" ]] || _ca=""
  if [[ -n "$_ca" ]]; then
    _code=$(NGTCP2_ENABLE_GSO=0 "$_curl_host" -sS -w '%{http_code}' -o /dev/null --max-time 15 --http3-only --cacert "$_ca" --resolve "record-platform.test:443:$_lb_ip" "https://record-platform.test/_caddy/healthz" 2>/dev/null) || _code=000
    if [[ "$_code" == "200" ]]; then
      ok "Caddy HTTP/3 OK via host Homebrew curl (record-platform.test:443 -> $_lb_ip)"
      exit 0
    fi
  fi
...
# Caddy serves record-platform.test (cert SAN). --resolve requires an IP; get ClusterIP or use TARGET_IP.
if [[ -n "${TARGET_IP:-}" ]]; then
  CURL_RESOLVE="record-platform.test:443:${TARGET_IP}"
else
  ...
  CURL_RESOLVE="record-platform.test:443:${_cluster_ip}"
fi
CURL_URL="https://record-platform.test/_caddy/healthz"
