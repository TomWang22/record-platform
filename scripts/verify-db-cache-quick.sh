...
REDIS_POD=$(kubectl -n "${HOUSING_NS:-record-platform}" get pods -l app=redis -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")

if [[ -n "$REDIS_POD" ]]; then
  if kubectl -n "${HOUSING_NS:-record-platform}" exec "$REDIS_POD" -- redis-cli ping >/dev/null 2>&1; then
    ok "Redis: Connected"
    
    # Get cache hit/miss stats (proves Redis is working)
    REDIS_STATS=$(kubectl -n "${HOUSING_NS:-record-platform}" exec "$REDIS_POD" -- redis-cli info stats 2>/dev/null || echo "")
    if [[ -n "$REDIS_STATS" ]]; then
      ...
      SCRIPT_COUNT=$(kubectl -n "${HOUSING_NS:-record-platform}" exec "$REDIS_POD" -- redis-cli script exists $(kubectl -n "${HOUSING_NS:-record-platform}" exec "$REDIS_POD" -- redis-cli script list 2>/dev/null | head -1 | cut -d' ' -f1 2>/dev/null || echo "") 2>/dev/null | grep -c "1" || echo "0")
  ...
  if [[ "${SKIP_REDIS_HOST_CHECK:-0}" == "1" ]]; then
    ...
    if [[ -n "$REDIS_REACHABLE" ]]; then
      ...
      warn "Redis: Not reachable from host (tried localhost, 127.0.0.1, host.docker.internal) — ensure Docker Compose Redis is running and port ${REDIS_PORT:-6379} is exposed"
    fi
  else
