...
_apply_overlay() {
  ...
  if [[ ! -d "$od" ]]; then
    bad "AUTO_HEAL overlay path invalid: $od"
    return 1
  fi
  if [[ -x "$SCRIPT_DIR/rp-apply-rp-app-manifests.sh" ]]; then
    say "AUTO_HEAL: bash scripts/rp-apply-rp-app-manifests.sh (overlay $OVERLAY_REL → $NS)"
    HOUSING_NS="$NS" DEPLOY_OVERLAY="$OVERLAY_REL" bash "$SCRIPT_DIR/rp-apply-rp-app-manifests.sh"
    return $?
  fi
  local _cmd
  if command -v kustomize &>/dev/null 2>&1; then
    _cmd="kustomize build \"$od\" | kubectl apply -f -"
  else
    _cmd="kubectl kustomize \"$od\" | kubectl apply -f -"
  fi
  say "AUTO_HEAL: $_cmd"
  local _log
  _log="$(mktemp)"
  if ! eval "$_cmd" 2>&1 | tee "$_log"; then
    bad "AUTO_HEAL apply failed"
    tail -30 "$_log" >&2
    rm -f "$_log"
    return 1
  fi
  rm -f "$_log"
  echo "AUTO_HEAL: verifying required Services again…"
  local _svc
  for _svc in ollama "${RP_RUNTIME_APP_DEPLOYS[@]}"; do
    if kubectl get svc "$_svc" -n "$NS" --request-timeout=15s &>/dev/null; then
      echo "  ✅ ${_svc} service exists"
    else
      bad "AUTO_HEAL: Service/$_svc still missing in $NS"
      return 1
    fi
  done
  ok "AUTO_HEAL apply + Service verification OK"
}

say "verify-deployment-integrity (Record Platform ns=$NS AUTO_HEAL=$AUTO_HEAL heal_attempt=$HEAL_ATTEMPT/$MAX_HEAL)"
...
if [[ "$AUTO_HEAL" == "1" ]]; then
  ...
  if ! _apply_overlay; then
    bad "AUTO_HEAL apply failed — fix overlay path or cluster permissions"
    exit 1
  fi
