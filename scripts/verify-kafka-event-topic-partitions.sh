...
rp_kafka_event_topics_fill || fail "Could not build topic list from proto/events mirror"
...
if [[ "${KAFKA_PARTITION_VERIFY_TARGET:-}" == "k8s" ]]; then
  ...
  _ns="${KAFKA_K8S_NS:-record-platform}"
...
} > /tmp/rp-kafka-verify.props'
  kubectl exec -n "$_ns" "$_pod" -- bash -ec "$_inner_props" || fail "Could not write TLS props in $_pod"
  _bs="kafka-0.kafka:9093"
  for t in "${TOPICS[@]}"; do
    out="$(kubectl exec -n "$_ns" "$_pod" -- kafka-topics --bootstrap-server "$_bs" --command-config /tmp/rp-kafka-verify.props --describe --topic "$t" 2>/dev/null | head -8 || true)"
...
_partition_for_topic() {
  ...
  docker compose exec -T kafka env "KAFKA_TOPIC=$t" sh -c '
    kafka-topics --bootstrap-server localhost:9093 --command-config /tmp/rp-kafka-verify.props --describe --topic "$KAFKA_TOPIC" 2>/dev/null | head -8
  '
}
