...
#
# API (Colima bridge): when kubectl context matches *colima*, runs ./scripts/rp-kube-api-health.sh first.
...
NS="${KRAFT_E2E_NS:-record-platform}"
say() { printf "\n\033[1m%s\033[0m\n" "$*"; }
pass() { echo "[OK] $*"; }
...
if [[ "$_run_api_health" == "1" ]] && [[ -f "$SCRIPT_DIR/rp-kube-api-health.sh" ]]; then
  bash "$SCRIPT_DIR/rp-kube-api-health.sh" || fail "k3s API unhealthy — bash scripts/rp-align-colima-kubeconfig.sh && bash scripts/rp-kube-api-health.sh"
fi
...
run_kafka_contract_json() {
  ...
  env \
    ...
    RP_KAFKA_TOPIC_SUFFIX="${RP_KAFKA_TOPIC_SUFFIX:-}" \
...
} > /tmp/rp-kraft-describe.props
kafka-topics --bootstrap-server kafka-0.kafka:9093 --command-config /tmp/rp-kraft-describe.props --describe
...
if [[ "${KRAFT_E2E_PHASE7_DELETE_TWO_BROKERS:-0}" == "1" ]]; then
  ...
  env REPO_ROOT="$REPO_ROOT" \
    ...
    KAFKA_CLIENT_CERT="${KAFKA_CLIENT_CERT}" \
    KAFKA_CLIENT_KEY="${KAFKA_CLIENT_KEY}" \
    bash "$SCRIPT_DIR/validate-kafka-stack-contract.sh" >/tmp/rp-kraft-preflight-fail.out 2>/tmp/rp-kraft-preflight-fail.err
  _pe=$?
  set -e
  [[ "$_pe" != "0" ]] || fail "Expected validate-kafka-stack-contract.sh to fail when <3 brokers; it exited 0"
  grep -q 'ERROR: Kafka quorum' /tmp/rp-kraft-preflight-fail.err 2>/dev/null || grep -q 'ERROR: Kafka quorum' /tmp/rp-kraft-preflight-fail.out 2>/dev/null || \
    warn "Gate failed (exit $_pe) but exact ERROR line not found in captured output (check /tmp/rp-kraft-preflight-fail.err)"
...
if [[ "${KRAFT_E2E_PHASE10_SMOKE_PRODUCE_CONSUME:-0}" == "1" ]]; then
  ...
  _topic="${KRAFT_E2E_SMOKE_TOPIC:-rp-kraft-e2e-smoke}"
  kctl exec -n "$NS" kafka-0 -- bash -ec "
TS_PASS=\$(cat /etc/kafka/secrets/kafka.truststore-password)
KS_PASS=\$(cat /etc/kafka/secrets/kafka.keystore-password)
KP_PASS=\$(cat /etc/kafka/secrets/kafka.key-password 2>/dev/null || echo \"\$KS_PASS\")
...
kafka-topics --bootstrap-server kafka-0.kafka:9093 --command-config /tmp/rp-smoke.props --create --if-not-exists --topic ${_topic} --replication-factor 3 --partitions 1
...
echo '${_msg}' | kafka-console-producer --bootstrap-server kafka-0.kafka:9093 --producer.config /tmp/rp-smoke.props --topic ${_topic}
...
timeout 25 kafka-console-consumer --bootstrap-server kafka-1.kafka:9093 --consumer.config /tmp/rp-smoke.props --topic ${_topic} --from-beginning --max-messages 1 2>/dev/null | tail -1
")"
