...
_pool_name=""
for _p in record-platform-pool record-platform-pool default-pool default; do
...
if [[ -z "$_pool_name" ]]; then
  warn "IPAddressPool record-platform-pool (or legacy record-platform-pool) not found. Apply infra/k8s/metallb/ (or install script)."
else
...
_l2_name=""
for _l2 in record-platform-l2 record-platform-l2; do
  if _kb -n "$NS_METALLB" get l2advertisement "$_l2" -o name >/dev/null 2>&1; then
    _l2_name="$_l2"
    break
  fi
done
if [[ -z "$_l2_name" ]]; then
  warn "L2Advertisement record-platform-l2 (or legacy record-platform-l2) not found. Custom traffic policy (L2 + optional nodeSelector) is in docs/METALLB_TRAFFIC_POLICY_AND_SCALE.md"
else
  ok "L2Advertisement $_l2_name exists (L2 mode; nodeSelector for priority in doc)"
fi
...
spec:
  ...
  - name: curl
    ...
    - |
      ...
      code1="000"
      out1=\$(curl -k -sS -w '%{http_code}' -o /tmp/b1 --max-time 10 -H "Host: record-platform.test" "https://\$LB_IP/_caddy/healthz" 2>&1) || true
      code1=\$(echo "\$out1" | tail -1)
      echo "HTTP1: \$code1 (curl stderr: \$(echo "\$out1" | head -5))"
      code2="000"
      out2=\$(curl -k -sS -w '%{http_code}' -o /tmp/b2 --max-time 10 --http2 -H "Host: record-platform.test" "https://\$LB_IP/_caddy/healthz" 2>&1) || true
...
spec:
  ...
  - name: curl
    ...
    - -c
    - |
      code=\$(curl -k -sS -o /dev/null -w '%{http_code}' --connect-timeout 3 --max-time 5 --resolve "record-platform.test:443:$lb_ip" "https://record-platform.test/_caddy/healthz" 2>/dev/null) || code="000"
      echo "\$code"
PODEOF2
      _node_ok=0
...
# Helper: curl to LB IP using record-platform.test for TLS (cert SAN). Returns 200 or 000.
_curl_lb_health() {
  curl -k -sS -o /dev/null -w "%{http_code}" --connect-timeout 3 --max-time 5 --resolve "record-platform.test:443:${1}" "https://record-platform.test/_caddy/healthz" 2>/dev/null || echo "000"
}
...
_curl_host_path_health() {
  if [[ -n "$host_https_port" ]] && [[ -n "$host_https_host" ]]; then
    curl -k -sS -o /dev/null -w "%{http_code}" --connect-timeout 3 --max-time 5 --resolve "record-platform.test:${host_https_port}:${host_https_host}" "https://record-platform.test:${host_https_port}/_caddy/healthz" 2>/dev/null || echo "000"
  else
...
if [[ "${SKIP_HOST_CURL:-0}" != "1" ]]; then
  ...
  if [[ -n "$host_reached_lb" ]] && { [[ -n "$lb_ip" ]] || [[ -n "$host_curl_host" ]]; }; then
    ...
    _h3_resolve="record-platform.test:${_h3_port}:${_h3_host}"
    _h3_url="https://record-platform.test:${_h3_port}/_caddy/healthz"
    [[ "$_h3_port" == "443" ]] && _h3_url="https://record-platform.test/_caddy/healthz"
    _h3_args=()
    ...
    if [[ "$ctx" == *"colima"* ]] && [[ -n "$lb_ip" ]] && kubectl get ns "$NS_ING" &>/dev/null; then
      ...
      _pod_h3_out=$(kubectl -n "$NS_ING" run "verify-h3-lb-$$" --rm -i --restart=Never --request-timeout=60s --quiet \
        ...
        --image="$_h3_img" -- \
        curl -k -sS -o /dev/null -w "%{http_code}\n" --connect-timeout 8 --max-time 20 --http3 --resolve "record-platform.test:443:${lb_ip}" "https://record-platform.test/_caddy/healthz" 2>/dev/null || echo "000")
    ...
    if [[ "$_http3_lb_ok" != "1" ]] && [[ "$ctx" == *"colima"* ]] && [[ -n "$lb_ip" ]] && command -v colima &>/dev/null 2>&1; then
      _vm_h3=$(colima ssh -- curl -k -sS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 10 --http3-only --resolve "record-platform.test:443:${lb_ip}" "https://record-platform.test/_caddy/healthz" 2>/dev/null || echo "000")
    ...
    if [[ "$_http3_lb_ok" != "1" ]] && [[ "$ctx" == *"colima"* ]] && [[ "$_h3_port" == "8443" ]] && [[ "$(uname -s)" == "Darwin" ]] && [[ -x "$CURL_BIN" ]] && "$CURL_BIN" --help all 2>/dev/null | grep -q -- "--http3-only"; then
      export NGTCP2_ENABLE_GSO="${NGTCP2_ENABLE_GSO:-0}"
      _colima_h3=$(_normalize_http_code "$("$CURL_BIN" --http3-only -k -sS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 15 --resolve "record-platform.test:8443:127.0.0.1" "https://record-platform.test:8443/_caddy/healthz" 2>/dev/null || echo "000")")
    ...
    if [[ "$_http3_lb_ok" != "1" ]] && [[ "$_h3_port" != "443" ]] && [[ "$ctx" != *"colima"* ]]; then
      ...
      if [[ "$(uname -s)" == "Darwin" ]] && [[ -x "$CURL_BIN" ]] && "$CURL_BIN" --help all 2>/dev/null | grep -q -- "--http3-only"; then
        _h3_exact_code=$("$CURL_BIN" --http3-only -k -sS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 15 \
            --resolve "record-platform.test:${_h3_port}:127.0.0.1" "https://record-platform.test:${_h3_port}/_caddy/healthz" 2>/dev/null || echo "000")
    ...
    if [[ "$_http3_lb_ok" != "1" ]] && [[ -f "${SCRIPT_DIR}/lib/http3.sh" ]] && { [[ "$ctx" != *"colima"* ]] || [[ "$_h3_port" == "443" ]]; }; then
      ...
      if [[ "$(uname -s)" == "Darwin" ]]; then
        ...
        [[ -n "$_docker_ip" ]] && _h3_resolve="record-platform.test:${_docker_port}:$_docker_ip" && _h3_url="https://record-platform.test:${_docker_port}/_caddy/healthz"
      fi
    ...
    if [[ "$_http3_lb_ok" != "1" ]] && [[ "$VERIFY_MODE" == "stable" ]]; then
      ...
      if [[ "$_http3_lb_ok" != "1" ]]; then
        ...
        info "Manual in-cluster check: kubectl -n $NS_ING run verify-h3-manual --rm -i --restart=Never --image=rmarx/curl-http3:latest --overrides='{\"spec\":{\"hostNetwork\":true}}' -- curl -k -v --http3 --resolve record-platform.test:443:$lb_ip https://record-platform.test/_caddy/healthz"
  ...
  if [[ "$ctx" == *"colima"* ]]; then
    ...
    if [[ "$_http3_lb_ok" == "1" ]]; then
      ...
      info "On Colima, NodePort is on VM; 127.0.0.1:$caddy_nodeport is not valid from Mac. Verify QUIC in-VM: colima ssh -- curl -k --http3-only --resolve record-platform.test:443:$lb_ip https://record-platform.test/_caddy/healthz"
    fi
  ...
  if [[ "$_http3_np_ok" != "1" ]] && [[ "$ctx" != *"colima"* ]]; then
    ...
    if [[ "$(uname -s)" == "Darwin" ]] && "$CURL_BIN" --help all 2>/dev/null | grep -q -- "--http3-only"; then
      _np_code=$(_normalize_http_code "$(NGTCP2_ENABLE_GSO=0 "$CURL_BIN" --http3-only -sS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 15 \
          --resolve "record-platform.test:${caddy_nodeport}:127.0.0.1" "${_np_args[@]}" "https://record-platform.test:${caddy_nodeport}/_caddy/healthz" 2>/dev/null || echo "000")")
    ...
    if [[ "$_http3_lb_ok" != "1" ]] && [[ -n "${lb_ip:-}" ]] && [[ "$ctx" == *"k3d"* ]] && [[ "${SKIP_LB_IP_SETUP:-0}" != "1" ]] && command -v socat >/dev/null 2>&1; then
      ...
      if sudo LB_IP="$lb_ip" NODEPORT="$caddy_nodeport" "$SCRIPT_DIR/setup-lb-ip-host-access.sh" 2>/dev/null; then
        ...
        _code_retry=$(_normalize_http_code "$(NGTCP2_ENABLE_GSO=0 "$CURL_BIN" --http3-only -sS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 12 \
            --resolve "record-platform.test:443:$lb_ip" "${_h3_args[@]}" "https://record-platform.test/_caddy/healthz" 2>/dev/null || echo "000")")
  ...
  if [[ "$_http3_lb_ok" != "1" ]] && [[ "$_http3_np_ok" != "1" ]]; then
    ...
    if [[ "$ctx" == *"colima"* ]]; then
      info "  Colima: QUIC is validated only in-VM. Run: colima ssh -- curl -k --http3-only --resolve record-platform.test:443:$lb_ip https://record-platform.test/_caddy/healthz"
      info "  If VM curl lacks --http3-only, install curl+ngtcp2 in the VM, or use bridged so Mac can hit LB IP directly (recommended): ./scripts/colima-start-k3s-bridged-clean.sh  then from Mac: curl -k --http3-only --resolve record-platform.test:443:$lb_ip https://record-platform.test/_caddy/healthz"
  ...
  if [[ "$_http3_lb_ok" != "1" ]] && [[ "${host_curl_port:-}" == "8443" ]]; then
    ...
    info "     NGTCP2_ENABLE_GSO=0 $_curl_path --http3-only -k -v --resolve record-platform.test:8443:127.0.0.1 https://record-platform.test:8443/_caddy/healthz"
...
if [[ "${SKIP_HOST_CURL:-0}" == "1" ]]; then
  ...
  if [[ -n "$host_reached_lb" ]] && { [[ -n "$lb_ip" ]] || [[ -n "$host_curl_host" ]]; }; then
    ...
    _url="https://record-platform.test/_caddy/healthz"
    [[ "$_port_6a" != "443" ]] && _url="https://record-platform.test:${_port_6a}/_caddy/healthz"
    _resolve_6a=(--resolve "record-platform.test:${_port_6a}:${_host_6a}")
    _url_h3="$_url"
    if [[ "$(uname -s)" == "Darwin" ]] && [[ "$_port_6a" == "443" ]]; then
      ...
      [[ -n "$_docker_ip_6a" ]] && _resolve_6a=(--resolve "record-platform.test:${_docker_port_6a}:$_docker_ip_6a") && _url_h3="https://record-platform.test:${_docker_port_6a}/_caddy/healthz"
    fi
    _h1=""
    _h2=""
    _h3=""
    ...
    _code=$(_normalize_http_code "$(curl -k -sS -o /dev/null -w "%{http_code}" --http1.1 --connect-timeout 3 --max-time 5 --resolve "record-platform.test:${_port_6a}:${_host_6a}" "$_url" 2>/dev/null || echo "000")")
    [[ "$_code" == "200" ]] && _h1="200" || _h1="failed ($_code)"
    _code=$(_normalize_http_code "$(curl -k -sS -o /dev/null -w "%{http_code}" --http2 --connect-timeout 3 --max-time 5 --resolve "record-platform.test:${_port_6a}:${_host_6a}" "$_url" 2>/dev/null || echo "000")")
      ...
      for _a in 1 2 3 4 5; do
        if [[ "$(uname -s)" == "Darwin" ]] && [[ -x "$CURL_BIN" ]] && "$CURL_BIN" --help all 2>/dev/null | grep -q -- "--http3-only"; then
          _h3_code=$(_normalize_http_code "$(NGTCP2_ENABLE_GSO=0 "$CURL_BIN" --http3-only -sS -o /dev/null -w "%{http_code}" --connect-timeout 5 --max-time 15 \
              --resolve "record-platform.test:${_port_6a}:${_host_6a}" "${_ca_6a[@]}" "$_url" 2>/dev/null || echo "000")")
        fi
...
if [[ -n "$host_reached_lb" ]] && [[ -n "$lb_ip" ]]; then
  ...
  if [[ "${_http3_lb_ok:-0}" == "1" ]]; then
    ...
    if [[ "$ctx" == *"colima"* ]]; then
      info "Host reachability: LB IP $lb_ip — HTTP/1.1, HTTP/2 verified. HTTP/3: colima ssh -- curl -k --http3-only --resolve record-platform.test:443:$lb_ip https://record-platform.test/_caddy/healthz  (or use bridged: ./scripts/colima-start-k3s-bridged-clean.sh so Mac can curl LB IP directly)."
    else
