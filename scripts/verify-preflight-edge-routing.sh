...
_verify_ingress_python() {
  ...
  _jf="$(mktemp "${TMPDIR:-/tmp}/rp-edge-ing.XXXXXX.json")"
  printf '%s\n' "$_ing_json" > "$_jf"
...
want_host = os.environ.get("_EDGE_VERIFY_HOST", "record-platform.test")
...
EXPECTED = [
    ("/api(/|$)(.*)", "ImplementationSpecific", "api-gateway", 4000),
    ("/auth(/|$)(.*)", "ImplementationSpecific", "api-gateway", 4000),
    ("/", "Prefix", "nginx", 8080),
]
...
if [[ "$RUN_INGRESS" -eq 1 ]]; then
  ...
  ok "Ingress: /api + /auth → api-gateway:4000, / → nginx:8080 (matches dev overlay + Caddyfile upstream port)"
fi
