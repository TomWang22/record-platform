...
**Lab observability (single edge IP via Caddy):** `https://record.test/jaeger/` (Jaeger UI + Query API), `/grafana/`, `/prometheus/` — see `infra/k8s/caddy-h3-configmap.yaml` + `Caddyfile`. Trace scripts prefer `JAEGER_QUERY_BASE=https://record.test/jaeger` (`scripts/lib/jaeger-resolve-query-base.sh`). OTel collector stays ClusterIP-only.
