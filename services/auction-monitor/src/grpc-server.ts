...
export function startGrpcServer(port: number) {
  ...
  if (process.env.ENABLE_GRPC_REFLECTION !== "false") {
    try {
      ...
      enableReflection(server, [protoPath], ["auction_monitor.AuctionMonitorService"]);
    } catch (err) {
      console.warn("[auction-monitor gRPC] Failed to enable reflection:", err);
    }
  }
  ...
  server.bindAsync(`0.0.0.0:${port}`, credentials, (err, actualPort) => {
    ...
    server.start()
    console.log(`[auction-monitor-grpc] server listening on ${actualPort} (HTTP/2 only)`)
  })
