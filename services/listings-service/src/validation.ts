...
/** RP metadata keys merged from top-level create body into amenities storage. */
const RP_AMENITY_BODY_KEYS: Record<string, string> = {
  format: "format",
  ...
  label: "label",
  pressing_year: "pressing_year",
  pressingYear: "pressing_year",
  subtitle: "subtitle",
  allow_offers: "allow_offers",
  allowOffers: "allow_offers",
  sale_type: "sale_type",
  domestic_shipping_cents: "domestic_shipping_cents",
  international_shipping_cents: "international_shipping_cents",
  shipping_service: "shipping_service",
  package_type: "package_type",
  ships_from_city: "ships_from_city",
  ships_from_region: "ships_from_region",
  ...
  offer_expiration_hours: "offer_expiration_hours",
  auto_accept_cents: "auto_accept_cents",
  auto_decline_cents: "auto_decline_cents",
  starting_bid_cents: "starting_bid_cents",
  reserve_price_cents: "reserve_price_cents",
  buy_it_now_cents: "buy_it_now_cents",
  auction_starts_at: "auction_starts_at",
  auction_ends_at: "auction_ends_at",
  auction_rollover: "auction_rollover",
  video_url: "video_url",
  source_record_id: "source_record_id",
};

export function rpAmenityEntriesFromBody(body: Record<string, unknown>): string[] {
  const out: string[] = [];
  for (const [bodyKey, slug] of Object.entries(RP_AMENITY_BODY_KEYS)) {
    const v = body[bodyKey];
    if (v == null || v === "") continue;
    out.push(`${slug}:${String(v).trim()}`);
  }
  return out;
}

// Normalizes amenities to `key:value` strings (arrays or RP object maps).
...
export function validateCreateListingInput(
  ...
  /** Legacy column — use `other` for RP listings; never default to housing slugs. */
  const residence_type = rtRaw ?? "other";
