...
describe("createListingsHttpApp", () => {
  ...
  it("GET / — search returns deduped items", async () => {
    ...
    const first = res.body.items[0] as Record<string, unknown>;
    expect(first.address_line1).toBeUndefined();
  ...
  it("GET /listings/:id — 200 public marketplace detail (no private address)", async () => {
    const app = createListingsHttpApp();
