...
const LISTING_SUBJECT_RE = /^\[listing:([0-9a-f-]{36})\]\s*(.*)$/i

function listingMetaFromSubject(subject: string): { listingId: string | null; listingTitle: string | null } {
  const match = String(subject || '').trim().match(LISTING_SUBJECT_RE)
  if (!match) return { listingId: null, listingTitle: null }
...
export default function messagesRouter(redis: Redis | null, cpuCores: number) {
  ...
  router.post('/', async (req: AuthedRequest, res: Response) => {
    ...
    try {
      ...
      if (recipient_id && !group_id) {
        const listingMeta = listingMetaFromSubject(subjectFinal)
        void pushMessageReceivedNotification({
          recipientId: String(recipient_id),
          senderId: String(sender_id),
          messageId: String(message.id ?? ''),
          threadId: String(message.thread_id ?? effectiveThreadId ?? ''),
          content: String(content),
          listingId: listingMeta.listingId,
          listingTitle: listingMeta.listingTitle,
        })
      }
    ...
    try {
      const { rows } = await pool.query(
        `SELECT t.thread_id, t.archived_at,
          (SELECT m.subject FROM messages.messages m
           WHERE (m.thread_id::text = t.thread_id::text OR m.group_id::text = t.thread_id::text)
             AND (m.recipient_id = $1::uuid OR m.sender_id = $1::uuid
               OR m.group_id IN (SELECT group_id FROM messages.group_members WHERE user_id = $1::uuid))
           ORDER BY m.created_at ASC LIMIT 1) AS subject,
          (SELECT m.created_at FROM messages.messages m
           WHERE (m.thread_id::text = t.thread_id::text OR m.group_id::text = t.thread_id::text)
             AND (m.recipient_id = $1::uuid OR m.sender_id = $1::uuid
               OR m.group_id IN (SELECT group_id FROM messages.group_members WHERE user_id = $1::uuid))
    ...
    try {
        ...
        `SELECT 1 WHERE
          ...
          OR EXISTS (
            ...
            WHERE (
                ...
                OR ${sqlHumanPairConversationId('m')} = $1
              )
              AND (
                m.sender_id = $2::uuid
      ...
      if (access.rows.length === 0) {
        const exists = await client.query(
          `SELECT 1 FROM messages.messages m
           WHERE m.thread_id::text = $1 OR m.group_id::text = $1 OR ${sqlHumanPairConversationId('m')} = $1
           LIMIT 1`,
          [loadKey],
        )
  ...
  router.delete('/thread/:threadId/archive', async (req: AuthedRequest, res: Response) => {
    ...
    try {
      await pool.query(
        `DELETE FROM messages.user_archived_threads WHERE user_id = $1 AND thread_id::text IN ($2::text, $3::text)`,
        [userId, loadKey, threadId],
      )
      await bustThreadCache(loadKey)
      await bustThreadCache(threadId)
      return res.status(204).end()
  ...
  router.post('/thread/:threadId/delete', async (req: AuthedRequest, res: Response) => {
    ...
    try {
      ...
      await bustThreadCache(loadKey)
      await bustThreadCache(threadId)
      res.status(201).json({ thread_id: loadKey, deleted_for_me: true })
    } catch (err: any) {
      if ((err as { code?: string })?.code === '42P01') {
  ...
  router.post('/:messageId/reply', async (req: AuthedRequest, res: Response) => {
    ...
    try {
      ...
      if (recipient_id) {
        const listingMeta = listingMetaFromSubject(String(parent.subject || ''))
        void pushMessageReceivedNotification({
          recipientId: String(recipient_id),
          senderId: String(sender_id),
          messageId: String(message.id ?? ''),
          threadId: String(message.thread_id ?? replyThreadId ?? ''),
          content: String(content),
          listingId: listingMeta.listingId,
          listingTitle: listingMeta.listingTitle,
        })
