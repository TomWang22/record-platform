...
export function createNotificationHttpApp(): Application {
    ...
    try {
      if (eventType === "message_received") {
        ...
        const senderId = String(
          (payload as Record<string, unknown>).sender_id ??
            (payload as Record<string, unknown>).senderId ??
            "",
        ).trim();
        ...
        const preview = String(
          (payload as Record<string, unknown>).preview ??
            (payload as Record<string, unknown>).body ??
            "",
        ).trim();
        const listingId = String(
          (payload as Record<string, unknown>).listing_id ??
            (payload as Record<string, unknown>).listingId ??
            "",
        ).trim();
        const listingTitle = String(
          (payload as Record<string, unknown>).listing_title ??
            (payload as Record<string, unknown>).listingTitle ??
            "",
        ).trim();
        const senderDisplayName = String(
          (payload as Record<string, unknown>).sender_display_name ??
            (payload as Record<string, unknown>).senderDisplayName ??
            "",
        ).trim();
        if (!MESH_UUID36.test(messageId)) {
          res.status(400).json({ error: "payload.message_id (uuid) required for message_received" });
          return;
        }
        const inserted = await createMessageReceivedNotification(pool, {
          recipientId: userId,
          senderId,
          messageId: MESH_UUID36.test(messageId) ? messageId.toLowerCase() : messageId,
          threadId,
          preview,
        ...
        if (inserted.inserted) {
          await invalidateNotificationListCacheForUser(userId);
        ...
        res.status(201).json({ ok: true, inserted: inserted.inserted });
        return;
      }
  ...
  app.get("/notifications", requireUser, async (req: AuthedRequest, res: Response) => {
    ...
    try {
      ...
      if (cached) {
        ...
        res.setHeader("X-RP-Server-Time-Ms", String(Date.now() - started));
      ...
      res.setHeader("X-RP-Server-Time-Ms", String(elapsedMs));
  ...
  app.post("/notifications/read-all", requireUser, async (req: AuthedRequest, res: Response) => {
    if (!pool) return res.status(503).json({ error: "db unavailable" });
    try {
      const r = await pool.query(
        `UPDATE notification.notifications
  ...
  /** Contract/E2E seed — requires NOTIFICATION_E2E_SEED=1 */
  app.post("/notifications/seed-contract", requireUser, async (req: AuthedRequest, res: Response) => {
    if (process.env.NOTIFICATION_E2E_SEED !== "1") {
      res.status(404).json({ error: "not found" });
      return;
    }
    if (!pool) return res.status(503).json({ error: "db unavailable" });
