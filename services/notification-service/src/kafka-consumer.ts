...
// RP: no booking topic (bookings skipped). Community + messaging + listings + notification domain events.
        ...
        await withKafkaConsumerSpan(
          ...
          async () => {
            ...
            try {
              ...
              if (!meta.userId && meta.eventType !== "MessageSent") {
              ...
              if (!ok) return;
              try {
                if (meta.eventType === "MessageSent") {
                  const j = JSON.parse(v.toString("utf8")) as Record<string, unknown>;
                  const recipientId = String(j.recipient_id || "").trim().toLowerCase();
                  const senderId = String(j.sender_id || "").trim();
                  const messageId = String(j.message_id || meta.eventId).trim().toLowerCase();
                  const threadId = String(j.thread_id || "").trim();
                  const preview = String(j.content || "").trim().slice(0, 160);
                  const listingId = String(j.listing_id || "").trim() || null;
                  const listingTitle = String(j.listing_title || j.listingTitle || "").trim() || null;
                  if (/^[0-9a-f-]{36}$/i.test(recipientId) && recipientId !== senderId) {
                    const inserted = await createMessageReceivedNotification(pool, {
                      ...
                      listingId,
                      listingTitle,
                      notificationSource: "kafka.messaging.MessageSent",
                    });
                    if (inserted.inserted) {
                      await invalidateNotificationListCacheForUser(recipientId);
