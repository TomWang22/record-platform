...
import {
  ...
  assertRedactedRow,
} from '../scripts/lib/phase28-controlled-matrix-summary.mjs';
...
describe('phase28 controlled matrix summary', () => {
  ...
  it('rejects forbidden fields in rows', () => {
    assert.throws(() => assertRedactedRow({ response_body: 'secret' }), /forbidden field/);
  });

  it('expects full matrix target constants', () => {
    assert.equal(MATRIX_TARGET.total, 25920);
    assert.equal(MATRIX_TARGET.perProtocol, 8640);
  });
});
