...
function chaosReadinessScore(clusterBrokerCount, leaderEntropyRatio, underReplicatedPartitions) {
    if (clusterBrokerCount < 2)
        return 1;
    const urFactor = underReplicatedPartitions === 0 ? 1 : Math.max(0, 1 - underReplicatedPartitions / 20);
    return Math.min(1, 0.7 * leaderEntropyRatio + 0.3 * urFactor);
}
export function validateContract(config, expectedTopics, topicMetadata, brokerConfigEntries, clusterBrokerCount) {
    const errors = [];
    ...
    for (const topicName of expectedTopics) {
        ...
        for (const p of t.partitions) {
            validatePartition(config, topicName, p, errors, brokerRack, config.requireRackAwareness && hasRackInfo);
...
function validatePartition(config, topicName, p, errors, brokerRack, rackAware) {
    ...
    if (rackAware && p.replicas.length > 1) {
        const racks = new Set(p.replicas.map((id) => {
