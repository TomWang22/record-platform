...
export default defineConfig(async () => {
  ...
  process.env.RP_KAFKA_TOPIC_SUFFIX = `.sys-${process.pid}-${Date.now()}`;
  ...
  const suffixClean = process.env.RP_KAFKA_TOPIC_SUFFIX.replace(/^\.+/u, "").replace(/[^a-zA-Z0-9_.-]/gu, "-");
  process.env.ANALYTICS_LISTING_KAFKA_GROUP ??= `rp-sys-contract-${suffixClean}`;
  ...
  const env: Record<string, string> = {
    ...
    RP_KAFKA_TOPIC_SUFFIX: process.env.RP_KAFKA_TOPIC_SUFFIX!,
