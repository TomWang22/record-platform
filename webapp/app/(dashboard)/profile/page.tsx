    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-lg font-semibold">{user?.name ?? 'Collector'}</p>
            <p className="text-sm text-slate-500">{user?.email}</p>
            <p className="mt-1 text-xs uppercase text-slate-400">Provider: {user?.provider}</p>
          </div>
        </div>
      </Card>
      <div className="grid gap-4 sm:grid-cols-3">
        <Card><p className="text-xs text-slate-500">Records</p><p className="text-2xl font-bold">{recordsCount}</p></Card>
        <Card><p className="text-xs text-slate-500">Feedback score</p><p className="text-2xl font-bold">—</p></Card>
        <Card><p className="text-xs text-slate-500">Active listings</p><p className="text-2xl font-bold">—</p></Card>
      </div>
