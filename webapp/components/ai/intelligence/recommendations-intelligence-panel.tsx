...
export function RecommendationsIntelligencePanel({ principalId, candidates }: RecommendationsIntelligencePanelProps) {
  ...
  return <IntelligencePanelShell title="Recommendations" description="Explainable matches only. Budget and negative preferences are enforced; sponsorship never affects rank and appreciation is not predicted." testId="intelligence-recommendations-panel" loading={state.loading} errorMessage={state.error} rateLimited={state.rateLimited} limitations={(state.result?.limitations as never) || []} evidence={(state.result?.evidence as never) || []} freshnessLabel="no_pay_to_rank=true; appreciation_prediction=false">
    <div className="space-y-2 text-sm">
      <button type="button" onClick={() => void run()} disabled={!principalId || state.loading} className="rounded-md bg-brand px-3 py-1.5 text-xs font-medium text-white disabled:opacity-50">Get recommendations</button>
