...
import { extractSourceTypes } from './helpers/ai-rag'
import {
  RECORD_INTELLIGENCE_PROMPTS,
  buildRecordIntelAggregate,
  evaluateDomainScenario,
  extractApiExcerpts,
  isOldBoilerplateOnly,
  leakageCheck,
  printRecordIntelConsoleSummary,
  responseExcerpt,
  writeRecordIntelArtifacts,
  type RecordIntelCaseResult,
  type RecordIntelSessionResult,
} from './helpers/ai-rag-record-intelligence'
...
test.describe('AI record intelligence UI acceptance (T20.13R)', () => {
  ...
  test('UI walkthrough — seven domain scenarios with evaluator capture', async ({ page, browser }) => {
    const timestamp = runTimestamp()
    const baseURL = (process.env.E2E_API_BASE ?? 'https://record-platform.test').replace(/\/$/, '')
    const viewport = page.viewportSize() ?? { width: 1280, height: 720 }
    const cases: RecordIntelCaseResult[] = []
    ...
    for (const { id, name, prompt } of RECORD_INTELLIGENCE_PROMPTS) {
      ...
      if (envelope?.summary) {
        ...
        if (await errorEl.count()) {
          answerText = (await errorEl.innerText()).trim()
        }
      }
      ...
      const apiExcerpts = envelope ? extractApiExcerpts(envelope) : []
      const domain = evaluateDomainScenario(id, answerText, sourceTypes)
      ...
      cases.push({
        ...
        useful: domain.answer_usefulness === 'high' ? 'yes' : domain.answer_usefulness === 'medium' ? 'partial' : 'no',
        old_boilerplate_only: oldBoilerplate,
        api_excerpts: apiExcerpts,
        domain,
      })

      expect.soft(httpStatus, `${id} HTTP`).toBe(200)
      ...
      expect.soft(domain.safety, `${id} safety`).toBe('pass')
    }

    const session: RecordIntelSessionResult = {
      ticket: 'T20.13R',
      ...
      cases,
      aggregate: buildRecordIntelAggregate(cases),
    }

    const paths = writeRecordIntelArtifacts(session, timestamp)
    printRecordIntelConsoleSummary(session, paths)

    expect(session.aggregate.ui_pass).toBe(7)
    expect(session.aggregate.cases).toBe(7)
    expect(session.aggregate.keyword_rule_engine).toBe(7)
    expect(session.aggregate.leakage).toBe('PASS')
    expect(session.aggregate.old_boilerplate_regression).toBe(false)
    expect(session.aggregate.avg_domain_score).toBeGreaterThanOrEqual(3.5)
    expect(session.aggregate.major_overclaiming).toBe(0)
    expect(session.aggregate.scenarios_with_next_action).toBeGreaterThanOrEqual(4)
  })
})
