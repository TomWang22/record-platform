...
export async function ensureInboxThreadForFilters(request: APIRequestContext): Promise<void> {
  ...
  const listingId = await createListingWithShipping(request, sellerToken, {
    title: `Filter inbox seed ${Date.now()}`,
  })
  ...
  const send = await request.post('/api/messages/send', {
    ...
    data: {
      ...
      message_type: 'direct',
      subject: `[listing:${listingId}] Filter inbox seed listing`,
